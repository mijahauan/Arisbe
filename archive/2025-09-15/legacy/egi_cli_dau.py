...
try:
    ...
    from egi_transformations_dau import (
        TransformationError,
        apply_de_iteration,
        apply_double_cut_addition,
        apply_double_cut_removal,
        apply_erasure,
        apply_insertion,
        apply_isolated_vertex_addition,
...
"""
    ...
    def _show_graph_info(self):
        ...
        print(
            f"Structure: {len(graph.V)} vertices, {len(graph.E)} edges, {len(graph.Cut)} cuts"
        )
    ...
    def _apply_isolated_vertex_addition(self, args: List[str]):
        ...
        is_generic = label is None or not (
    ...
    def _apply_isolated_vertex_removal(self, args: List[str]):
        ...
        try:
            self._save_to_history()
            self.current_graph = apply_isolated_vertex_removal(
                self.current_graph, vertex_id
            )
...
Examples:
  ...
  python egi_cli_dau.py --egif "*x" --transform addvertex  # Transform
    ...
    parser.add_argument("--transform", help="Transformation to apply")
