...
class SubgraphSelector:
    ...
    def select_by_bounds(
        ...
        width: float,
        height: float,
        element_positions: Dict[ElementID, Tuple[float, float]],
    ) -> SubgraphSelection:
    ...
    def select_by_elements(self, element_ids: Set[ElementID]) -> SubgraphSelection:
        ...
        return SubgraphSelection(
            ...
            selection_bounds=(
                ...
                0,
                0,
            ),  # Bounds not applicable for explicit selection
        )
    ...
    def _determine_context_polarity(self, area_id: ElementID) -> TransformationContext:
        ...
        return (
            TransformationContext.POSITIVE
            if depth % 2 == 0
            else TransformationContext.NEGATIVE
        )
...
class TransformationRuleDiscovery:
    ...
    def discover_applicable_rules(
        self, selection: SubgraphSelection
    ) -> List[ApplicableRule]:
        ...
        for rule in self.rule_catalog:
            confidence = self._evaluate_rule_applicability(
                rule, selection, vertices, edges, cuts
            )
            if confidence > 0.0:
    ...
    def _evaluate_rule_applicability(
        self,
        rule: ApplicableRule,
        selection: SubgraphSelection,
...
class WorkflowOrchestrator:
    ...
    def _generate_preview_description(
        self, rule: ApplicableRule, selection: SubgraphSelection
    ) -> str:
    ...
    def _predict_resulting_elements(
        self, rule: ApplicableRule, selection: SubgraphSelection
    ) -> Set[ElementID]:
        ...
        if rule.rule_name == "insertion":
            ...
            return selection.elements | {
                f"new_{rule.rule_name}_{len(selection.elements)}"
            }
        elif rule.rule_name == "erasure":
...
def demonstrate_user_workflow():
    ...
    nu_mapping = frozendict({"e1": ("v1", "v2"), "e2": ("v2", "v3")})

    area_mapping = frozendict(
        {"sheet": frozenset(["v1", "e1", "c1"]), "c1": frozenset(["v2", "e2", "v3"])}
    )

    rel_mapping = frozendict({"e1": "Loves", "e2": "Knows"})
    ...
    print("=" * 50)
    print(
        f"📍 Selected {len(selection.elements)} elements in {selection.context_polarity.value} context"
    )
