...
class CompositionTransformer:
    ...
    def _setup_standard_contexts(self):
        ...
        deep_context = StandardCompositionContexts.create_deeper_context(
            "deep", depth=4
        )
        self.context_manager.register_context(deep_context)

    def start_composition_session(
        self, context_id: str, session_id: str, description: str = ""
    ) -> CompositionSession:
        """
    ...
    def apply_transformation_in_context(
        self,
        rule_name: str,
        ...
        **kwargs,
        ...
        request = TransformationRequest(
            ...
            operation_details={"selected_subgraph": selected_elements or [], **kwargs},
    ...
    def build_simple_graph(
        self, predicates: List[str], connections: List[Tuple[str, str]] = None
    ) -> TransformationResponse:
        """
    ...
    def create_endoporeutic_game_session(
        self, domain_model_egif: str, game_id: str = "endoporeutic_game"
    ) -> EndoporeuticGameContext:
        """
...
def test_composition_transformer():
    ...
    session = transformer.start_composition_session(
        "standard", "test_session_1", "Testing basic graph building"
    )
    ...
    if analysis:
        print(
            f"Initial state - Cuts: {analysis.cut_count}, Areas: {len(analysis.areas)}"
        )
    ...
    try:
        ...
        session2 = transformer2.start_composition_session(
            "standard", "test_session_2", "Testing connected graph building"
        )
