...
class EGIExporter:
    ...
    def export_drawing_schema(
        ...
        try:
            ...
            if format == ExportFormat.EGI_JSON:
                ...
                return ExportResult(
                    success=False, errors=[f"Unsupported export format: {format}"]
                )
    ...
    def _convert_to_egi(self, drawing_schema: Dict) -> "EGIConversionResult":
        ...
        try:
            ...
            return EGIConversionResult(
                success=len(errors) == 0, egi=egi, errors=errors, warnings=warnings
            )

        except Exception as e:
            return EGIConversionResult(
                success=False, errors=[f"EGI conversion failed: {str(e)}"]
            )
        ...
        try:
            ...
            egi_data = {
                ...
                "egi": {
                    ...
                    "cuts": [
                        {"id": c.id, "area_contents": list(egi.area.get(c.id, []))}
                        for c in egi.Cut
                    ],
                    "sheet": egi.sheet,
            ...
            if output_path:
                ...
                return ExportResult(
                    success=True, content=content, format=ExportFormat.EGI_JSON
                )

        except Exception as e:
            return ExportResult(
                success=False, errors=[f"EGI JSON export failed: {str(e)}"]
            )
    ...
    def _export_egif(
        ...
        try:
            ...
            if output_path:
                ...
                return ExportResult(
                    success=True, content=content, format=ExportFormat.EGIF
                )

        except Exception as e:
            return ExportResult(success=False, errors=[f"EGIF export failed: {str(e)}"])
    ...
    def _export_cgif(
        ...
        try:
            ...
            return ExportResult(success=False, errors=[f"CGIF export failed: {str(e)}"])
    ...
    def _export_clif(
        ...
        try:
            ...
            return ExportResult(success=False, errors=[f"CLIF export failed: {str(e)}"])

    def _export_drawing_schema(
        self, drawing_schema: Dict, output_path: Optional[Path]
    ) -> ExportResult:
        ...
        try:
            ...
            if output_path:
                ...
                return ExportResult(
                    success=True, content=content, format=ExportFormat.DRAWING_SCHEMA
                )

        except Exception as e:
            return ExportResult(
                success=False, errors=[f"Drawing schema export failed: {str(e)}"]
            )
    ...
    def validate_drawing_schema(self, drawing_schema: Dict) -> List[str]:
        ...
        if "vertices" in drawing_schema and not isinstance(
            drawing_schema["vertices"], list
        ):
            errors.append("Field 'vertices' must be a list")

        if "predicates" in drawing_schema and not isinstance(
            drawing_schema["predicates"], list
        ):
            errors.append("Field 'predicates' must be a list")

        if "ligatures" in drawing_schema and not isinstance(
            drawing_schema["ligatures"], list
        ):
...
class ExportManager:
    ...
    def export_with_dialog(
        self, drawing_schema: Dict, suggested_name: str = "graph"
    ) -> ExportResult:
        ...
        return self.exporter.export_drawing_schema(
            drawing_schema, ExportFormat.EGIF, output_path
        )
    ...
    def export_to_string(
        self, drawing_schema: Dict, format: ExportFormat
    ) -> ExportResult:
