...
from egi_spatial_correspondence import LigatureGeometry, SpatialBounds, SpatialElement
...
class NetworkXEGILayoutEngine:
    ...
    def generate_layout(
        self, egi, area_bounds: Dict[str, SpatialBounds]
    ...
    def _compute_graphviz_layout(self, G: nx.DiGraph) -> GraphvizLayoutResult:
        ...
        try:
            ...
            return GraphvizLayoutResult(
                node_positions=pos, cluster_bounds=cluster_bounds, edge_paths=edge_paths
            )
    ...
    def _estimate_cluster_bounds(
        self, G: nx.DiGraph, pos: Dict[str, Tuple[float, float]]
    ) -> Dict[str, SpatialBounds]:
    ...
    def _scale_and_separate_positions(
    ...
    def _create_manual_area_layout(
        self, G: nx.DiGraph
    ) -> Dict[str, Tuple[float, float]]:
    ...
    def _compute_proper_cluster_bounds(
        self, G: nx.DiGraph, pos: Dict[str, Tuple[float, float]]
    ) -> Dict[str, SpatialBounds]:
    ...
    def _extract_spatial_elements(
        self, egi, layout_result: GraphvizLayoutResult
    ) -> Dict[str, SpatialElement]:
            ...
            if vertex.id in layout_result.node_positions:
                ...
                spatial_layout[vertex.id] = SpatialElement(
                    ...
                    spatial_bounds=SpatialBounds(
                        x=x - 15, y=y - 10, width=30, height=20
                    ),
        ...
        for edge in egi.E:
            if edge.id in layout_result.node_positions:
                ...
                spatial_layout[edge.id] = SpatialElement(
                    ...
                    spatial_bounds=SpatialBounds(
                        x=x - 25, y=y - 12, width=50, height=24
                    ),
                    ligature_geometry=None,
                )
    ...
    def _add_networkx_ligatures(
        self, egi, layout: Dict[str, SpatialElement]
    ) -> Dict[str, SpatialElement]:
