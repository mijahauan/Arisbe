...
def create_large_nested_egi(
    num_root_cuts: int = 20, max_depth: int = 4
    ...
    def create_nested_structure(
        parent_area: str, depth: int, cuts_at_level: int
    ) -> None:
...
def benchmark_spatial_queries(
    view_system: HierarchicalViewSystem, num_queries: int = 1000
) -> None:
    ...
    print(
        f"  - Average per query: {((end_time - start_time) / num_queries) * 1000:.3f} ms"
    )
    print(f"  - Total contexts found: {total_results}")
    print(f"  - Average contexts per query: {total_results / num_queries:.1f}")


def demonstrate_context_collapse(
    view_system: HierarchicalViewSystem, egi: RelationalGraphWithCuts
) -> None:
...
def analyze_scalability(
    ...
    if visible_contexts:
        visible_area = sum(
            c.spatial_bounds.area() for c in visible_contexts if c.spatial_bounds
        )
...
def main():
    ...
    for case_name, num_roots, max_depth in test_cases:
        ...
        print(
            f"Cuts per ms: {len(egi.Cut) / ((creation_time + build_time) * 1000):.1f}"
        )
