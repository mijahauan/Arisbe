...
class ErgasterionContext:
    ...
    def create_origination_sequence(
        self, initial_egi_id: str, description: str = ""
    ) -> str:
    ...
    def apply_transformation_to_sequence(
        self,
        sequence_id: str,
        rule_type: TransformationRuleType,
        transformation_data: Dict[str, Any],
        ...
        step = self.repository.transformation_steps[
            [
                s
                for s in self.repository.transformation_steps.values()
...
def demonstrate_immutable_architecture():
    ...
    for i, step in enumerate(sequence.steps, 1):
        print(
            f"   {i}. {step.rule_type.value}: {step.source_egi_id} → {step.target_egi_id}"
        )
