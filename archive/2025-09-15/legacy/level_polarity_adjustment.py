...
class LevelPolarityAdjuster:
    ...
    def calculate_adjustment(
        self, rule_name: str, source_context_depth: int, target_context_depth: int
    ) -> PolarityAdjustment:
        """
    ...
    def adjust_subgraph_polarity(
        self, subgraph_egi: RelationalGraphWithCuts, adjustment: PolarityAdjustment
    ...
    def _adjust_cut_levels(
        self, egi: RelationalGraphWithCuts, depth_change: int
    ) -> RelationalGraphWithCuts:
        """
    ...
    def calculate_nesting_depth(
        self, area_id: ElementID, egi: RelationalGraphWithCuts
    ) -> int:
...
def test_polarity_adjustment():
    ...
    dc_minus_adj = adjuster.calculate_adjustment(
        "DC-", source_context_depth=2, target_context_depth=0
    )
    print(
        f"DC- adjustment: depth_change={dc_minus_adj.depth_change}, requires_adjustment={dc_minus_adj.requires_adjustment}"
    )

    # Test 2: DC+ adjustment calculation
    print("\n--- Test 2: DC+ Adjustment ---")
    dc_plus_adj = adjuster.calculate_adjustment(
        "DC+", source_context_depth=0, target_context_depth=2
    )
    print(
        f"DC+ adjustment: depth_change={dc_plus_adj.depth_change}, requires_adjustment={dc_plus_adj.requires_adjustment}"
    )

    # Test 3: INS adjustment calculation
    print("\n--- Test 3: INS Adjustment ---")
    ins_adj = adjuster.calculate_adjustment(
        "INS", source_context_depth=0, target_context_depth=1
    )
    print(
        f"INS adjustment: depth_change={ins_adj.depth_change}, requires_adjustment={ins_adj.requires_adjustment}"
    )

    # Test 4: ERA no adjustment
    print("\n--- Test 4: ERA No Adjustment ---")
    era_adj = adjuster.calculate_adjustment(
        "ERA", source_context_depth=1, target_context_depth=0
    )
    print(
        f"ERA adjustment: depth_change={era_adj.depth_change}, requires_adjustment={era_adj.requires_adjustment}"
    )
