...
class EGILineage:
    ...
    def add_descendant(
        self, descendant_egi_id: str, transformation_step_id: str
    ) -> None:
...
class ProvenanceTracker:
    ...
    def get_transformation_chain(self, egi_id: str) -> Optional[TransformationChain]:
        ...
        for chain in self.transformation_chains.values():
            ...
            for step_id in chain.transformation_steps:
                ...
                if step and (
                    step.source_egi_id == egi_id or step.target_egi_id == egi_id
                ):
                    return chain
        return None
    ...
    def analyze_transformation_patterns(
        self, egi_ids: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_chain_length": sum(
                chain.chain_length for chain in self.transformation_chains.values()
            )
            / max(len(self.transformation_chains), 1),
        }
    ...
    def _update_lineages_for_transformation(
        self, step: TransformationStep, event_id: str
    ) -> None:
...
def demonstrate_provenance_tracking():
    ...
    for i, transform in enumerate(transformations, 1):
        ...
        if step:
            ...
            print(
                f"🔄 Transformation {i}: {transform['rule_type'].value} → {new_egi_id[:8]}..."
            )
    ...
    print(
        f"Current State: {report['current_state']['vertices']}V, {report['current_state']['edges']}E, {report['current_state']['cuts']}C"
    )
