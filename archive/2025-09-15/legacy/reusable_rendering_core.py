...
class CutRenderer:
    ...
    def get_cut_style(self, nesting_depth: int) -> RenderingStyle:
        ...
        opacity = max(
            0.3, 1.0 - nesting_depth * 0.15
        )  # More transparent for deeper nesting
...
class LigatureRenderer:
    ...
    def render_identity_ligature(
        self, vertex_positions: List[Tuple[float, float]]
    ) -> List[str]:
        ...
        if len(vertex_positions) == 2:
            ...
            if distance > 100:  # Cross-area ligature - use curved path
                ...
                elements.append(
                    f'<path d="{path}" stroke="black" stroke-width="2" fill="none"/>'
                )
            else:
            ...
            for x, y in vertex_positions:
                elements.append(
                    f'<line x1="{branch_x}" y1="{branch_y}" x2="{x}" y2="{y}" stroke="black" stroke-width="2"/>'
                )

            # Mark branch point
            elements.append(
                f'<circle cx="{branch_x}" cy="{branch_y}" r="2" fill="black"/>'
            )

        return elements

    def render_predicate_ligature(
        self,
        predicate_rect: Tuple[float, float, float, float],
