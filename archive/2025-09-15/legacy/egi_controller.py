...
from logic_spatial_validator import ValidationResult, create_validator
...
class EGIController:
    ...
    def handle_user_interaction(self, interaction: UserInteraction) -> OperationResult:
        ...
        if interaction.action == "add_vertex":
            vertex_id = interaction.parameters.get(
            ...
            edge_id = interaction.parameters.get(
            ...
            cut_id = interaction.parameters.get(
    ...
    def _refresh_all_presentations(self) -> None:
        ...
        if self.current_spatial_layout:
            current_egi = self.egi_system.get_egi()
            self.current_validation_result = self.validator.validate_concordance(
                current_egi, self.current_spatial_layout
            )

            # Log validation results
            print(
                f"Logic-Spatial Validation: {self.current_validation_result.validation_summary}"
            )
    ...
    def _generate_area_aligned_layout(self) -> Dict[str, Any]:
        ...
        if self.use_networkx_layout:
            try:
                ...
                try:
                    layout = self.networkx_engine.generate_layout(
                        current_egi, area_bounds
                    )
                    return layout
                finally:
    ...
    def _generate_validated_layout(self, egi) -> Dict[str, Any]:
        ...
        for cut in egi.Cut:
            ...
            cut_requirements = self._determine_cut_spatial_requirements(
                cut.id, parent_area, egi
            )
            ...
            # Add cut to layout
            from egi_spatial_correspondence import SpatialBounds, SpatialElement
        ...
        for edge in egi.E:
            if edge.id in layout:
                ...
                if len(connected_vertices) >= 1:
                    ...
                    edge_center = (
                        edge_element.spatial_bounds.x
                        + edge_element.spatial_bounds.width / 2,
                        edge_element.spatial_bounds.y
                        + edge_element.spatial_bounds.height / 2,
                    )
                    ...
                    for vertex_id in connected_vertices:
                        if vertex_id in layout:
                            ...
                            vertex_center = (
                                vertex_element.spatial_bounds.x
                                + vertex_element.spatial_bounds.width / 2,
                                vertex_element.spatial_bounds.y
                                + vertex_element.spatial_bounds.height / 2,
                            )
                            ...
                            if edge_area == vertex_area:
                                ...
                                routed_path = self._route_ligature_around_cuts(
                                    edge_center, vertex_center, layout
                                )
    ...
    def _ligature_crosses_cut_boundary(
        self,
    ...
    def _line_intersects_rectangle(
        self, p1: Tuple[float, float], p2: Tuple[float, float], bounds
    ) -> bool:
        ...
        if self._line_segments_intersect(
            (x1, y1), (x2, y2), (rect_x, rect_y), (rect_x, rect_y + rect_h)
        ):
            return True
        # Right edge
        if self._line_segments_intersect(
        ...
        # Top edge
        if self._line_segments_intersect(
            (x1, y1), (x2, y2), (rect_x, rect_y), (rect_x + rect_w, rect_y)
        ):
            return True
        # Bottom edge
        if self._line_segments_intersect(
    ...
    def _route_ligature_around_cuts(
        self,
        start_point: Tuple[float, float],
        end_point: Tuple[float, float],
        layout: Dict[str, Any],
        ...
        if self._path_intersects_any_obstacle(path, obstacles):
            ...
            for extra in (8, 16, 24, 36):
                ...
                path = self._find_path_around_obstacles(
                    start_point, end_point, inflated
                )
        ...
        if self._path_intersects_any_obstacle(path, obstacles):
            raise ValueError(
                "Ligature routing failed: path crosses cut boundary in same area"
            )
    ...
    def _find_path_around_obstacles(
        ...
        if not any(
            self._line_intersects_rectangle_dict(start, end, o) for o in obstacles
        ):
            return [start, end]
        ...
        open_set = [
            (0 + heuristic(nodes[start_idx], nodes[goal_idx]), 0, start_idx, -1)
        ]
    ...
    def _path_intersects_any_obstacle(
        self, path: List[Tuple[float, float]], obstacles: List[Dict[str, float]]
    ) -> bool:
    ...
    def _line_intersects_rectangle_dict(
        self, p1: Tuple[float, float], p2: Tuple[float, float], rect: Dict[str, float]
    ) -> bool:
    ...
    def _route_around_rectangle(
        ...
        best_corner = min(
            corners, key=lambda c: (c[0] - mid_x) ** 2 + (c[1] - mid_y) ** 2
        )

        return [start, best_corner, end]

    def _determine_cut_spatial_requirements(
        self, cut_id: str, parent_area: str, egi
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "spatial_constraints": self._analyze_element_spatial_constraints(
                cut_elements, egi
            ),
        }

    def _analyze_element_spatial_constraints(
        self, elements: set, egi
    ) -> List[Dict[str, Any]]:
    ...
    def _find_cut_position_for_requirements(
        self,
        ...
        for attempt in range(30):
            ...
            proposed_bounds = {"x": x, "y": y, "width": cut_width, "height": cut_height}

            # Check if position satisfies all requirements
            if (
                self._bounds_within_parent(proposed_bounds, parent_bounds)
                and not self._overlaps_existing_elements(
                    proposed_bounds, existing_layout
                )
                and self._satisfies_spatial_constraints(
                    proposed_bounds, requirements["spatial_constraints"]
                )
            ):
    ...
    def _satisfies_spatial_constraints(
        self, bounds: Dict[str, float], constraints: List[Dict[str, Any]]
    ) -> bool:
    ...
    def _bounds_within_parent(
        self, child_bounds: Dict[str, float], parent_bounds: Dict[str, float]
    ) -> bool:
        """Check if child bounds are completely within parent bounds."""
        return (
            child_bounds["x"] >= parent_bounds["x"]
            and child_bounds["y"] >= parent_bounds["y"]
            and child_bounds["x"] + child_bounds["width"]
            <= parent_bounds["x"] + parent_bounds["width"]
            and child_bounds["y"] + child_bounds["height"]
            <= parent_bounds["y"] + parent_bounds["height"]
        )

    def _overlaps_existing_elements(
        self, proposed_bounds: Dict[str, float], existing_layout: Dict[str, Any]
    ) -> bool:
    ...
    def add_edge(self) -> OperationResult:
        ...
        if len(current_egi.V) == 0:
            return OperationResult(
                success=False, error="Need vertices before adding edges"
            )
    ...
    def validate_current_layout(self) -> ValidationResult:
        ...
        if self.current_spatial_layout:
            ...
            from logic_spatial_validator import (
                ValidationResult,
                ValidationViolation,
                ViolationType,
            )
    ...
    def _find_non_colliding_position(
        ...
        exclusion_zones = self._get_exclusion_zones_for_area(
            area_bounds, existing_layout
        )
        ...
        for _ in range(max_attempts):
            ...
            x = (
                area_bounds["x"]
                + margin
                + random.random() * (area_bounds["width"] - width - 2 * margin)
            )
            y = (
                area_bounds["y"]
                + margin
                + random.random() * (area_bounds["height"] - height - 2 * margin)
            )
    ...
    def _get_exclusion_zones_for_area(
        self, area_bounds: Dict[str, float], existing_layout: Dict[str, Any]
    ) -> List[Dict[str, float]]:
    ...
    def _position_in_exclusion_zones(
        self, proposed_bounds: Dict[str, float], exclusion_zones: List[Dict[str, float]]
    ) -> bool:
    ...
    def _bounds_contained_in(
        self, inner_bounds: Dict[str, float], outer_bounds: Dict[str, float]
    ) -> bool:
        """Check if inner bounds are completely contained within outer bounds."""
        return (
            inner_bounds["x"] >= outer_bounds["x"]
            and inner_bounds["y"] >= outer_bounds["y"]
    ...
    def _bounds_overlap_dict(
        self, bounds1: Dict[str, float], bounds2: Dict[str, float]
    ) -> bool:
        """Check if two bounds dictionaries overlap."""
        return not (
            bounds1["x"] + bounds1["width"] <= bounds2["x"]
            or bounds2["x"] + bounds2["width"] <= bounds1["x"]
            or bounds1["y"] + bounds1["height"] <= bounds2["y"]
            or bounds2["y"] + bounds2["height"] <= bounds1["y"]
        )
    ...
    def get_valid_placement_areas(
        self, element_type: str, element_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def check_collision(
        self, element_bounds: Dict[str, float], exclude_element: Optional[str] = None
    ) -> bool:
    ...
    def validate_element_movement(
        self, element_id: str, new_position: Tuple[float, float]
    ) -> Dict[str, Any]:
    ...
    def _can_place_element_in_area(
        self, element_type: str, area_id: str, element_id: Optional[str] = None
    ) -> bool:
    ...
    def _point_in_bounds(
        self, point: Tuple[float, float], bounds: Dict[str, float]
    ) -> bool:
