...
class ArisbeMainApplication:
    ...
    def setup_main_interface(self):
        ...
        title_label = ttk.Label(title_frame, text="ARISBE", font=("Arial", 24, "bold"))
        title_label.pack()
    ...
    def create_module_panel(
        self, parent, title, subtitle, description, command, row, col
    ):
        ...
        desc_label = ttk.Label(
            panel, text=description, font=("Arial", 9), justify=tk.LEFT, wraplength=250
        )
        desc_label.pack(pady=10, padx=10)

        # Launch button
        launch_btn = ttk.Button(
            panel, text=f"Launch {title}", command=command, style="Accent.TButton"
        )
