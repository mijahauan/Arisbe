...
try:
    ...
    from egi_export import ExportFormat, get_export_manager

    from corpus_integration import CorpusCategory, get_corpus_integration
...
class ExportDockWidget(QDockWidget):
        ...
        try:
            ...
            if result.success:
                ...
                QMessageBox.information(
                    self, "Export Success", f"Exported to clipboard as {format_name}"
                )
            else:
                error_msg = (
    ...
    def export_to_file(self):
        ...
        # Get file extension
        extensions = {"EGIF": "egif", "EGI JSON": "json", "Drawing Schema": "json"}
        ...
        try:
            ...
            if result.success:
                QMessageBox.information(
                    self, "Export Success", f"Exported to {filename}"
                )
            else:
                error_msg = (
                    "\n".join(result.errors) if result.errors else "Unknown error"
    ...
    def quick_egif_export(self):
        ...
        try:
            ...
            if result.success:
                QMessageBox.information(
                    self, "Export Success", f"Quick exported to {result.file_path}"
                )
            else:
                error_msg = (
                    "\n".join(result.errors) if result.errors else "Unknown error"
...
class DrawingEditorIntegration:
    ...
    def on_corpus_item_selected(self, item_id: str, metadata: dict):
        ...
        try:
            ...
            if not corpus_integration:
                QMessageBox.warning(
                    self.drawing_editor, "Error", "Corpus integration not available"
                )
                return
    ...
    def _populate_model_from_schema(self, schema: dict):
        ...
        print(
            f"Received schema with {len(schema.get('vertices', []))} vertices, "
