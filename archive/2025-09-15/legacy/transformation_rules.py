...
class TransformationRule(ABC):
    ...
    @abstractmethod
    def validate(
        self, graph: RelationalGraphWithCuts, context: TransformationContext
    ) -> ValidationResult:
    ...
    def get_context_polarity(
        self, graph: RelationalGraphWithCuts, area_id: str
    ) -> ContextPolarity:
...
class InsertionRule(TransformationRule):
    ...
    def validate(
        ...
        if target_area == graph.sheet:
            ...
            if polarity == ContextPolarity.NEGATIVE:
                suggestions.append(
                    "Inserting into negative context - consider logical implications"
                )
    ...
    def apply(
        ...
        if not validation.is_valid:
            return TransformationResult(success=False, validation_result=validation)
...
class TransformationEngine:
    ...
    def _initialize_rules(self):
        ...
        self.rules[TransformationRuleType.DOUBLE_CUT_INSERTION] = (
    ...
    def get_rule(
        self, rule_type: TransformationRuleType
    ...
    def validate_transformation(
        ...
        if not rule:
            return ValidationResult(
                is_valid=False, errors=[f"Unknown rule type: {rule_type}"]
            )
    ...
    def get_applicable_rules(
        self, graph: RelationalGraphWithCuts, selected_elements: List[str]
    ) -> List[Tuple[TransformationRuleType, ValidationResult]]:
        ...
        for rule_type, rule in self.rules.items():
            ...
            context = TransformationContext(
                rule_type=rule_type, source_elements=selected_elements
            )
