...
class TransformationRulePalette(QWidget):
    ...
    def setup_ui(self):
        ...
        # Rule categories
        self.create_rule_category(
            "Basic Rules",
            [
                (
                    TransformationRuleType.INSERTION,
                    "Always valid - insert any graph anywhere",
                ),
...
class PracticeSessionPanel(QWidget):
    ...
    def start_session(self):
        ...
        self.current_session = PracticeSession(
            session_id=str(uuid.uuid4())[:8], start_time=datetime.now().isoformat()
        )
        ...
        self.progress_label.setText(
            "Session active - select elements to practice transformations"
        )
    ...
    def update_stats_display(self):
        ...
        if session.transformations_attempted > 0:
            success_rate = (
                session.transformations_successful / session.transformations_attempted
            ) * 100
...
class PracticeModeDockWidget(QDockWidget):
    ...
    def on_rule_selected(self, rule_type: TransformationRuleType):
        ...
        context = TransformationContext(
            rule_type=rule_type, source_elements=self.selected_elements.copy()
        )
    ...
    def apply_transformation(self):
        ...
        # Create transformation context
        context = TransformationContext(
            rule_type=self.current_rule, source_elements=self.selected_elements.copy()
        )
        ...
        if result.success:
            ...
            QMessageBox.information(
                self, "Success", f"Transformation applied: {result.changes_description}"
            )
        else:
