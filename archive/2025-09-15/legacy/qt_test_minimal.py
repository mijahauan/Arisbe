        ...
        try:
            ...
            if "linear_replica" in layout:
                ...
                self.status_label.setText(
                    f"EGI Graph: {layout['linear_replica']} ({len(self.scene.items())} items)"
                )
            else:
                self.status_label.setText(
                    f"EGI Graph Loaded: {len(self.scene.items())} items"
                )
...
def main():
    ...
    print(
        "qt_test_minimal.py is temporarily disabled while the headless EGI↔spatial adapter is stabilized."
