...
from subgraph_extractor import BoundaryHandling, SelectionCriteria, SubgraphExtractor
...
class PeirceDauViewGenerator(ViewGenerator):
    ...
    def _determine_visible_elements(
        ...
        for cut in egi.Cut:
            ...
            for parent_cut in egi.Cut:
                if (
                    cut.id in egi.area.get(parent_cut.id, set())
                    and context_states.get(
                        parent_cut.id, ContextState(parent_cut.id)
                    ).is_expanded
                    == False
                ):
    ...
    def _render_vertex(
        ...
        font_size = max(
            hints.min_text_size, min(hints.max_text_size, 12 * viewport.zoom_level)
        )
    ...
    def _render_edge(
        ...
        font_size = max(
            hints.min_text_size, min(hints.max_text_size, 14 * viewport.zoom_level)
        )
    ...
    def _get_nesting_depth(
        self, element_id: ElementID, egi: RelationalGraphWithCuts
    ) -> int:
...
class DynamicViewManager:
    ...
    def generate_view(
        self, egi: RelationalGraphWithCuts, viewport: Optional[ViewportBounds] = None
    ) -> GraphView:
    ...
    def get_navigation_options(
        self, egi: RelationalGraphWithCuts, current_element: ElementID
    ) -> Dict[NavigationMode, List[ElementID]]:
        ...
        if current_element in egi._vertex_map:
            ...
            for edge_id, vertices in egi.nu.items():
                if current_element in vertices:
                    options[NavigationMode.LIGATURE].extend(
                        [v for v in vertices if v != current_element]
                    )
...
class ViewBasedSubgraphSelector:
    ...
    def select_visible_subgraph(
        self, egi: RelationalGraphWithCuts, selection_criteria: SelectionCriteria
        ...
        visible_elements = {
            e.element_id for e in self.view_manager.current_view.elements if e.visible
        }
        ...
        filtered_criteria = SelectionCriteria(
            ...
            seed_elements=selection_criteria.seed_elements.intersection(
                visible_elements
            ),
    ...
    def focus_on_subgraph(
        self, egi: RelationalGraphWithCuts, subgraph_elements: Set[ElementID]
    ):
