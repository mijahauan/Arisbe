...
class InsertVertex(EGIOperation):
    ...
    def apply(self, egi: RelationalGraphWithCuts) -> RelationalGraphWithCuts:
        ...
        target_area = (
            egi.sheet
            if (self.area_id is None or self.area_id == "sheet")
            else self.area_id
        )
        if os.environ.get("ARISBE_DEBUG_EGI") == "1":
            print(
                f"DEBUG: InsertVertex.apply() - vertex_id: {self.vertex_id}, area_id: {target_area}"
            )
...
class InsertEdge(EGIOperation):
    ...
    def apply(self, egi: RelationalGraphWithCuts) -> RelationalGraphWithCuts:
        ...
        # Delegate to core helper to preserve invariants
        return egi.with_edge(
            Edge(id=self.edge_id),
            tuple(self.vertices),
            self.relation,
            context_id=target_area,
        )
...
class InsertCut(EGIOperation):
    ...
    def apply(self, egi: RelationalGraphWithCuts) -> RelationalGraphWithCuts:
        ...
        target_parent = (
            egi.sheet
            if (self.parent_area is None or self.parent_area == "sheet")
            else self.parent_area
        )
        if os.environ.get("ARISBE_DEBUG_EGI") == "1":
            print(
                f"DEBUG: InsertCut.apply() - cut_id: {self.cut_id}, parent_area: {target_parent}"
            )
        # Delegate to core helper to preserve invariants
...
class MoveElement(EGIOperation):
    ...
    def validate(self, egi: RelationalGraphWithCuts) -> bool:
        ...
        element_exists = any(
            self.element_id in area_elements for area_elements in egi.area.values()
        )
...
class DeleteElement(EGIOperation):
    ...
    def validate(self, egi: RelationalGraphWithCuts) -> bool:
        # Element must exist
        return (
            any(v.id == self.element_id for v in egi.V)
            or any(e.id == self.element_id for e in egi.E)
            or any(c.id == self.element_id for c in egi.Cut)
        )
...
class RenameVertex(EGIOperation):
    ...
    def apply(self, egi: RelationalGraphWithCuts) -> RelationalGraphWithCuts:
        ...
        if self.is_constant:
            ...
            if new_name in (alph.R or frozenset()) or new_name in (
                alph.F or frozenset()
            ):
                raise ValueError(
                    f"Constant '{new_name}' collides with relation/function names in Alphabet"
                )
        ...
        if self.is_constant:
            ...
            if new_ar.get(new_name, 1) != 1:
                new_ar[new_name] = 1
            new_alphabet = AlphabetDAU(
                C=frozenset(new_C), F=alph.F, R=alph.R, ar=frozendict(new_ar)
            ).with_defaults()
        else:
...
class VisualConformanceValidator:
    ...
    @staticmethod
    def validate_visual_operation(
        egi: RelationalGraphWithCuts, visual_change: Dict
    ) -> bool:
        ...
        if operation_type == "move_element":
            ...
            return VisualConformanceValidator._validate_cut_nesting(
                egi, element_id, target_area
            )

        elif operation_type == "resize_cut":
            # Ensure cut boundaries don't violate containment
            return VisualConformanceValidator._validate_cut_boundaries(
                egi, visual_change
            )
    ...
    @staticmethod
    def _validate_cut_nesting(
        egi: RelationalGraphWithCuts, element_id: str, target_area: str
    ) -> bool:
...
class EGIRepository:
    ...
    def apply_visual_change(self, visual_change: Dict) -> OperationResult:
        ...
        if not self.validator.validate_visual_operation(self.egi, visual_change):
            return OperationResult(
                success=False, error="Visual change violates EGI constraints"
            )
        ...
        return OperationResult(
            success=False, error="Cannot convert visual change to formal operation"
        )
    ...
    def _visual_to_operation(self, visual_change: Dict) -> Optional[EGIOperation]:
        ...
        if change_type == "move_element":
            return MoveElement(
                visual_change["element_id"], visual_change["target_area"]
            )
        elif change_type == "delete_element":
            return DeleteElement(visual_change["element_id"])
        elif change_type == "add_vertex":
            return InsertVertex(
                visual_change["vertex_id"], visual_change.get("area_id", "sheet")
            )
            ...
            return InsertCut(
                visual_change["cut_id"], visual_change.get("parent_area", "sheet")
            )
...
class EGISystem:
    ...
    def insert_vertex(
        self, vertex_id: str, area_id: Optional[str] = None
    ) -> OperationResult:
    ...
    def insert_cut(
        self, cut_id: str, parent_area: Optional[str] = None
    ) -> OperationResult:
    ...
    def rename_vertex(
        self, vertex_id: str, new_text: str, is_constant: bool
    ) -> OperationResult:
    ...
    def load_linear(
        self, text: str, format_hint: Optional[str] = None
    ) -> RelationalGraphWithCuts:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to parse as EGIF ({e1}) or CGIF ({e2})"
                ) from e2
...
class VisualEGIBridge:
    ...
    def handle_visual_event(self, event: Dict) -> OperationResult:
        ...
        return OperationResult(
            success=False, error=f"Unknown visual event: {event_type}"
        )
    ...
    def _handle_drag(self, event: Dict) -> OperationResult:
        ...
        if not self.alignment_engine:
            ...
            self.alignment_engine = create_spatial_alignment_engine(
                self.egi_system.get_egi()
            )
        ...
        if updated_layout != self.current_layout:
            ...
            if (
                old_element
                and new_element
                and old_element.logical_area != new_element.logical_area
            ):
