...
class Chapter21DiagramWidget(QWidget):
    ...
    def create_toolbar(self) -> QWidget:
        ...
        self.mode_combo.addItems(
            ["Organon (Explore)", "Ergasterion (Create)", "Agon (Evaluate)"]
        )
        ...
        self.format_combo.addItems(["Diagram", "EGIF", "CGIF", "CLIF", "FOPL"])
    ...
    def update_diagram_view(self):
        ...
        view = self.egi_engine.get_view(
            self.state.current_egi, self.state.view_specification
        )
    ...
    def synchronize_formats(self):
        ...
        try:
            ...
            equivalence_valid = self.egi_engine.validate_round_trip_equivalence(
    ...
    def start_transformation_wizard(self):
        ...
        try:
            ...
            wizard = self.wizard_system.create_wizard(
                self.state.active_format, self.state.current_egi
            )
            ...
            if result.success and result.final_egi:
                ...
                self.transformation_completed.emit(
                    result.final_egi,
                    (
                        result.transformation_applied.value
                        if result.transformation_applied
                        else "unknown"
                    ),
                )
    ...
    def on_mode_changed(self, mode_text: str):
        ...
        self.state.interaction_mode = mode_mapping.get(
            mode_text, InteractionMode.ORGANON
        )
        ...
        self.status_label.setText(
            f"Mode changed to {self.state.interaction_mode.value}"
        )
    ...
    def on_format_changed(self, format_text: str):
        ...
        self.state.active_format = format_mapping.get(
            format_text, DisplayFormat.DIAGRAM
        )
...
def test_chapter21_gui_integration():
    ...
    from egi_core_dau import Edge, ElementID, Vertex
