...
class ConcreteInsertionRule(TransformationRule):
    ...
    def apply(
        self, egi: RelationalGraphWithCuts, transformation_data: Dict[str, Any]
    ...
    def _insert_vertex(
        self, egi: RelationalGraphWithCuts, vertex_id: str, target_area: ElementID
    ) -> RelationalGraphWithCuts:
...
class ConcreteErasureRule(TransformationRule):
    ...
    def _erase_vertex(
        self, egi: RelationalGraphWithCuts, vertex_id: str
    ) -> RelationalGraphWithCuts:
    ...
    def _erase_edge(
        self, egi: RelationalGraphWithCuts, edge_id: str
    ) -> RelationalGraphWithCuts:
    ...
    def _erase_cut(
        self, egi: RelationalGraphWithCuts, cut_id: str
    ) -> RelationalGraphWithCuts:
        ...
        for cut in egi.Cut:
            if cut.id in source_elements:
                ...
                new_cut_contents = frozenset(
                    element_mapping.get(elem_id, elem_id)
                    for elem_id in original_cut_contents
                )
...
class EGITransformationPipeline:
    ...
    def apply_transformation(
        ...
        if not rule.can_apply(source_snapshot.egi_state, context_type):
            raise ValueError(
                f"Rule {rule_type} cannot be applied in context {context_type}"
            )
...
def demonstrate_concrete_transformations():
    ...
    print(
        f"✅ After vertex insertion: {len(egi_2_state.V)} vertices, {len(egi_2_state.E)} edges"
    )
    ...
    print(
        f"✅ After edge insertion: {len(egi_3_state.V)} vertices, {len(egi_3_state.E)} edges"
    )
    ...
    print(
        f"✅ After cut insertion: {len(egi_4_state.V)} vertices, {len(egi_4_state.E)} edges, {len(egi_4_state.Cut)} cuts"
    )
    ...
    egi_5_id = pipeline.apply_transformation(
        ...
        transformation_data={"source_elements": {"v1"}, "target_area": "sheet"},
        context_type=ContextType.ERGASTERION,
    ...
    print(
        f"✅ After iteration: {len(egi_5_state.V)} vertices, {len(egi_5_state.E)} edges, {len(egi_5_state.Cut)} cuts"
    )
