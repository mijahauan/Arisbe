...
class DeltaCompressor:
    ...
    def compute_transformation_delta(
        self,
        ...
        delta = TransformationDelta(
            operation=operation, result=result, structural_deltas=structural_deltas
        )
    ...
    def _compute_vertex_deltas(
        self, before: RelationalGraphWithCuts, after: RelationalGraphWithCuts
        ...
        for edge_id in set(before.nu.keys()) | set(after.nu.keys()):
            ...
            if before_vertices != after_vertices:
                deltas.append(
                    StructuralDelta(
                        delta_type=(
                            DeltaType.RELATION_MODIFIED
                            if edge_id in before.nu and edge_id in after.nu
                            else (
                                DeltaType.RELATION_ADDED
                                if edge_id in after.nu
            ...
            if before_elements != after_elements:
                deltas.append(
                    StructuralDelta(
                        delta_type=(
                            DeltaType.RELATION_MODIFIED
                            if cut_id in before.area and cut_id in after.area
                            else (
                                DeltaType.RELATION_ADDED
                                if cut_id in after.area
                                else DeltaType.RELATION_REMOVED
                            )
                        ),
                        element_id=cut_id,
    ...
    def _compress_delta(self, delta: TransformationDelta):
        ...
        delta.compression_ratio = (
            delta.compressed_size / delta.original_size
            if delta.original_size > 0
            else 0
        )
    ...
    def _vertices_equal(self, v1: Vertex, v2: Vertex) -> bool:
        ...
        return (
            v1.id == v2.id
            and getattr(v1, "label", "") == getattr(v2, "label", "")
            and getattr(v1, "type", "") == getattr(v2, "type", "")
        )
    ...
    def _edges_equal(self, e1: Edge, e2: Edge) -> bool:
        ...
        return (
            e1.id == e2.id
            and getattr(e1, "label", "") == getattr(e2, "label", "")
            and getattr(e1, "type", "") == getattr(e2, "type", "")
        )
...
class EfficientHistoricalStorage:
    ...
    def replay_from_deltas(
        self, start_egi: RelationalGraphWithCuts, delta_sequence: List[str]
    ...
    def _apply_delta(
        self, egi: RelationalGraphWithCuts, delta: TransformationDelta
    ) -> RelationalGraphWithCuts:
    ...
    def _apply_structural_delta(
        self, egi: RelationalGraphWithCuts, delta: StructuralDelta
    ):
        ...
        elif delta.delta_type in [
            DeltaType.RELATION_ADDED,
            DeltaType.RELATION_REMOVED,
            DeltaType.RELATION_MODIFIED,
        ]:
            self._modify_relation_from_delta(egi, delta)

    def _add_element_from_delta(
        self, egi: RelationalGraphWithCuts, delta: StructuralDelta
    ):
    ...
    def _remove_element_from_delta(
        self, egi: RelationalGraphWithCuts, delta: StructuralDelta
    ):
    ...
    def _modify_element_from_delta(
        self, egi: RelationalGraphWithCuts, delta: StructuralDelta
    ):
    ...
    def _modify_relation_from_delta(
        self, egi: RelationalGraphWithCuts, delta: StructuralDelta
    ):
        ...
        return {
            **self.storage_stats,
            ...
            "space_saved_percentage": (
                ...
                if self.storage_stats["average_compression_ratio"] > 0
                else 0
            ),
        }
