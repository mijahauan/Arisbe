...
class SpatialBounds:
    ...
    @classmethod
    def from_qrect(cls, rect: QRectF) -> 'SpatialBounds':
        """Create SpatialBounds from Qt QRectF."""
        return cls(rect.x(), rect.y(), rect.width(), rect.height())
    
    def to_qrect(self) -> QRectF:
        """Convert to Qt QRectF."""
    ...
    def contains_qpoint(self, point: QPointF) -> bool:
        """Check if Qt point is inside bounds."""
...
class CutSpatialInfo:
    ...
    parent_area: Optional[ElementID]  # None for root-level cuts
    nesting_level: int
    elements: Set[ElementID] = None  # Elements contained in this area
...
class RTreeCutTracker:
    """R-tree based spatial index for tracking cuts and elements."""
    ...
    def __init__(self, canvas_width: float = 10000, canvas_height: float = 10000):
        ...
        self.cuts: Dict[ElementID, CutSpatialInfo] = {}
        self.elements: Dict[ElementID, ElementID] = {}  # element_id -> containing_area_id
        self.element_positions: Dict[ElementID, SpatialBounds] = {}  # element_id -> bounds
    ...
    def remove_cut(self, cut_id: ElementID) -> bool:
    ...
    def move_cut(self, cut_id: ElementID, new_bounds: SpatialBounds) -> bool:
    ...
    def add_element(self, element_id: ElementID, bounds: SpatialBounds, area_id: ElementID):
        """Add an element to the spatial index."""
        self.elements[element_id] = area_id
        self.element_positions[element_id] = bounds
        
        # Add to area's element set
        if area_id in self.cuts:
            self.cuts[area_id].elements.add(element_id)
    
    def remove_element(self, element_id: ElementID):
        """Remove an element from the spatial index."""
        if element_id in self.elements:
            area_id = self.elements[element_id]
            del self.elements[element_id]
            del self.element_positions[element_id]
            
            # Remove from area's element set
            if area_id in self.cuts:
                self.cuts[area_id].elements.discard(element_id)
    ...
    def get_deepest_area_at_point(self, point: QPointF) -> Optional[ElementID]:
        ...
        # Check all cuts to find the deepest one containing the point
        for cut_info in self.cuts.values():
            if cut_info.bounds.contains_qpoint(point):
                if cut_info.nesting_level > max_depth:
                    max_depth = cut_info.nesting_level
                    deepest_area = cut_info.cut_id
        
        return deepest_area
    
    def find_safe_position_in_area(self, area_id: ElementID, element_size: SpatialBounds) -> Optional[QPointF]:
        """Find a collision-free position within the specified area."""
        if area_id not in self.cuts:
            return None
        
        area_bounds = self.cuts[area_id].bounds
        
        # Try positions with some spacing from edges and other elements
        margin = 20.0
        step = 30.0
        
        for y in range(int(area_bounds.top + margin), int(area_bounds.bottom - margin - element_size.height), int(step)):
            for x in range(int(area_bounds.left + margin), int(area_bounds.right - margin - element_size.width), int(step)):
                test_bounds = SpatialBounds(x, y, element_size.width, element_size.height)
                
                # Check if position is free of collisions
                if self._is_position_free(test_bounds, area_id):
                    return QPointF(x, y)
        
        # Fallback to center if no free position found
        return QPointF(area_bounds.center_x, area_bounds.center_y)
    
    def _is_position_free(self, bounds: SpatialBounds, area_id: ElementID) -> bool:
        """Check if a position is free of collisions with other elements."""
        # Check collision with child cuts
        child_cuts = self.get_cuts_in_area(area_id)
        for child_cut in child_cuts:
            if bounds.intersects(child_cut.bounds):
                return False
        
        # Check collision with other elements in the same area
        if area_id in self.cuts:
            for element_id in self.cuts[area_id].elements:
                if element_id in self.element_positions:
                    element_bounds = self.element_positions[element_id]
                    if bounds.intersects(element_bounds):
                        return False
        
        return True

    def get_cuts_in_area(self, area_id: Optional[ElementID]) -> List[CutSpatialInfo]:
        """Get all cuts directly contained in an area."""
        return [cut for cut in self.cuts.values() if cut.parent_area == area_id]

    def get_nested_cuts(self, area_id: Optional[ElementID]) -> List[CutSpatialInfo]:
    ...
    def get_spatial_extent(self, area_id: Optional[ElementID]) -> Optional[SpatialBounds]:
    ...
    def get_cut_info(self, cut_id: ElementID) -> Optional[CutSpatialInfo]:
