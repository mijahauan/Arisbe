...
function layoutSingleCut(payload) {
    ...
    const coreSim = d3.forceSimulation(coreNodes)
        .force('link', d3.forceLink(coreLinks)
            .id(d => d.id)
            .distance(d => 30)  // Realistic distance that accounts for other forces
            .strength(0.5)  // Moderate - allows geometric optimization within constraints
        )
        .force('collision', d3.forceCollide()
            .radius(d => {
                const baseRadius = Math.sqrt(d.width * d.width + d.height * d.height) / 2;
                // Both vertices and predicates need buffer for clean separation
                return baseRadius + 10;
            })
            .strength(0.6)  // Softer to allow link forces to work
            .iterations(3)
        )
        .force('charge', d3.forceManyBody()
            .strength(-50)  // Gentle repulsion helps spread elements
            .distanceMax(150)
        )
        .force('walls', forceWalls(bounds, margin))
    ...
    for (let i = 0; i < 500; ++i) coreSim.tick();
    ...
    function force(alpha) {
        for (const node of nodes) {
            ...
            for (const obstacle of fixedObstacles) {
                ...
                const minDist = nodeRadius + obstRadius + 30;  // 30px buffer for clear separation
                
                // If too close, apply EXTREMELY STRONG repulsive force
                // This MUST dominate all other forces to prevent area violations
                if (dist < minDist) {
                    // Non-linear force: exponentially stronger when very close
                    const overlap = (minDist - dist) / minDist;
                    // Exponential growth makes this force absolutely dominant
                    const strength = 5.0 * Math.exp(3 * overlap);  // Exponential explosion near boundary
