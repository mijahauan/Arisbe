...
# DEPRECATION NOTICE:
# This renderer used an older hook-line model that is not Dau-compliant.
# It is retained only for historical reference and should not be used.
# Use `src/diagram_renderer_dau.py` instead, which renders heavy identity
# lines directly to predicate peripheries with no separate hooks.
warnings.warn(
    "diagram_renderer.py is deprecated. Use diagram_renderer_dau.py (Dau-compliant, no hook lines).",
    DeprecationWarning,
    stacklevel=2,
)
...
class RenderingContext:
    ...
    # Selection state
    selected_elements: Dict[ElementID, bool] = None
    highlighted_elements: Dict[ElementID, bool] = None
    ...
    def __post_init__(self):
        if self.selected_elements is None:
            object.__setattr__(self, 'selected_elements', {})
        if self.highlighted_elements is None:
            object.__setattr__(self, 'highlighted_elements', {})
...
class ElementRenderer:
    ...
    # DEPRECATED: Predicate hooks rendering (non-Dau). Do not call in new code.
    def _render_predicate_hooks(self, center: Coordinate, 
        ...
        for _, attachment_point in attachment_points.items():
