...
|---|---|---|
| **`LOAD_ERROR: ELK layout failed: Cannot find module 'elkjs'`** — every graph fails to render, though the server starts and the API answers | The Node-side layout worker (`src/elk_worker.js`) cannot find its dependency. `node_modules/` is not tracked in git, so a fresh clone has none. This is the first thing a new checkout hits. | Run `npm install` in the repository root (about a second). Node.js is a hard prerequisite alongside Python and uv — see the install notes in `README.md` / `docs/install.qmd`. |
