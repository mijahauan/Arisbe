...
def main():
    ...
    # Determinism canary: a second MONO + FED must match. Re-run the SAME
    # arrangement fed used (broker or passive) — else the canary is a no-op on
    # the broker branch and prints a PASS it never checked.
