...
| Tropism (warm-set re-poll) | DESIGNED — **empirically mandated** | — | Run 2's finding: ingestion alone cannot test durability; only M's state directing re-engagement can. The arc's re-entry point; design draft + run-3 priors in [AUTOMATED_ENDOPOREUTIC_GAME.md](AUTOMATED_ENDOPOREUTIC_GAME.md) §13. |
