...
- **R8. Argument order IS visually encoded — by a per-style convention** (revised). Earlier this said order was not visually distinguishable and lived only in `port_index`; that was the absence of a drawn convention, not a limit of pictures. Both namesakes encode it, differently, and `style.argument_order_convention` selects which the renderer draws and `eg_reader` reads:
  ...
  - **`"clockwise"`** (Peirce, CP 4.470 — hooks "taken in their order clockwise"; Convention 13 — start from the hook vertically above the spot, optional numeric index to mark/override the start): the order is the clockwise angular order of the hooks around the spot. The **reader** recovers it from the geometry; where the layout's natural placement does *not* already read clockwise-as-`ν` (it can't always, since each line points straight at its vertex), `assign_order_labels` draws a **numeric index on just those relations** — Peirce's own Convention-13 override — and the reader uses it there. With that, the clockwise round trip recovers the full `ν` across the corpus, both engines, while clean relations stay unlabelled. (Pure geometric clockwise *placement* enforcement — no numerals at all — remains future layout work.)
