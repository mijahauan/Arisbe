...
| Schrödinger / Prigogine (the thermodynamic floor) | How does a structure persist against the decay its own physics predicts? | Any sign at all — a dissipative structure persists in service of nothing, and the tradition stops there | Beneath the membrane: the *precondition* the kytos doctrine assumes and never states | [FROM_THERMODYNAMICS_TO_SEMIOSIS.md](FROM_THERMODYNAMICS_TO_SEMIOSIS.md) §2. Nothing built: Arisbe's units are immortal and unfunded, so the floor this row names is **absent from every loop the project has run** | **queued-conjecture** |
| Bickhard (interactivism) *(examined and set aside as the threshold, 2026-07-31: "somehow indicate" and "detect failure" have already crossed the line they were called to explain)* | When does an indication acquire a truth condition *for the system itself*? | An assertion calculus and a record — anticipation with nothing drawn, licensed, or replayable | The seam the whole project sits on: self-maintenance below, representation above | [FROM_THERMODYNAMICS_TO_SEMIOSIS.md](FROM_THERMODYNAMICS_TO_SEMIOSIS.md) §3. The self-maintenant → **recursively self-maintenant** distinction is the sharpest available statement of the transition Arisbe starts past. Identified 2026-07-31 as the gap this map had: Maturana & Varela were already recorded as *not* transferring because autopoiesis refuses representation — Bickhard keeps self-maintenance as the ground and derives representation from it | **queued-conjecture** |
| Pattee (the epistemic cut) | How do rate-independent constraints arise within, and govern, rate-dependent dynamics? | A community — the cut drawn for one system, with no account of how a second reads the first's symbols | **The threshold of record** (author's ruling, 2026-07-31). The epistemic cut names what this project has worked on throughout under other names — the membrane, ink against object, map against terrain, linear form against drawn form | [FROM_THERMODYNAMICS_TO_SEMIOSIS.md](FROM_THERMODYNAMICS_TO_SEMIOSIS.md) §3a, with **intent as discriminated binding** and **Intent 0** (brute Secondness, no options) as the author's positive proposal; §4a records that the calculus already carries the nesting | **ratified-doctrine** *(the threshold choice and the Intent-0 framing; no reconstruction built, so nothing measured)* |
| Deacon (teleodynamics) *(examined 2026-07-31: earns cooperative persistence, never reaches "for what?")* | How can an end that has not happened be causal? | Anything executable — the autogen stays a proposed minimal model, not a run one | The persistence condition a minimal kytos would have to meet | [FROM_THERMODYNAMICS_TO_SEMIOSIS.md](FROM_THERMODYNAMICS_TO_SEMIOSIS.md) §3 | **queued-conjecture** |
| Millikan (teleosemantics) | What fixes what a state represents? | — (the answer is historical, and history is what Arisbe's units lack) | Content-fixing across a lineage. **Carries a sharp consequence:** on this account no representational content exists before a lineage, so reproduction stops being decoration on a population experiment and becomes constitutive of what it hopes to observe | [FROM_THERMODYNAMICS_TO_SEMIOSIS.md](FROM_THERMODYNAMICS_TO_SEMIOSIS.md) §3; bears directly on the queued D-series | **queued-conjecture** |
| Barbieri (code biology) | Does sign-hood require an arbitrary code and a codemaker? | — (a rival threshold, recorded because it disagrees) | A dissenting placement of the threshold: on this reading gradient-following does not yet signify | [FROM_THERMODYNAMICS_TO_SEMIOSIS.md](FROM_THERMODYNAMICS_TO_SEMIOSIS.md) §3 | **queued-conjecture** |
