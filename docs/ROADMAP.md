...
| v | The measure and the kytos | [THE_MEASURE_OF_KNOWLEDGE.md](THE_MEASURE_OF_KNOWLEDGE.md) + [THE_KYTOS.md](THE_KYTOS.md) as a fractal, vector-valued (never scalar-over-agents) knowledge measure. E1–E3b put executed partition-economics evidence behind it. **The West correspondence is no longer available as "the named quantitative frontier" without qualification** (Examination VIII, 2026-08-01): the scaling claim proper is graded `predicted-absent` — on a substrate where every unit reaches every other at uniform cost, the borrowed derivation gives β = 1 exactly — with a named, liftable blocker (condition 9, a reach structure whose cost of reaching grows with extent, [WEST_IN_KYTE_PROGRAM.md](WEST_IN_KYTE_PROGRAM.md) §8). A draft may cite West as the frontier's *specification* — the nine conditions are a real entry price, arrived at adversarially — but not as a correspondence the project has evidence for. The vector "return gift" is **withdrawn as stated**: scalarity does not select West's exponent, and under Ruling B a knowledge measure is an instrument, never an optimand | A cognitive-science or complexity-science venue interested in scaling laws applied to knowledge/institution measures |
