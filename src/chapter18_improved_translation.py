...
class ImprovedChapter18Translator(EnhancedChapter18Translator):
    ...
    def _reconstruct_quantifiers(self, base_formula: str, egi: RelationalGraphWithCuts) -> str:
...
class LogicalEquivalenceChecker:
    ...
    def _normalize_formula(formula: FOPLFormula) -> FOPLFormula:
        ...
        return LogicalEquivalenceChecker._apply_variable_normalization(formula, get_normalized_var)
    
    @staticmethod
    def _apply_variable_normalization(formula: FOPLFormula, var_normalizer) -> FOPLFormula:
        ...
        # Handle identity as atomic formula with special relation
            ...
            norm_left = LogicalEquivalenceChecker._apply_variable_normalization(formula.left, var_normalizer)
            norm_right = LogicalEquivalenceChecker._apply_variable_normalization(formula.right, var_normalizer)
            return ConjunctionFormula(norm_left, norm_right)
        
        elif isinstance(formula, NegationFormula):
            norm_inner = LogicalEquivalenceChecker._apply_variable_normalization(formula.formula, var_normalizer)
            ...
            norm_left = LogicalEquivalenceChecker._apply_variable_normalization(formula.antecedent, var_normalizer)
            norm_right = LogicalEquivalenceChecker._apply_variable_normalization(formula.consequent, var_normalizer)
    ...
    def _structural_equivalence(f1: FOPLFormula, f2: FOPLFormula) -> bool:
        ...
        if isinstance(f1, AtomicFormula):
            ...
            return (f1.variables == f2.variables) or \
                   (len(f1.variables) == 2 and len(f2.variables) == 2 and
                    f1.variables[0] == f2.variables[1] and f1.variables[1] == f2.variables[0])
        
        elif isinstance(f1, ConjunctionFormula):
            # Conjunction is commutative
            return (LogicalEquivalenceChecker._structural_equivalence(f1.left, f2.left) and
                   LogicalEquivalenceChecker._structural_equivalence(f1.right, f2.right)) or \
                   (LogicalEquivalenceChecker._structural_equivalence(f1.left, f2.right) and
                   LogicalEquivalenceChecker._structural_equivalence(f1.right, f2.left))
        
        elif isinstance(f1, NegationFormula):
            return LogicalEquivalenceChecker._structural_equivalence(f1.formula, f2.formula)
        
        elif isinstance(f1, ExistentialFormula):
            return (f1.variable == f2.variable and
                   LogicalEquivalenceChecker._structural_equivalence(f1.formula, f2.formula))
            ...
            return (LogicalEquivalenceChecker._structural_equivalence(f1.antecedent, f2.antecedent) and
                   LogicalEquivalenceChecker._structural_equivalence(f1.consequent, f2.consequent))
