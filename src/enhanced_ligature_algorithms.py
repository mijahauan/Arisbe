...
from theta_relation import ThetaPath, ThetaRelationEngine, ThetaRelationResult
...
class EnhancedLigatureAlgorithms:
    ...
    def analyze_ligature_network(
        self, egi: RelationalGraphWithCuts, vertices: Set[ElementID]
    ) -> LigatureNetwork:
        """
    ...
    def enhanced_move_branches_along_ligature(
        self,
        egi: RelationalGraphWithCuts,
        source_vertex: ElementID,
        target_vertex: ElementID,
        ligature_context: ElementID,
        ...
        theta_result = self.theta_engine.compute_theta_relation(
            egi, source_vertex, target_vertex
        )
        ...
        if not theta_result.are_theta_related:
            return EnhancedLigatureResult(
                ...
                theta_violations=[
                    f"No Θ relation between {source_vertex} and {target_vertex}"
                ],
        ...
        if not same_component:
            theta_violations.append(
                f"Vertices {source_vertex} and {target_vertex} in different Θ components"
            )
        ...
        for path in theta_result.paths:
            if not self._validate_path_context_constraints(egi, path, ligature_context):
                theta_violations.append(
                    f"Path {path.vertices} violates context constraints"
                )
        ...
        try:
            basic_result = self.basic_ligature_engine.apply_rule(
                ...
                egi,
                ligature_context,
                frozenset([source_vertex, target_vertex]),
            )
    ...
    def enhanced_extend_ligature(
        ...
        for new_vertex in new_vertices:
            ...
            for existing_vertex in existing_ligature:
                theta_result = self.theta_engine.compute_theta_relation(
                    egi, new_vertex, existing_vertex
                )
                if theta_result.are_theta_related:
                    ...
                    for path in theta_result.paths:
                        if not self._validate_path_context_constraints(
                            egi, path, target_context
                        ):
                            theta_violations.append(
            ...
            if not can_connect:
                theta_violations.append(
                    f"New vertex {new_vertex} cannot connect to existing ligature via Θ relation"
                )
    ...
    def _validate_path_context_constraints(
        self, egi: RelationalGraphWithCuts, path: ThetaPath, required_context: ElementID
    ) -> bool:
    ...
    def _check_transitivity_violations(
        self, ligature_network: LigatureNetwork
    ) -> List[str]:
        ...
        for i, v1 in enumerate(vertices):
            for j, v2 in enumerate(vertices[i + 1 :], i + 1):
                for k, v3 in enumerate(vertices[j + 1 :], j + 1):
                    ...
                    has_12 = (v1, v2) in ligature_network.theta_paths or (
                        v2,
                        v1,
                    ) in ligature_network.theta_paths
                    has_23 = (v2, v3) in ligature_network.theta_paths or (
                        v3,
                        v2,
                    ) in ligature_network.theta_paths
                    has_13 = (v1, v3) in ligature_network.theta_paths or (
                        v3,
                        v1,
                    ) in ligature_network.theta_paths
    ...
    def validate_ligature_consistency(
            ...
            if egi.rel.get(edge.id) == "=":
                ...
                if len(vertex_sequence) == 2:
                    identity_edges.append(
                        (edge.id, vertex_sequence[0], vertex_sequence[1])
                    )
        ...
        for edge_id, v1, v2 in identity_edges:
            ...
            if not theta_result.are_theta_related:
                violations.append(
                    f"Identity edge {edge_id} connects vertices {v1}, {v2} not related by Θ"
                )
            else:
                ...
                if not valid_context:
                    violations.append(
                        f"Identity edge {edge_id} violates context constraints for Θ path"
                    )
...
def demonstrate_enhanced_ligature_algorithms():
    ...
    test_egi = RelationalGraphWithCuts(
        ...
        rel=frozendict({ElementID("id_AB"): "=", ElementID("id_BC"): "="}),
    )
    ...
    print(
        f"   Identity edges: {[e.id for e in test_egi.E if test_egi.rel.get(e.id) == '=']}"
    )
