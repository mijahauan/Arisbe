...
class FinalChapter18Translator(EnhancedChapter18Translator):
    ...
    def psi_translate(self, formula: FOPLFormula) -> RelationalGraphWithCuts:
        ...
        return RelationalGraphWithCuts(
            V=result.V, E=result.E, nu=result.nu, sheet=result.sheet,
            Cut=result.Cut, area=result.area, rel=frozendict(metadata_rel)
        )
    ...
    def _has_existential_quantifier(self, formula: FOPLFormula) -> bool:
        ...
        if isinstance(formula, ExistentialFormula):
            ...
            return (self._has_existential_quantifier(formula.left) or 
                   self._has_existential_quantifier(formula.right))
            ...
            return (self._has_existential_quantifier(formula.antecedent) or 
                   self._has_existential_quantifier(formula.consequent))
    ...
    def _translate_area_to_fopl_final(self, egi: RelationalGraphWithCuts, 
