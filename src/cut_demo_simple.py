...
def create_test_egi(case_name: str) -> RelationalGraphWithCuts:
    ...
    if case_name == "empty":
        return RelationalGraphWithCuts(
            V=frozenset(), E=frozenset(), nu=frozendict(),
            sheet="sheet", Cut=frozenset(),
            area=frozendict({"sheet": frozenset()}), rel=frozendict()
        )
    ...
    return RelationalGraphWithCuts(
        V=frozenset(), E=frozenset(), nu=frozendict(),
        sheet="sheet", Cut=cuts, area=area_mapping, rel=frozendict()
    )
...
def print_correspondence_info(correspondence: CutAreaCorrespondence):
    ...
    print(f"Canvas: {summary['canvas_bounds']['width']}×{summary['canvas_bounds']['height']}")
    ...
    if summary['total_cuts'] > 0:
        ...
        for cut_id, cut_info in summary['cut_bounds'].items():
            ...
            print(f"    Bounds: ({cut_info['x']:.1f}, {cut_info['y']:.1f}) {cut_info['width']:.1f}×{cut_info['height']:.1f}")
        print()
