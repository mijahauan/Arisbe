...
class EndoporeuticGameEngine:
    ...
    def setup_ui(self):
        ...
        ttk.Label(toolbar, text="Endoporeutic Game", font=("Arial", 14, "bold")).pack(side=tk.LEFT)
        
        ttk.Button(toolbar, text="New Game", command=self.start_new_game).pack(side=tk.RIGHT, padx=5)
        ttk.Button(toolbar, text="Load Hypothesis", command=self.load_hypothesis).pack(side=tk.RIGHT, padx=5)
    ...
    def setup_game_board(self, parent):
        ...
        for rule_name in self.transformation_rules.keys():
            btn = ttk.Button(controls_frame, text=rule_name, 
    ...
    def setup_umpire_panel(self, parent):
        ...
        umpire_scroll = ttk.Scrollbar(analysis_frame, orient=tk.VERTICAL, command=self.umpire_text.yview)
        ...
        ttk.Button(decision_frame, text="Accept Move", 
                  command=self.umpire_accept_move).pack(side=tk.LEFT, padx=2)
        ttk.Button(decision_frame, text="Reject Move", 
                  command=self.umpire_reject_move).pack(side=tk.LEFT, padx=2)
        ttk.Button(decision_frame, text="End Inning", 
                  command=self.umpire_end_inning).pack(side=tk.LEFT, padx=2)
    ...
    def setup_hypothesis_manager(self, parent):
        ...
        self.hypothesis_tree = ttk.Treeview(list_frame, columns=("status", "score"), show="tree headings")
        ...
        hyp_scroll = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.hypothesis_tree.yview)
        ...
        ttk.Button(hyp_actions, text="Test Selected", 
                  command=self.test_hypothesis).pack(side=tk.LEFT, padx=2)
        ttk.Button(hyp_actions, text="Compare", 
                  command=self.compare_hypotheses).pack(side=tk.LEFT, padx=2)
        ttk.Button(hyp_actions, text="Archive", 
                  command=self.archive_hypothesis).pack(side=tk.LEFT, padx=2)
    
    def start_new_game(self):
        """Start a new game inning."""
        if self.current_inning and self.current_inning.outcome == GameOutcome.IN_PROGRESS:
            if not messagebox.askyesno("New Game", "End current game and start new one?"):
                return
    ...
    def attempt_move(self, rule_name: str):
        ...
        if not self.current_inning or self.current_inning.outcome != GameOutcome.IN_PROGRESS:
            messagebox.showwarning("No Game", "No active game. Start a new game first.")
            return
        
        # For now, show move attempt
        self.update_umpire_analysis(f"Attempting move: {rule_name}\n\nMove validation not yet fully implemented.")
    ...
    def umpire_reject_move(self):
        ...
        if self.current_inning:
            self.update_umpire_analysis("Move rejected by umpire. Invalid transformation.")
    ...
    def umpire_end_inning(self):
        ...
        outcome_text += f"Umpire Decision: {decision.value.replace('_', ' ').title()}\n\n"
        ...
        if outcome == GameOutcome.CONTRADICTION:
            ...
            outcome_text += "You may proceed to empirical testing or propose a new hypothesis."
    ...
    def draw_current_egi(self):
        ...
        if self.current_inning:
            ...
            self.game_canvas.create_text(300, 200, text="Current EGI State", font=("Arial", 16))
            
            # Show basic stats
            egi = self.current_inning.hypothesis
            stats_text = f"Vertices: {len(egi.V)}, Edges: {len(egi.E)}, Cuts: {len(egi.Cut)}"
    ...
    def compare_hypotheses(self):
        ...
        selection = self.hypothesis_tree.selection()
        if len(selection) < 2:
            messagebox.showwarning("Insufficient Selection", "Select at least 2 hypotheses to compare")
            return
        
        # TODO: Implement hypothesis comparison
        messagebox.showinfo("Compare Hypotheses", "Hypothesis comparison not yet implemented")
    ...
    def archive_hypothesis(self):
        ...
        messagebox.showinfo("Archive Hypothesis", "Hypothesis archiving not yet implemented")
    ...
    def update_hypothesis_display(self):
        ...
        for hypothesis in self.competing_hypotheses.values():
            self.hypothesis_tree.insert("", "end",
