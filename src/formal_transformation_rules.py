...
class DoubleCutInsertionRule(FormalTransformationRule):
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            new_area_mapping[context.target_area] = (current_area_contents - elements_to_enclose) | frozenset([outer_cut_id])
...
class DoubleCutErasureRule(FormalTransformationRule):
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            # Remove both cuts
            new_cuts = frozenset(cut for cut in egi.Cut 
                               if cut.id not in {outer_cut_id, inner_cut_id})
            ...
            for area_id, contents in new_area_mapping.items():
                if outer_cut_id in contents:
                    ...
                    new_contents = (contents - frozenset([outer_cut_id])) | inner_cut_contents
        ...
        try:
            ...
            if hasattr(context, 'insertion_edge_id'):
                ...
                for element_id in context.selected_subgraph:
                    if str(element_id).startswith("new_vertex_") or str(element_id).startswith("inserted_"):
            ...
            new_area_mapping = dict(egi.area)
            current_area_contents = new_area_mapping.get(context.target_area, frozenset())
            new_area_mapping[context.target_area] = current_area_contents | frozenset(inserted_elements)
...
class ErasureRule(FormalTransformationRule):
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            # Update nu mapping
            new_nu = frozendict({k: v for k, v in egi.nu.items() if k not in expanded_subgraph})
            
            # Update rel mapping
            new_rel = frozendict({k: v for k, v in egi.rel.items() if k not in expanded_subgraph})
...
class IterationRule(FormalTransformationRule):
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            new_area_mapping = dict(egi.area)
            current_area_contents = new_area_mapping.get(context.target_area, frozenset())
            new_area_mapping[context.target_area] = current_area_contents | frozenset(copied_elements)
            ...
            return TransformationResult(
                ...
                changes_made={
                    ...
                    "target_area": str(context.target_area),
                    "element_mapping": {str(k): str(v) for k, v in element_mapping.items()}
                }
            )
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            if result.success:
                ...
                result.changes_made["deiterated_elements"] = result.changes_made.pop("erased_elements", [])
...
def demonstrate_transformation_rules():
    ...
    print(f"Starting EGI: {len(test_egi.V)} vertices, {len(test_egi.E)} edges, {len(test_egi.Cut)} cuts")
    ...
    for i, test_case in enumerate(test_cases):
        ...
        if test_case["rule"] == "INS" and i > 0:
            ...
            cut_areas = [area_id for area_id in current_egi.area.keys() 
        ...
        if result.success:
            ...
            print(f"   Result: {len(current_egi.V)}V, {len(current_egi.E)}E, {len(current_egi.Cut)}C")
        else:
            print(f"   ❌ FAILED: {result.error_message}")
        
        print()
    
    print(f"📊 Final EGI: {len(current_egi.V)} vertices, {len(current_egi.E)} edges, {len(current_egi.Cut)} cuts")
