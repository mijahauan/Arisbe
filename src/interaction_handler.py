...
from diagram_coordinator import DiagramCoordinator, Point2D, InteractionMode
...
class InteractionHandler:
    ...
    def handle_mouse_press(self, event: QMouseEvent, scene_pos: QPointF, item: Optional[QGraphicsItem] = None) -> bool:
    ...
    def _handle_drag_end(self, pos: Point2D) -> bool:
        ...
        if element_id:
            ...
            if self.on_status_update:
                status = f"Moved {element_id}" if success else f"Failed to move {element_id}"
                self.on_status_update(status)
