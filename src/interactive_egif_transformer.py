...
class InteractiveEGIFTransformer:
        ...
        try:
            ...
            for i, vertex in enumerate(egi.V):
                elements[f"vertex_{i}"] = {
                    ...
                    "is_constant": hasattr(egi, "rho")
                    and vertex.id in egi.rho
                    and egi.rho[vertex.id] is not None,
            ...
            negative_areas = [
                area for area in areas.values() if area["polarity"] == "negative"
            ]
            if negative_areas:
                suggestions.append(
                    {
            ...
            positive_areas = [
                ...
                if area["polarity"] == "positive" and area["element_count"] > 0
            ]
            if positive_areas:
                suggestions.append(
                    {
            ...
            empty_cuts = [
                area
                for area in areas.values()
                if area["type"] == "cut" and area["element_count"] == 0
            ]
            if len(empty_cuts) >= 2:
                suggestions.append(
                    {
    ...
    def display_analysis(self, analysis: GraphAnalysis) -> None:
        ...
        print(
            f"Structure: {analysis.vertex_count}V, {analysis.edge_count}E, {analysis.cut_count}C"
        )
        ...
        for area_name, area_info in analysis.areas.items():
            ...
            print(
                f"   {polarity_icon} {area_name}: {area_info['description']} ({area_info['element_count']} elements)"
            )
            ...
            for elem_name, elem_info in analysis.elements.items():
                if elem_info["type"] == "vertex":
                    if elem_info["is_constant"]:
                        print(
                            f"   🔸 {elem_name}: Constant \"{elem_info['constant_name']}\""
                        )
                    else:
                        print(f"   🔹 {elem_name}: Generic variable")
                elif elem_info["type"] == "edge":
                    print(
                        f"   🔗 {elem_name}: {elem_info['relation']} (arity {elem_info['arity']})"
                    )
    ...
    def display_yaml_structure(self, analysis: GraphAnalysis) -> None:
        ...
        for elem_name, elem_info in analysis.elements.items():
            ...
            if elem_info["type"] == "vertex" and elem_info["is_constant"]:
                structure["elements"][elem_name]["constant"] = elem_info[
                    "constant_name"
                ]
            elif elem_info["type"] == "edge":
    ...
    def get_user_choice(
        self, prompt: str, options: Dict[int, Any], allow_custom: bool = False
    ) -> Tuple[int, Any]:
        ...
        while True:
            try:
                ...
                if choice_num in options:
                    ...
                    print(
                        f"❌ Invalid choice. Please enter a number between {min(options.keys())} and {max(options.keys())}"
                    )
    ...
    def select_target_area(self, analysis: GraphAnalysis, rule: str) -> str:
        ...
        if rule == "INS":
            valid_areas = {
                name: info
                for name, info in analysis.areas.items()
    ...
    def get_subgraph_specification(
        self, rule: str, analysis: GraphAnalysis, target_area: str = None
    ) -> Dict[str, Any]:
        ...
        if rule == "INS":
            ...
            for i, (elem_name, elem_info) in enumerate(available_elements.items(), 1):
                if elem_info["type"] == "vertex":
                    ...
                    content_desc = (
                        f"({elem_info['content_count']} elements)"
                        if elem_info["content_count"] > 0
                        else "(empty)"
                    )
            ...
            valid_destinations = self.get_valid_iteration_destinations(
                analysis, target_area
            )
            ...
            for i, (area_name, area_info) in enumerate(valid_destinations.items(), 1):
                ...
                print(
                    f"   {i}. {area_name}: {area_info['description']} {polarity_symbol}"
                )
            ...
            print(
                "   (Select elements to enclose, or leave empty for empty double cut)"
            )
            ...
            if not available_elements:
                print(
                    f"   No elements found in the selected area - will create empty double cut"
                )
                return {"selected_elements": []}
            ...
            print(
                "   (Enter comma-separated numbers for multiple elements, or press Enter for empty double cut)"
            )
            ...
            if not potential_double_cuts:
                return {
                    "selected_elements": [],
                    "error": "No double cut patterns found in selected area",
                }
    ...
    def get_valid_iteration_destinations(
        self, analysis: GraphAnalysis, source_area: str
    ) -> Dict[str, Any]:
    ...
    def is_area_nested_within(
        self,
        egi: RelationalGraphWithCuts,
        child_area_id: ElementID,
    ...
    def run_interactive_session(self) -> None:
        ...
        try:
            ...
            operation_details = self.get_subgraph_specification(
                rule, self.analysis, target_area
            )
            ...
            response = self.interface.apply_transformation(
                request, existing_egi=self.current_egi
            )
            ...
            if response.success:
                ...
                continue_choice = (
                    input("\nContinue transforming this result? (y/n): ")
                    .strip()
                    .lower()
                )
