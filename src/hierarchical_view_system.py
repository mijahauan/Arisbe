...
class ViewContext:
    ...
    def should_show_details(self, current_zoom: float) -> bool:
        ...
        return (current_zoom >= self.zoom_threshold and 
                self.view_level in [ViewLevel.DETAILED, ViewLevel.EXPANDED])
...
class RTreeNode:
    ...
    def insert_context(self, context_id: str, bounds: SpatialBounds, 
                      view_system: 'HierarchicalViewSystem') -> None:
    ...
    def _calculate_enlargement(self, current: SpatialBounds, 
                             new: SpatialBounds) -> float:
...
class HierarchicalViewSystem:
    ...
    def build_hierarchical_view(self, egi: RelationalGraphWithCuts, 
    ...
    def get_visible_contexts(self, query_bounds: Optional[SpatialBounds] = None) -> List[ViewContext]:
        ...
        if len(visible_contexts) > self.max_visible_contexts and self.egi:
            ...
            visible_contexts.sort(key=lambda c: c.calculate_complexity(self.egi), reverse=True)
    ...
    def _build_context_hierarchy(self, egi: RelationalGraphWithCuts, max_depth: int) -> None:
        """Build hierarchy of contexts based on EGI structure and complexity."""
        # Start with root areas
        root_cuts = [c.id for c in egi.Cut if self._find_parent_area(c.id, egi) == "sheet"]
    ...
    def _group_cuts_into_contexts(self, cuts: List[str], egi: RelationalGraphWithCuts, 
    ...
    def _build_nested_contexts(self, parent_context_id: str, parent_cuts: List[str], 
                              egi: RelationalGraphWithCuts, remaining_depth: int) -> None:
        ...
        if nested_cuts:
            ...
            nested_groups = self._group_cuts_into_contexts(nested_cuts, egi, remaining_depth)
            ...
            for i, nested_group in enumerate(nested_groups):
                ...
                nested_context.summary = self._calculate_context_summary(nested_group, egi)
                ...
                # Continue recursion
                self._build_nested_contexts(nested_context_id, nested_group, egi, remaining_depth - 1)
    
    def _calculate_context_summary(self, cuts: List[str], egi: RelationalGraphWithCuts) -> ContextSummary:
        ...
        return ContextSummary(cut_count, max_depth, predicate_count, vertex_count, complexity_score)
    ...
    def _layout_contexts(self, egi: RelationalGraphWithCuts) -> None:
        ...
        root_contexts = [c for c in self.contexts.values() if c.parent_id == "sheet_context"]
    ...
    def _layout_sibling_contexts(self, contexts: List[ViewContext], parent_bounds: SpatialBounds) -> None:
    ...
    def _layout_nested_contexts(self, parent_context_id: str, egi: RelationalGraphWithCuts) -> None:
    ...
    def _build_spatial_index(self) -> None:
        ...
        for context_id, context in self.contexts.items():
            if context.spatial_bounds:
                self.spatial_index.insert_context(context_id, context.spatial_bounds, self)
