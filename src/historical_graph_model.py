...
from simplified_sheet_of_assertion import SheetMetadata, SheetOfAssertion, SheetPurpose
from transformation_engine import OperationRequest, OperationResult
from universal_composition import (
    CompositionSequence,
    CompositionSource,
    CompositionStep,
)
...
class GraphHistory:
    ...
    def create_transformation_event(
        self, operation: OperationRequest, result: OperationResult, author: str = ""
    ) -> str:
    ...
    def create_branch(
        self, branch_name: str, from_event_id: Optional[str] = None
    ) -> bool:
        ...
        for event in events:
            if event.operation and event.event_type == HistoryEventType.TRANSFORMATION:
                step = CompositionStep(
                    operation=event.operation, step_description=event.description
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "creation_time": (
                min(event.timestamp for event in self.events.values())
                if self.events
                else None
            ),
        }
...
class HistoricalGraph:
    ...
    def merge_from_graph(self, other_graph: "HistoricalGraph", author: str = "") -> str:
        ...
        merge_event = HistoryEvent(
            ...
            merged_from=(
                [other_graph.history.current_event_id]
                if other_graph.history.current_event_id
                else []
            ),
        )
    ...
    def export_with_history(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "snapshots": {
                eid: self._serialize_egi(egi) for eid, egi in self.snapshots.items()
            },
        }
...
class HistoricalGraphRepository:
    ...
    def find_graphs_by_provenance(
        self, source_type: CompositionSource
    ) -> List[HistoricalGraph]:
