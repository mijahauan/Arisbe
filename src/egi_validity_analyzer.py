...
class EGIValidityAnalyzer:
    ...
    def analyze_egi(self, egi: RelationalGraphWithCuts, source_egif: Optional[str] = None,
                   egi_id: Optional[str] = None) -> ValidationReport:
            ...
            for edge in egi.E:
                ...
                if vertex_id in edge_vertices:
                    for other_vertex_id in edge_vertices:
                        if other_vertex_id != vertex_id and other_vertex_id not in visited:
                            dfs(other_vertex_id)
    ...
    def _check_structural_integrity(self, egi: RelationalGraphWithCuts, report: ValidationReport):
        ...
        if not hasattr(egi, 'sheet') or egi.sheet is None:
            self._add_issue(report, ValidationSeverity.CRITICAL, "structure",
        ...
        for duplicate_id in duplicates:
            self._add_issue(report, ValidationSeverity.CRITICAL, "structure",
                          f"Duplicate element ID: {duplicate_id}", [duplicate_id])
    
    def _check_referential_consistency(self, egi: RelationalGraphWithCuts, report: ValidationReport):
        ...
        for edge in egi.E:
            ...
            for vertex_id in edge_vertices:
                if vertex_id not in valid_ids:
                    self._add_issue(report, ValidationSeverity.ERROR, "references",
        ...
        for area_id, contents in egi.area.items():
            if area_id not in valid_ids:
                self._add_issue(report, ValidationSeverity.ERROR, "references",
                              f"Area {area_id} is not a valid element ID", [area_id])
    ...
    def _check_area_containment(self, egi: RelationalGraphWithCuts, report: ValidationReport):
        ...
        for element_id, count in element_area_count.items():
            if count > 1:
                self._add_issue(report, ValidationSeverity.ERROR, "containment",
        ...
        for element_id in orphaned:
            self._add_issue(report, ValidationSeverity.ERROR, "containment",
                          f"Element {element_id} is not contained in any area", [element_id])
        
        # Check that sheet of assertion has an area entry
        if egi.sheet not in egi.area:
            self._add_issue(report, ValidationSeverity.CRITICAL, "containment",
                          "Sheet of assertion has no area entry", [egi.sheet])
    
    def _check_cut_nesting(self, egi: RelationalGraphWithCuts, report: ValidationReport):
        ...
        def has_cycle(node: ElementID, visited: Set[ElementID], rec_stack: Set[ElementID]) -> bool:
            visited.add(node)
            rec_stack.add(node)
        ...
        for area_id in egi.area.keys():
            if area_id not in visited:
                if has_cycle(area_id, visited, set()):
                    self._add_issue(report, ValidationSeverity.CRITICAL, "nesting",
                                  f"Containment cycle detected involving area {area_id}", [area_id])
    
    def _check_edge_endpoints(self, egi: RelationalGraphWithCuts, report: ValidationReport):
        ...
        for edge in egi.E:
            ...
            if len(edge_vertices) < 1:
                self._add_issue(report, ValidationSeverity.ERROR, "edges",
                              f"Edge {edge.id} has no vertices", [edge.id])
            
            # Check for self-loops in binary relations
            if len(edge_vertices) == 2 and edge_vertices[0] == edge_vertices[1]:
                self._add_issue(report, ValidationSeverity.WARNING, "edges",
                              f"Edge {edge.id} is a self-loop", [edge.id])
    ...
    def _check_logical_consistency(self, egi: RelationalGraphWithCuts, report: ValidationReport):
        ...
        for cut in egi.Cut:
            ...
            if not cut_contents:
                self._add_issue(report, ValidationSeverity.WARNING, "logic",
                              f"Cut {cut.id} is empty (may be removable by DC-)", [cut.id])
            ...
            if len(cut_contents) == 1:
                ...
                if any(inner_cut.id == inner_element for inner_cut in egi.Cut):
                    ...
                    if not inner_cut_contents:
                        self._add_issue(report, ValidationSeverity.INFO, "logic",
    ...
    def _check_egif_round_trip(self, egi: RelationalGraphWithCuts, source_egif: str, 
                              report: ValidationReport):
        ...
        try:
            ...
            if len(source_egi.V) != len(generated_egi.V):
                self._add_issue(report, ValidationSeverity.WARNING, "round_trip",
            ...
            self._add_issue(report, ValidationSeverity.ERROR, "round_trip",
                          f"EGIF round-trip failed: {str(e)}", [])
    
    def _add_issue(self, report: ValidationReport, severity: ValidationSeverity,
                  category: str, description: str, element_ids: List[ElementID],
    ...
    def generate_report_summary(self, report: ValidationReport) -> str:
        ...
        if report.issues:
            ...
            for issue in report.issues:
                lines.append(f"[{issue.severity.value.upper()}] {issue.category}: {issue.description}")
                if issue.element_ids:
                    lines.append(f"  Elements: {', '.join(map(str, issue.element_ids))}")
