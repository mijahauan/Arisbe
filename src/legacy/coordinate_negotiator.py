...
from spatial_region_manager import SpatialRegionManager, SpatialRegion
...
class RenderingBackend(Protocol):
    ...
    def get_scene_position(self, screen_x: float, screen_y: float) -> Tuple[float, float]:
...
class CoordinateNegotiator:
    ...
    def __init__(self, region_manager: SpatialRegionManager, rendering_backend: Optional[RenderingBackend] = None):
        ...
        # Element tracking
        self.element_positions: Dict[str, Tuple[float, float]] = {}  # Data model coordinates
    ...
    def register_element(self, element_id: str, data_x: float, data_y: float, 
        ...
        self.element_bounds[element_id] = SpatialBounds(logical_x, logical_y, width, height)
    
    def update_element_position(self, element_id: str, data_x: float, data_y: float) -> None:
        ...
        if element_id in self.element_bounds:
            ...
            self.element_bounds[element_id] = SpatialBounds(logical_x, logical_y, bounds.width, bounds.height)
    ...
    def get_logical_area_for_rendering_position(self, render_x: float, render_y: float) -> str:
    ...
    def get_rendering_position_for_data(self, data_x: float, data_y: float) -> Tuple[float, float]:
        """Convert data model coordinates to rendering coordinates."""
        logical_x, logical_y = self.data_to_logical.apply(data_x, data_y)
        return self.logical_to_rendering.apply(logical_x, logical_y)
    
    def get_data_position_for_rendering(self, render_x: float, render_y: float) -> Tuple[float, float]:
    ...
    def synchronize_with_rendering_backend(self) -> None:
        ...
        for element_id in self.element_positions:
            ...
            if actual_bounds:
                ...
                expected_render_x, expected_render_y = self.get_rendering_position_for_data(data_x, data_y)
                ...
                if abs(dx) > 1.0 or abs(dy) > 1.0:
                    print(f"COORDINATE SYNC: Adjusting transform for {element_id}: offset ({dx}, {dy})")
                    self.logical_to_rendering.offset_x += dx
                    self.logical_to_rendering.offset_y += dy
    
    def create_region_for_cut(self, cut_id: str, data_x: float, data_y: float, 
                             width: float, height: float, parent_area: str = "sheet") -> str:
        ...
        print(f"COORDINATE NEGOTIATOR: Created region {logical_area_id} at logical ({logical_x}, {logical_y}, {logical_width}, {logical_height})")
        
        return logical_area_id
    
    def get_valid_placement_area(self, render_x: float, render_y: float) -> Optional[str]:
    ...
    def get_region_bounds_in_rendering_coordinates(self, logical_area_id: str) -> Optional[SpatialBounds]:
    ...
    def debug_coordinate_systems(self) -> Dict[str, Any]:
        ...
        info = {
            "transforms": {
                "data_to_logical": {
                    "scale": (self.data_to_logical.scale_x, self.data_to_logical.scale_y),
                    "offset": (self.data_to_logical.offset_x, self.data_to_logical.offset_y)
                },
                "logical_to_rendering": {
                    "scale": (self.logical_to_rendering.scale_x, self.logical_to_rendering.scale_y),
                    "offset": (self.logical_to_rendering.offset_x, self.logical_to_rendering.offset_y)
                }
            },
            "elements": {},
            "regions": {}
        ...
        for region_id, region in self.region_manager.regions.items():
            render_bounds = self.get_region_bounds_in_rendering_coordinates(region.logical_area_id)
    ...
    def negotiate_coordinate_mapping(self, gui_viewport_bounds: Tuple[float, float, float, float], 
                                   logical_workspace_bounds: Tuple[float, float, float, float]) -> None:
        """
        ...
        logical_min_x, logical_min_y, logical_width, logical_height = logical_workspace_bounds
        ...
        offset_x = gui_min_x + (gui_width - logical_width * scale) / 2 - logical_min_x * scale
        offset_y = gui_min_y + (gui_height - logical_height * scale) / 2 - logical_min_y * scale
        ...
        print(f"COORDINATE NEGOTIATION: GUI viewport {gui_viewport_bounds} → Logical workspace {logical_workspace_bounds}")
        print(f"COORDINATE NEGOTIATION: Scale={scale}, Offset=({offset_x}, {offset_y})")
    
    def auto_negotiate_from_gui_bounds(self, gui_min_x: float, gui_min_y: float, 
                                     gui_max_x: float, gui_max_y: float) -> None:
        """
        ...
        logical_size = 1000
        logical_workspace_bounds = (-logical_size/2, -logical_size/2, logical_size, logical_size)
...
class QtRenderingBackend:
    ...
    def get_item_bounds(self, item_id: str) -> Optional[SpatialBounds]:
        ...
        for item in self.scene.items():
            if (hasattr(item, 'vertex_id') and item.vertex_id == item_id) or \
               (hasattr(item, 'predicate_id') and item.predicate_id == item_id) or \
               (hasattr(item, 'cut_id') and item.cut_id == item_id):
