...
from rtree_cut_tracker import CutSpatialInfo, RTreeCutTracker, SpatialBounds
...
from historical_graph_model import CompositionSource, HistoricalGraph
...
class SelectionParameters:
    ...
    custom_predicate: Optional[Callable[[ElementID, RelationalGraphWithCuts], bool]] = (
        None
    )
...
class SubgraphExtractor:
    ...
    def extract_subgraph(
        ...
        spatial_tracker: Optional[RTreeCutTracker] = None,
        ...
        try:
            ...
            if not selected_elements:
                return SubgraphExtractionResult(
                    success=False, error_message="No elements selected by criterion"
                )
            ...
            return SubgraphExtractionResult(
                success=False, error_message=f"Extraction failed: {str(e)}"
            )

    def _select_connected_component(
        self, egi: RelationalGraphWithCuts, params: SelectionParameters
    ) -> Set[ElementID]:
    ...
    def _create_subgraph_structure(
        self, source_egi: RelationalGraphWithCuts, selected_elements: Set[ElementID]
            ...
            if cut_id in source_egi._cut_map:
                ...
                if cut_id in source_egi.area:
                    ...
                    included_area = frozenset(
                        elem for elem in area_elements if elem in selected_elements
                    )
                    if included_area:
    ...
    def _extract_spatial_info(
        self, selected_elements: Set[ElementID], spatial_tracker: RTreeCutTracker
    ) -> Dict[ElementID, SpatialBounds]:
    ...
    def create_historical_subgraph(
        self, extraction_result: SubgraphExtractionResult, metadata: SheetMetadata
    ) -> Optional[HistoricalGraph]:
