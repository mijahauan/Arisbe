...
from egi_transformation_history import (
    EGITransformationHistory, StateSnapshot, TransformationStep, 
    LogicalProvenance, HistoryBranchType
)
from domain_ontology_model import DomainModelManager, SemanticAnnotation, ConceptMapping
...
class CollaborationMetadata:
    ...
    access_permissions: Dict[str, str] = field(default_factory=dict)  # user_id -> permission_level
...
class EnhancedEGITransformationHistory(EGITransformationHistory):
    ...
    def add_transformation_with_domain_context(self,
                                             rule_name: str,
    ...
    def export_proof_sequence(self, 
        ...
        proof_text = self._generate_proof_export(sequence, export_format, include_domain_context)
    ...
    def add_semantic_annotation_to_state(self,
                                       state_id: str,
    ...
    def validate_transformation_sequence(self, 
                                       from_state_id: str, 
                                       to_state_id: str) -> Dict[str, Any]:
        ...
        for step in sequence.steps:
            ...
            for context_id in step.affected_domain_contexts:
                issues = self.domain_model_manager.validate_domain_consistency(context_id)
                validation_results["domain_consistency_issues"].extend(issues)
        
        return validation_results
    
    def get_natural_language_narrative(self, 
                                     from_state_id: str, 
                                     to_state_id: str) -> str:
        ...
        if from_state.natural_language_summary:
            narrative_parts.append(f"Starting with: {from_state.natural_language_summary}")
        ...
        for i, step in enumerate(sequence.steps, 1):
            ...
            if step.logical_provenance:
                narrative_parts.append(f"   Justification: {step.logical_provenance.semantic_interpretation}")
    ...
    def _generate_state_natural_language(self, 
                                       egi: RelationalGraphWithCuts, 
                                       domain_contexts: Set[str]) -> str:
    ...
    def _generate_transformation_natural_language(self,
                                                rule_name: str,
                                                context,
                                                domain_contexts: Set[str]) -> str:
        ...
        if domain_contexts:
            ...
            for context_id in domain_contexts:
                if context_id in self.domain_model_manager.domain_contexts:
                    context_names.append(self.domain_model_manager.domain_contexts[context_id].name)
    ...
    def _generate_proof_export(self, 
                             sequence,  # TransformationSequence
                             export_format: ProofExportFormat,
                             include_domain_context: bool) -> str:
    ...
    def _export_natural_deduction(self, sequence, include_domain_context: bool) -> str:
        ...
        for i, step in enumerate(sequence.steps, 1):
            ...
            if step.logical_provenance:
                ...
                lines.append(f"   Justification: {step.logical_provenance.semantic_interpretation}")
            lines.append("")
    ...
    def _determine_status(self, result) -> str:
        ...
        return TransformationStatus.APPLIED if result.success else TransformationStatus.FAILED
    ...
    def get_collaboration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "lock_status": "locked" if self.collaboration_metadata.lock_holder else "unlocked",
