...
from subgraph_extractor import SubgraphExtractionResult, SelectionParameters
from transformation_rules import TransformationRule, TransformationRuleType, TransformationContext, ValidationResult
...
class TransformationDiscoveryResult:
    ...
    available_transformations: List[AvailableTransformation] = field(default_factory=list)
...
class TransformationDiscoveryEngine:
    ...
    def __init__(self, transformation_rules: Dict[TransformationRuleType, TransformationRule]):
        ...
        self.rule_operations = {
            ...
            TransformationRuleType.DEITERATION: [
                OperationType.DEITERATE_ELEMENT
            ],
    ...
    def discover_transformations(self, egi: RelationalGraphWithCuts,
                               extraction_result: SubgraphExtractionResult) -> TransformationDiscoveryResult:
        """
        ...
        try:
            ...
            for rule_type, rule in self.transformation_rules.items():
                ...
                for transformation in transformations:
                    if transformation.availability == TransformationAvailability.AVAILABLE:
                        available_transformations.append(transformation)
                    elif transformation.availability == TransformationAvailability.CONTEXTUALLY_INVALID:
    ...
    def _analyze_contexts(self, egi: RelationalGraphWithCuts, 
                         selected_elements: Set[ElementID]) -> Dict[str, Any]:
    ...
    def _discover_rule_transformations(self, egi: RelationalGraphWithCuts,
                                     selected_elements: Set[ElementID],
                                     rule_type: TransformationRuleType,
                                     rule: TransformationRule,
                                     context_analysis: Dict[str, Any]) -> List[AvailableTransformation]:
        ...
        for operation_type in operations:
            transformation = self._analyze_operation_availability(
                egi, selected_elements, rule_type, operation_type, 
    ...
    def _analyze_operation_availability(self, egi: RelationalGraphWithCuts,
                                      selected_elements: Set[ElementID],
                                      rule_type: TransformationRuleType,
                                      operation_type: OperationType,
                                      rule: TransformationRule,
                                      context_analysis: Dict[str, Any]) -> AvailableTransformation:
        ...
        return AvailableTransformation(
            ...
            missing_requirements=validation_result.suggestions if validation_result else []
        )
    
    def _create_example_operation(self, operation_type: OperationType,
                                selected_elements: Set[ElementID]) -> OperationRequest:
    ...
    def _determine_availability(self, validation_result: Optional[ValidationResult],
                              rule_type: TransformationRuleType,
                              context_analysis: Dict[str, Any]) -> TransformationAvailability:
    ...
    def _generate_descriptions(self, operation_type: OperationType,
                             rule_type: TransformationRuleType,
                             availability: TransformationAvailability,
                             selected_elements: Set[ElementID]) -> Tuple[str, str]:
        """Generate user-friendly descriptions."""
        
        operation_descriptions = {
            OperationType.ADD_VERTEX: ("Add Vertex", "Insert a new vertex in negative context"),
            OperationType.ADD_EDGE: ("Add Edge", "Insert a new edge connecting vertices"),
            OperationType.ADD_CUT: ("Add Cut", "Insert a new cut around elements"),
            OperationType.REMOVE_VERTEX: ("Remove Vertex", "Erase vertex from positive context"),
            OperationType.REMOVE_EDGE: ("Remove Edge", "Erase edge from positive context"),
            OperationType.REMOVE_CUT: ("Remove Cut", "Erase cut from positive context"),
            OperationType.ITERATE_SUBGRAPH: ("Iterate", "Copy subgraph to deeper context"),
            OperationType.DEITERATE_ELEMENT: ("De-iterate", "Remove iterated element"),
            OperationType.ADD_DOUBLE_CUT: ("Add Double Cut", "Add double cut around selection"),
...
class SubgraphTransformationWorkflow:
    ...
    def analyze_selection(self, egi: RelationalGraphWithCuts,
                         extraction_result: SubgraphExtractionResult) -> TransformationDiscoveryResult:
    ...
    def get_transformation_menu(self, discovery_result: TransformationDiscoveryResult) -> Dict[str, Any]:
    ...
    def create_operation_request(self, discovery_result: TransformationDiscoveryResult,
                               transformation_id: str,
                               additional_parameters: Dict[str, Any] = None) -> Optional[OperationRequest]:
