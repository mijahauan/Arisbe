...
class ImprovedResizeHandle(QGraphicsRectItem):
    ...
    def _validate_resize(self, new_rect):
        ...
        for item in self.scene().items():
            if (
                hasattr(item, "cut_id")
                and item != self.parent_cut
                and hasattr(item, "sceneBoundingRect")
            ):
                ...
                if new_rect.intersects(other_rect):
                    ...
                    new_rect_expanded = new_rect.adjusted(
                        -margin, -margin, margin, margin
                    )
                    other_rect_expanded = other_rect.adjusted(
                        -margin, -margin, margin, margin
                    )

                    if not (
                        new_rect_expanded.contains(other_rect)
                        or other_rect_expanded.contains(new_rect)
                    ):
...
class InteractivePredicate(QGraphicsTextItem):
    ...
    def itemChange(self, change, value):
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionChange:
            ...
            self.renderer._update_element_area_containment(
                self.predicate_id, self.pos()
            )
        return super().itemChange(change, value)
    ...
    def _validate_position(self, new_pos):
            ...
            if isinstance(item, InteractivePredicate) and item != self:
                ...
                if new_rect.intersects(other_rect):
                    self._show_violation_message(
                        f"SYNTACTIC VIOLATION: Predicate {self.predicate_id} not permitted to overlap with Predicate {item.predicate_id}",
                        new_pos,
                    )
                    return False
        ...
        for item in self.scene().items():
            if isinstance(item, StyledCutItem):
                cut_pos = item.pos()
                cut_rect = QRectF(
                    cut_pos.x(), cut_pos.y(), item.rect().width(), item.rect().height()
                )
        ...
        if containing_cut:
            ...
            cut_rect = QRectF(
                cut_pos.x(),
                cut_pos.y(),
                containing_cut.rect().width(),
                containing_cut.rect().height(),
            )
            if not cut_rect.contains(new_rect):
                self._show_violation_message(
                    f"SEMANTIC VIOLATION: Predicate {self.predicate_id} not permitted to move outside containing Cut {containing_cut.cut_id}",
                    new_pos,
                )
                return False
...
class StyledCutItem(QGraphicsRectItem):
    ...
    def __init__(
        ...
        validation_mode: str = "composition",
        nesting_depth: int = 0,
    ):
        super().__init__(rect)
        self.cut_id = cut_id
        ...
        print(
            f"DEBUG: Created enhanced StyledCutItem {cut_id} with interactive={interactive}, validation_mode={validation_mode}, depth={nesting_depth}"
        )
    ...
    def hoverEnterEvent(self, event):
        ...
        print(
            f"DEBUG: Cut {self.cut_id} hoverEnterEvent - interactive={self.interactive}"
        )
        if self.interactive:
    ...
    def _validate_position(self, new_pos):
        ...
        # Get proposed new rectangle
        new_rect = QRectF(
            new_pos.x(), new_pos.y(), self.rect().width(), self.rect().height()
        )
        ...
        for item in self.scene().items():
            if isinstance(item, StyledCutItem) and item != self:
                ...
                if new_rect.intersects(other_rect):
                    ...
                    if new_rect_expanded.contains(
                        other_rect
                    ) or other_rect_expanded.contains(new_rect):
                        print(
                            f"DEBUG: Allowing cut containment: {self.cut_id} and {item.cut_id}"
                        )
                    ...
                    if self._attempt_collision_avoidance(new_pos):
                        ...
                        self._show_violation_message(
                            f"SYNTACTIC VIOLATION: Cut {self.cut_id} not permitted to partially overlap with Cut {item.cut_id}",
                            new_pos,
                        )
                        return False
        ...
        if self.validation_mode == "practice":
            ...
            for pred in contained_predicates:
                ...
                pred_rect = QRectF(
                    pred_pos.x(),
                    pred_pos.y(),
                    pred.boundingRect().width(),
                    pred.boundingRect().height(),
                )
                if not new_rect.contains(pred_rect):
                    self._show_violation_message(
                        f"SEMANTIC VIOLATION: Cut {self.cut_id} not permitted to move away from enclosed Predicate {pred.predicate_id}",
                        new_pos,
                    )
                    return False
            ...
            if isinstance(item, (InteractivePredicate, InteractiveVertex)):
                ...
                if cut_rect.contains(item_rect.center()):
                    ...
                    if hasattr(item, "renderer"):
                        if isinstance(item, InteractivePredicate):
                            item.renderer.update_ligatures_for_predicate(
                                item.predicate_id
                            )
    ...
    def _is_valid_avoidance_position(self, item, new_pos):
        ...
        new_rect = QRectF(
            new_pos.x(), new_pos.y(), item.rect().width(), item.rect().height()
        )

        # Check against all other cuts
        for other_item in item.scene().items():
            if (
                isinstance(other_item, StyledCutItem)
                and other_item != item
                and other_item != self
            ):
                ...
                other_rect = QRectF(
                    other_pos.x(),
                    other_pos.y(),
                    other_item.rect().width(),
                    other_item.rect().height(),
                )
                if new_rect.intersects(other_rect):
                    return False
        ...
        for item in self.scene().items():
            if isinstance(item, StyledCutItem) and item != self:
                ...
                if scene_rect.intersects(other_rect):
                    ...
                    self._show_violation_message(
                        f"RESIZE WARNING: Cut {self.cut_id} overlapping with Cut {item.cut_id}",
                        current_pos,
                    )
    ...
    def mousePressEvent(self, event):
        ...
        print(
            f"DEBUG: Cut {self.cut_id} mousePressEvent - interactive={self.interactive}, button={event.button()}"
        )
...
class SharedDiagramRenderer:
    ...
    def render_egdf(self, doc: EGDFDocument, interactive: bool = True) -> None:
        ...
        egi_inline = (
            doc.egi_ref.get("inline", {}) if isinstance(doc.egi_ref, dict) else {}
        )
        ...
        for vertex_id, vertex_data in vertices.items():
            print(
                f"Drawing vertex {vertex_id} at ({vertex_data.get('x')}, {vertex_data.get('y')})"
            )
        ...
        for pred_id, pred_data in predicates.items():
            print(
                f"Drawing predicate {pred_id}: '{pred_data.get('text')}' at ({pred_data.get('x')}, {pred_data.get('y')})"
            )
    ...
    def update_ligatures_for_vertex(self, vertex_id: str) -> None:
        ...
        for edge_id, lines in self.ligature_map.items():
            for line in lines:
                ...
                p2_match = (
                    abs(old_line.p2().x() - old_vertex_pos.x()) < 2
                    and abs(old_line.p2().y() - old_vertex_pos.y()) < 2
                )
                p1_match = (
                    abs(old_line.p1().x() - old_vertex_pos.x()) < 2
                    and abs(old_line.p1().y() - old_vertex_pos.y()) < 2
                )
    ...
    def update_ligatures_for_predicate(self, predicate_id: str) -> None:
        ...
        if predicate_id in self.ligature_map:
            ...
            for item in self.scene.items():
                if (
                    isinstance(item, InteractivePredicate)
    ...
    def render_cut(self, cut_dto, interactive: bool = True) -> StyledCutItem:
        ...
        cut_item = StyledCutItem(
            rect,
            cut_id,
            self.style_manager,
            interactive,
            validation_mode="composition",
            nesting_depth=depth,
        )
        ...
        if interactive:
            ...
            print(
                f"DEBUG: Cut {cut_id} configured as interactive with flags: movable={cut_item.flags() & QGraphicsItem.GraphicsItemFlag.ItemIsMovable}, selectable={cut_item.flags() & QGraphicsItem.GraphicsItemFlag.ItemIsSelectable}"
            )
        ...
        for outer_id, outer_item in self.cut_items.items():
            ...
            for inner_id, inner_item in self.cut_items.items():
                ...
                if outer_rect.contains(inner_rect):
                    ...
                    if self._is_valid_double_cut(
                        outer_rect, inner_rect, outer_id, inner_id
                    ):
                        double_cuts.append((outer_id, inner_id))

        return double_cuts

    def _is_valid_double_cut(
        self, outer_rect: QRectF, inner_rect: QRectF, outer_id: str, inner_id: str
    ) -> bool:
        ...
        for item in self.scene.items():
            ...
            if hasattr(item, "cut_id") and (
                getattr(item, "cut_id", None) == outer_id
                or getattr(item, "cut_id", None) == inner_id
            ):
                continue
            ...
            if outer_rect.intersects(item_rect) and not inner_rect.contains(item_rect):
                ...
                if isinstance(item, QGraphicsLineItem):
                    ...
                    if (
                        (start_in_outer and not start_in_inner)
                        or (end_in_outer and not end_in_inner)
                        or (start_in_inner and end_in_outer)
                        or (end_in_inner and start_in_outer)
                    ):
                        continue

                # Disallow vertices, predicates, or other cuts between the double cut
                if (
                    isinstance(item, QGraphicsTextItem)  # Predicates
                    or isinstance(item, QGraphicsEllipseItem)  # Vertices
                    or (
                        hasattr(item, "cut_id")
                        and getattr(item, "cut_id", None) not in [outer_id, inner_id]
                    )
                ):  # Other cuts
                    return False
    ...
    def _add_arity_annotation(
        self, predicate_item: QGraphicsTextItem, arity: int
    ) -> None:
        ...
        annotation.setPos(
            predicate_item.x() + pred_rect.width() + 2, predicate_item.y() - 2
        )

        self.scene.addItem(annotation)

    def _add_variable_annotation(
        self, vertex_item: QGraphicsItem, var_name: str
    ) -> None:
    ...
    def _add_vertex_name_display(
        self, vertex_item: QGraphicsItem, vertex_name: str
    ) -> None:
    ...
    def _draw_cut(
        ...
        if cut_id in self.cut_items:
            ...
            try:
                ...
                existing_cut.setRect(
                    QRectF(0, 0, w, h)
                )  # Rect is relative to item position
                return
            except RuntimeError:
    ...
    def render_vertex(
        self, vertex_dto, gui_x: float = None, gui_y: float = None
    ) -> InteractiveVertex:
        ...
        if vertex_id in self.vertex_items:
            ...
            try:
                print(
                    f"RENDER VERTEX: Vertex {vertex_id} already exists, returning existing"
                )
        ...
        dot.setPen(
            QPen(
                QColor(style.get("border_color", "#000000")),
                float(style.get("border_width", 1)),
            )
        )
        ...
        label = (
            vertex_dto.get("label")
            if hasattr(vertex_dto, "get")
            else getattr(vertex_dto, "label", None)
        )
        if label:
            from PySide6.QtGui import QFont
            from PySide6.QtWidgets import QGraphicsTextItem
    ...
    def _draw_ligatures(
        self, nu_map: Dict, predicate_rects: Dict[str, QRectF], vertices: Dict
    ) -> None:
        """Draw ligatures with proper boundary anchoring."""
    ...
    def _detect_containing_area_for_position(self, position):
        ...
        if hasattr(self, "coordinator") and self.coordinator:
            print(
                f"DEBUG: Found coordinator with {len(self.coordinator.egi_state.cuts)} cuts"
            )
            ...
            for cut_id, cut_dto in self.coordinator.egi_state.cuts.items():
                ...
                print(
                    f"DEBUG: Checking cut {cut_id} at pos ({cut_x}, {cut_y}), size {cut_width}x{cut_height}"
                )

                # Check if position is within this cut's bounds
                if (
                    cut_x <= position.x() <= cut_x + cut_width
                    and cut_y <= position.y() <= cut_y + cut_height
                ):
                    ...
                    if actual_cut_item:
                        ...
                        print(
                            f"DEBUG: Using CALCULATED bounds for {cut_id} (no Qt item found)"
                        )

                    print(
                        f"DEBUG: Cut {cut_id} bounds: left={cut_left}, right={cut_right}, top={cut_top}, bottom={cut_bottom}"
                    )
                    print(f"DEBUG: Movement at x={position.x()}, y={position.y()}")

                    # Check if scene position is inside cut bounds
                    if (
                        cut_left <= position.x() <= cut_right
                        and cut_top <= position.y() <= cut_bottom
                    ):
                        ...
                        print(
                            f"DEBUG: COORDINATE MISMATCH - Visual vs Logical bounds don't align"
                        )
        else:
            ...
            if hasattr(self, "coordinator") and self.coordinator:
                ...
                if hasattr(self.coordinator, "generate_egi_from_diagram"):
                    ...
                    if new_egi:
                        ...
                        print(
                            f"DEBUG: Successfully regenerated EGI with updated area assignments"
                        )
                    else:
                        print(f"DEBUG: Failed to generate EGI from diagram")
                else:
                    print(
                        f"DEBUG: Coordinator missing generate_egi_from_diagram method"
                    )
            else:
    ...
    def _refresh_egif(self):
        ...
        for widget in self.scene.views():
            if hasattr(widget, "parent") and hasattr(
