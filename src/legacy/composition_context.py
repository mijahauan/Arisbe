...
class StandardCompositionContexts:
    ...
    @staticmethod
    def create_double_cut_context(
        context_id: str = "standard_composition",
    ) -> CompositionContext:
        """
        ...
        if composition_area is None:
            raise ValueError(
                "Could not identify composition area in double cut structure"
            )
    ...
    @staticmethod
    def create_deeper_context(
        context_id: str = "deep_composition", depth: int = 4
    ) -> CompositionContext:
        """
        ...
        if composition_area is None:
            raise ValueError(
                "Could not identify composition area in deep context structure"
            )
    ...
    @staticmethod
    def create_domain_model_context(
        domain_egif: str, context_id: str = "domain_composition"
    ) -> CompositionContext:
        """
        ...
        if composition_area is None:
            raise ValueError(
                "Could not identify composition area in domain model context"
            )
...
class EndoporeuticGameContext:
    ...
    def add_move(
        self, player: str, move_type: str, content: str, description: str = ""
    ) -> None:
...
def test_composition_contexts():
    ...
    try:
        domain_context = StandardCompositionContexts.create_domain_model_context(
            domain_egif
        )
