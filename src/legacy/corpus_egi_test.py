...
class CorpusEGITestWindow(QMainWindow):
    ...
    def load_selected_egi(self):
        ...
        try:
            ...
            self.status_text.append(
                f"🔄 Loading EGI: {selected_item.get('title', 'Unknown')}"
            )
            ...
            spatial_layout = (
                self.current_controller.spatial_correspondence.generate_complete_spatial_layout()
            )
    ...
    def add_egi_to_controller(self, egi):
        ...
        for edge in egi.E:
            ...
            relation_name = (
                egi.get_relation_name(edge.id)
                if hasattr(egi, "get_relation_name")
                else egi.rel.get(edge.id, edge.id)
            )
        ...
        for edge_id, vertex_sequence in egi.nu.items():
            ...
            for vertex_id in vertex_sequence:
                self.current_controller.bind_vertex_to_predicate(
                    vertex_id, edge_id, e_ctx
                )
    ...
    def render_egi_predicate(self, command):
        ...
        try:
            for (
                area_id,
                area,
            ) in self.current_controller.logical_system.logical_areas.items():
    ...
    def render_egi_cut(self, command):
        ...
        brush_color = (
            QColor(240, 240, 240, 100) if (depth % 2 == 1) else QColor(255, 255, 255, 0)
        )
...
def main():
    ...
    print(
        "corpus_egi_test.py is temporarily disabled while the headless EGI↔spatial adapter is stabilized."
