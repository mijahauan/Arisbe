...
class SpatialRegion:
    ...
    def get_available_area(self) -> float:
        ...
        child_area = sum(
            child.width * child.height for child in self.get_child_regions()
        )
...
class SpatialRegionManager:
    ...
    def create_cut_region(
        self,
        cut_logical_area_id: str,
        parent_logical_area_id: str,
        requested_size: Optional[Tuple[float, float]] = None,
    ) -> str:
        """
        ...
        if not parent_region_id:
            raise ValueError(
                f"Parent logical area {parent_logical_area_id} has no spatial region"
            )
        ...
        if os.environ.get("ARISBE_DEBUG_EGI") == "1":
            print(
                f"DEBUG: Created cut region {cut_region_id} at ({cut_x}, {cut_y}, {cut_width}, {cut_height}) within parent {parent_region_id}"
            )

        return cut_region_id

    def get_region_for_logical_area(
    ...
    def get_logical_area_at_point(self, x: float, y: float) -> str:
        ...
        deepest_region = max(
            containing_regions, key=lambda r: self._get_region_depth(r.region_id)
        )
    ...
    def adjust_region_for_content_change(
        self, logical_area_id: str, content_bounds: Tuple[float, float, float, float]
    ):
        """
        ...
        if os.environ.get("ARISBE_DEBUG_EGI") == "1":
            print(
                f"DEBUG: Adjusted region {region.region_id} to ({new_x}, {new_y}, {new_width}, {new_height})"
            )
