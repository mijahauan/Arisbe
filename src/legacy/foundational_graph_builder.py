...
from immutable_transformation_architecture import TransformationRuleType, ContextType, TransformationStep
...
class AssertionSheet:
    ...
    constructions: Dict[str, str] = field(default_factory=dict)  # construction_id -> egi_id
...
class FoundationalGraphBuilder:
    ...
    def __init__(self, default_sheet_type: SheetType = SheetType.COMMON_SHEET):
        ...
        if default_sheet_type == SheetType.COMMON_SHEET:
            self.common_sheet_id = self.create_assertion_sheet(
                "Common Assertion Sheet",
                "Shared sheet for all graph constructions"
            )
    ...
    def start_construction(self, title: str, description: str, 
                          sheet_id: Optional[str] = None) -> str:
        ...
        if sheet_id is None and self.common_sheet_id:
            ...
            sheet_id = self.create_assertion_sheet(
                f"Sheet for {title}",
                f"Individual assertion sheet for {description}"
            )
    ...
    def add_to_construction(self, construction_id: str, 
    ...
    def copy_subgraph_pattern(self, source_egi_id: str, target_construction_id: str,
        ...
        for edge in source_egi.E:
            if edge.element_id in source_egi.nu:
                ...
                new_vertex_sequence = tuple(f"{pattern_name}_{vid}" for vid in vertex_sequence)
    ...
    def fill_disjunction_pattern(self, pattern_egi_id: str, x_elements: List[Dict[str, Any]], 
                                y_elements: List[Dict[str, Any]]) -> str:
        """
    ...
    def _determine_rule_type(self, transformation_data: Dict[str, Any]) -> TransformationRuleType:
