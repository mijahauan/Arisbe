...
class SimpleGraphBuilder:
    ...
    def create_empty_context(
        self, context_type: GraphBuildingContext = GraphBuildingContext.EMPTY_SHEET
    ) -> str:
    ...
    def start_building_session(self, title: str) -> str:
        ...
        session = BuildingSession(session_id=session_id, title=title)
    ...
    def build_graph_utterance(
        self,
        title: str,
        description: str,
        building_steps: List[Dict[str, Any]],
        context_type: GraphBuildingContext = GraphBuildingContext.EMPTY_SHEET,
    ) -> str:
        ...
        for i, step_data in enumerate(building_steps):
            try:
                ...
                for step in self.pipeline.repository.transformation_steps.values():
                    if (
                        step.target_egi_id == new_egi_id
                        and step.source_egi_id == current_egi_id
                    ):
    ...
    def list_utterances(self) -> List[GraphUtterance]:
        ...
        return sorted(
            self.utterances.values(), key=lambda u: u.created_at, reverse=True
        )
...
def create_basic_graph_utterances():
    ...
    for utterance_def in utterances:
        ...
        print(
            f"   Final state: {analysis['final_state']['vertices']}V, {analysis['final_state']['edges']}E, {analysis['final_state']['cuts']}C"
        )
    ...
    # Overall analysis
    total_steps = sum(
        builder.analyze_utterance(uid)["transformation_steps"]
        for uid in built_utterances
    )
