...
class SpatialElement:
    ...
    edge_sup_bounds: Optional[SpatialBounds] = (
        None  # for arity superscript near predicate
    )
...
class LogicalLayout:
    ...
    ligature_endpoints: List[Tuple[str, str]] = field(
...
class Chapter16Validator:
    ...
    def validate_branch_movement(
        self, ligature_id: str, branch_point: BranchingPoint, new_position: float
    ) -> bool:
    ...
    def validate_ligature_retraction(
        self, ligature_id: str, target_vertex: str
    ) -> bool:
    ...
    def validate_ligature_rearrangement(
        self, old_geometry: LigatureGeometry, new_geometry: LigatureGeometry
    ) -> bool:
...
class SpatialCorrespondenceEngine:
    ...
    def generate_spatial_layout_staged(
        self, canvas_size: Tuple[int, int] = (1200, 800)
    ...
    def build_logical_layout(self) -> LogicalLayout:
        ...
        try:
            pad_x = (
                float(pad[0])
                if isinstance(pad, (list, tuple)) and len(pad) >= 1
                else 2.0
            )
            pad_y = (
                float(pad[1])
        ...
        for v in self.egi.V:
            ...
            ll.elements[v.id] = LogicalElement(
                id=v.id, kind="vertex", area_id=area_id, est_width=10.0, est_height=10.0
            )
        for e in self.egi.E:
            ...
            ll.elements[e.id] = LogicalElement(
                id=e.id, kind="edge", area_id=area_id, est_width=est_w, est_height=est_h
            )
    ...
    def fit_to_canvas(
        self, logical_layout: LogicalLayout, canvas_size: Tuple[int, int]
    ) -> Tuple[Dict[str, SpatialBounds], Dict[str, Tuple[float, float, float, float]]]:
        ...
        def size_area(aid: str) -> Tuple[float, float]:
            ...
            inner_w = max(
                math.sqrt(total_area) if total_area > 0 else 0.0,
                max_elem_w,
                max_child_w,
            )
            inner_h = max(
                (total_area / inner_w) if inner_w > 0 else 0.0, max_elem_h, max_child_h
            )
        ...
        top_levels = [
            a.id for a in logical_layout.areas.values() if a.parent == self.egi.sheet
        ]
        ...
        used_w = (
            max((b.x + b.width for b in area_mappings.values()), default=1.0) + 20.0
        )
        used_h = (
            max((b.y + b.height for b in area_mappings.values()), default=1.0) + 20.0
        )
    ...
    def _generate_cut_areas(self, layout: Dict[str, SpatialElement]):
        ...
        for i, cut in enumerate(cuts_ordered):
            ...
            if os.environ.get("ARISBE_DEBUG_EGI") == "1":
                try:
                    print(
                        f"DEBUG: Cut {cut.id} initial size estimate width={bounds.width:.1f} height={bounds.height:.1f} (contents={len(cut_contents)})"
                    )
                except Exception:
                    pass
            ...
            if (
                getattr(self, "layout_mode", "direct") == "staged"
                and parent_area != self.egi.sheet
            ):
            ...
            if parent_bounds:
                ...
                if os.environ.get("ARISBE_DEBUG_EGI") == "1":
                    try:
                        print(
                            f"DEBUG: Considering nesting cut {cut.id} into parent {parent_area}; parent_inner=({parent_inner_w:.1f},{parent_inner_h:.1f}) child_req=({cut_width:.1f},{cut_height:.1f}) expand_w={need_expand_w} expand_h={need_expand_h}"
                        )
                    except Exception:
                        pass
                if need_expand_w or need_expand_h:
                    ...
                    if os.environ.get("ARISBE_DEBUG_EGI") == "1":
                        try:
                            print(
                                f"DEBUG: Expanding parent {parent_area} from ({parent_bounds.width:.1f},{parent_bounds.height:.1f}) to ({new_parent_w:.1f},{new_parent_h:.1f}) for child {cut.id}"
                            )
                        except Exception:
                            pass
                    new_parent_bounds = SpatialBounds(
                        parent_bounds.x, parent_bounds.y, new_parent_w, new_parent_h
                    )
                    # Update parent in current layout if present
                    parent_elt = layout.get(parent_area)
                    if (
                        parent_elt
                        and getattr(parent_elt, "element_type", None) == "cut"
                    ):
                    ...
                    for pb, pb_parent in placed_bounds:
                        if (
                            pb is parent_bounds
                            and pb_parent
                            == self._determine_cut_parent_area(parent_area)
                        ):
            ...
            try:
                shift_dx = (
                    float(_ss[0])
                    if isinstance(_ss, (list, tuple)) and len(_ss) >= 1
                    else 40.0
                )
                shift_dy = (
                    float(_ss[1])
            ...
            while any_sibling_overlap():
                ...
                if parent_bounds:
                    nx = min(
                        parent_bounds.x + parent_bounds.width - bounds.width - 10.0,
                        bounds.x + shift_dx,
                    )
                    ny = min(
                        parent_bounds.y + parent_bounds.height - bounds.height - 10.0,
                        bounds.y + shift_dy,
                    )
                    ...
                    bx = max(
                        ...
                        min(
                            bounds.x,
                            parent_bounds.x + parent_bounds.width - bounds.width - 5.0,
                        ),
                    )
                    by = max(
                        parent_bounds.y + 5.0,
                ...
                if guard > 50:
                    if os.environ.get("ARISBE_DEBUG_EGI") == "1":
                        print(
                            f"DEBUG: Sibling-overlap shift guard tripped for cut {cut.id} in parent {parent_area}; bounds={bounds}"
                        )
                    break
            if os.environ.get("ARISBE_DEBUG_EGI") == "1":
                try:
                    ...
                    print(
                        f"DEBUG: Final bounds for cut {cut.id}: (x={bounds.x:.1f}, y={bounds.y:.1f}, w={bounds.width:.1f}, h={bounds.height:.1f}) {pb_desc}"
                    )
                except Exception:
                    pass
    ...
    def _generate_ligatures_with_branching(
        self, layout: Dict[str, SpatialElement]
    ) -> None:
        """Generate ligatures with proper branching points per Chapter 16."""
        # Use graph-aware layout that understands predicate relationships
        vertex_positions = self._calculate_graph_aware_layout()
        ...
        for vertex_id, pos in vertex_positions.items():
            layout[vertex_id] = SpatialElement(
                ...
                element_type="vertex",
                position=pos,
                bounds=SpatialBounds(pos.x - 5, pos.y - 5, 10, 10),
                style_class="vertex",
            )

        # Handle any remaining vertices not in the graph-aware layout
        for vertex in self.egi.V:
            if vertex.id not in layout:
                # Place unconnected vertices in default positions
                logical_area = self._determine_vertex_area(vertex.id)
            ...
            bounds = self._ligature_bounds_excluding_children(
                lig_area, path_points, layout
            )
    ...
    def _position_predicates(self, layout: Dict[str, SpatialElement]):
        ...
        for edge in self.egi.E:
            ...
            if not connected_vertices:
                ...
                cx, cy = self._pick_safe_point_in_area(
                    logical_area, layout, (200.0, 200.0)
                )
            else:
                ...
                for vid in connected_vertices:
                    ...
                    if ve and ve.spatial_bounds:
                        ...
                        pts.append(
                            self._pick_safe_point_in_area(
                                v_area, layout, (200.0, 200.0)
                            )
                        )
            ...
            def rects_overlap(
                a: SpatialBounds, b: SpatialBounds, margin: float = 0.0
            ) -> bool:
                return not (
                    a.x + a.width <= b.x - margin
                    or b.x + b.width <= a.x - margin
                    or a.y + a.height <= b.y - margin
                    or b.y + b.height <= a.y - margin
                )
            ...
            while any(rects_overlap(bounds, ob, sep_margin) for ob in obstacles):
                ...
                bounds = SpatialBounds(
                    rcx - est_w / 2.0, rcy - est_h / 2.0, est_w, est_h
                )
                bounds = self._clamp_bounds_into_area(bounds, logical_area)
                bounds = self._avoid_child_cut_holes(bounds, logical_area, layout)
                guard += 1
            ...
            if cut_b:
                ...
                nx = min(
                    max(bounds.x, cut_b.x),
                    cut_b.x + max(0.0, cut_b.width - bounds.width),
                )
                ny = min(
                    max(bounds.y, cut_b.y),
                    cut_b.y + max(0.0, cut_b.height - bounds.height),
                )
                nw = bounds.width
                nh = bounds.height
            ...
            layout[edge.id] = SpatialElement(
                ...
                spatial_bounds=bounds,
                relation_name=(
                    self.egi.rel.get(edge.id, "") if hasattr(self.egi, "rel") else ""
                ),
            )
    ...
    def _snap_ligatures_to_predicates(self, layout: Dict[str, SpatialElement]) -> None:
        ...
        for lid, elt in list(layout.items()):
            if elt.element_type != "ligature" or not getattr(
                elt, "ligature_geometry", None
            ):
                continue
            ...
            for v_id, base in bases:
                ...
                for eid in connected:
                    ...
                    hook_x, hook_y = self._hook_point_for_edge_argument(
                        e_elem.spatial_bounds, arg_index, arity
                    )
                    ex, ey = hook_x, hook_y
                    # Inward nudge
                    ...
                    if edge_area == lig_area:
                        for oid, oelem in layout.items():
                            ...
                            pad = float(
                                self.style.resolve(type="layout").get(
                                    "label_obstacle_padding", 6.0
                                )
                            )
                            label_obs.append(
                                (
                    ...
                    approach_pt = (
                        hook_x + ux * approach_margin,
                        hook_y + uy * approach_margin,
                    )
                    if edge_area != lig_area:
                        ...
                        if cut_b:
                            ax, ay = approach_pt
                            ax = min(
                                max(ax, cut_b.x + 1.0), cut_b.x + cut_b.width - 1.0
                            )
                            ay = min(
                                max(ay, cut_b.y + 1.0), cut_b.y + cut_b.height - 1.0
                            )
                            approach_pt = (ax, ay)
                    ...
                    arm_path = self._visibility_route(
                        base, approach_pt, label_obs + eff_obs + area_walls
                    )
                    ...
                    def _pt_in_rect(
                        p: Tuple[float, float], r: Tuple[float, float, float, float]
                    ) -> bool:
                        rx, ry, rw, rh = r
                        return (rx < p[0] < rx + rw) and (ry < p[1] < ry + rh)

                    def _pt_on_rect_border(
                        p: Tuple[float, float], r: Tuple[float, float, float, float]
                    ) -> bool:
                        rx, ry, rw, rh = r
                        on_left = abs(p[0] - rx) < 1e-6 and (ry - 1e-6) <= p[1] <= (
                            ry + rh + 1e-6
                        )
                        on_right = abs(p[0] - (rx + rw)) < 1e-6 and (ry - 1e-6) <= p[
                            1
                        ] <= (ry + rh + 1e-6)
                        on_top = abs(p[1] - ry) < 1e-6 and (rx - 1e-6) <= p[0] <= (
                            rx + rw + 1e-6
                        )
                        on_bottom = abs(p[1] - (ry + rh)) < 1e-6 and (rx - 1e-6) <= p[
                            0
                        ] <= (rx + rw + 1e-6)
                        ...
                        for j in range(1, len(path)):
                            ...
                            for rr in raw_label_rects_all:
                                if self._segment_intersects_rect(pa, pb, rr):
                                    ain = _pt_in_rect(pa, rr) or _pt_on_rect_border(
                                        pa, rr
                                    )
                                    bin = _pt_in_rect(pb, rr) or _pt_on_rect_border(
                                        pb, rr
                                    )
                                    if not (ain ^ bin):
                                        return True
                    ...
                    padded_cuts = [
                        (x - base_pad, y - base_pad, w + 2 * base_pad, h + 2 * base_pad)
                        for (x, y, w, h) in raw_cut_rects_all
                    ]
                    repaired = [arm_path[0]]
                    for i in range(1, len(arm_path)):
                        a, bpt = repaired[-1], arm_path[i]
                        # Detect true pass-throughs of any label rect
                        offending_rect: Optional[Tuple[float, float, float, float]] = (
                            None
                        )
                        crosses = False
                        for rr in raw_label_rects_all:
                            if self._segment_intersects_rect(a, bpt, rr):
                                ...
                                bin = _pt_in_rect(bpt, rr) or _pt_on_rect_border(
                                    bpt, rr
                                )
                                if not (ain ^ bin):
                                    crosses = True
                        ...
                        if offending_rect is not None:
                            ...
                            for c in candidates:
                                ...
                                det2 = self._visibility_route(
                                    c, bpt, label_rects_padded
                                )
                                if (
                                    det1
                                    and det2
                                    and not _seg_still_crosses(det1)
                                    and not _seg_still_crosses(det2)
                                ):
                            ...
                            if not success:
                                ...
                                lsuccess = False
                                for w in waypoints:
                                    det1 = self._visibility_route(
                                        a, w, label_rects_heavy
                                    )
                                    det2 = self._visibility_route(
                                        w, bpt, label_rects_heavy
                                    )
                                    if (
                                        det1
                                        and det2
                    ...
                    try:
                        if edge_area == lig_area:
                            ...
                            self._assert_path_within_area(
                                arm_path,
                                lig_area,
                                allowed_child=edge_area,
                                allow_last_inside_child=True,
                            )
                            ...
                            if getattr(oelem, "element_type", "") == "edge":
                                ...
                                heavy_labels.append(
                                    (
                                        ob.x - heavy_pad,
                                        ob.y - heavy_pad,
                                        ob.width + 2 * heavy_pad,
                                        ob.height + 2 * heavy_pad,
                                    )
                                )
                        retry = self._visibility_route(
                            base, approach_pt, heavy_labels + area_walls
                        )
                        if retry:
                            if retry[0] != base:
                                retry = [base] + retry
                    ...
                    new_bounds = self._ligature_bounds_excluding_children(
                        lig_area, arm_path, layout
                    )
                    layout[new_id] = SpatialElement(
                        element_id=new_id,
                        element_type="ligature",
                        logical_area=lig_area,
                        spatial_bounds=new_bounds,
                        ligature_geometry=lg,
                    )
            ...
            if created_any:
                layout_tokens = self.style.resolve(type="layout")
                enable_branch = bool(
                    layout_tokens.get("enable_vertex_branch_consolidation", False)
                )
                if enable_branch:
                    # Group newly created ligatures by vertex id
                    base_by_vertex: Dict[str, Tuple[float, float]] = {
                        vid: b for vid, b in bases
                    }
                    for v_id, base in base_by_vertex.items():
                        ...
                        for nid in created_ids:
                            ...
                            if (
                                not geom_i
                                or not geom_i.spatial_path
                                or len(geom_i.spatial_path) < 2
                            ):
                                continue
                        ...
                        avg_dy /= norm
                        trunk_len = float(
                            layout_tokens.get("ligature_trunk_offset", 12.0)
                        )
                        hub = (
                            base[0] + avg_dx * trunk_len,
                            base[1] + avg_dy * trunk_len,
                        )
                        ...
                        if area_bounds:
                            hx = min(
                                max(hub[0], area_bounds.x + 1.0),
                                area_bounds.x + area_bounds.width - 1.0,
                            )
                            hy = min(
                                max(hub[1], area_bounds.y + 1.0),
                                area_bounds.y + area_bounds.height - 1.0,
                            )
                            hub = (hx, hy)
                        ...
                        b2h = self._visibility_route(
                            base, hub, label_obs_v + cut_obs_v + area_walls_v
                        ) or [base, hub]
                        ...
                        for nid in created_ids:
                            ...
                            if old[0] != base:
                                ...
                                conn = self._visibility_route(
                                    base, old[0], label_obs_v + cut_obs_v + area_walls_v
                                ) or [base, old[0]]
                                prefix = conn
                            else:
                                prefix = []
                            ...
                            if prefix:
                                ...
                                hub_to_pref = self._visibility_route(
                                    newp[-1],
                                    prefix[0],
                                    label_obs_v + cut_obs_v + area_walls_v,
                                )
                                if hub_to_pref:
                            ...
                            hub_to_first = self._visibility_route(
                                newp[-1],
                                target_after_hub,
                                label_obs_v + cut_obs_v + area_walls_v,
                            )
                            ...
                            for oid, oelem in layout.items():
                                if getattr(oelem, "element_type", "") != "edge":
                                    continue
                                ob = oelem.spatial_bounds
                                raw_label_rects_all.append(
                                    (ob.x, ob.y, ob.width, ob.height)
                                )
                                label_rects_all.append(
                                    (
                                        ob.x - pad_v,
                                        ob.y - pad_v,
                                        ob.width + 2 * pad_v,
                            ...
                            raw_cut_rects_all: List[
                                Tuple[float, float, float, float]
                            ] = []
                            for c in self.egi.Cut:
                                b = self.correspondence.area_mappings.get(c.id)
                                if b:
                                    raw_cut_rects_all.append(
                            ...
                            def _pt_on_border2(
                                ...
                                rx, ry, rw, rh = r
                                on_left = abs(p[0] - rx) < 1e-6 and (ry - 1e-6) <= p[
                                    1
                                ] <= (ry + rh + 1e-6)
                                on_right = abs(p[0] - (rx + rw)) < 1e-6 and (
                                    ry - 1e-6
                                ) <= p[1] <= (ry + rh + 1e-6)
                                on_top = abs(p[1] - ry) < 1e-6 and (rx - 1e-6) <= p[
                                    0
                                ] <= (rx + rw + 1e-6)
                                on_bottom = abs(p[1] - (ry + rh)) < 1e-6 and (
                                    rx - 1e-6
                                ) <= p[0] <= (rx + rw + 1e-6)
                                return on_left or on_right or on_top or on_bottom

                            def _pass_through(
                                a: Tuple[float, float], bpt: Tuple[float, float]
                            ) -> bool:
                                for rr in raw_label_rects_all + raw_cut_rects_all:
                                    if self._segment_intersects_rect(a, bpt, rr):
                                        ain = _pt_in_rect2(a, rr) or _pt_on_border2(
                                            a, rr
                                        )
                                        binv = _pt_in_rect2(bpt, rr) or _pt_on_border2(
                                            bpt, rr
                                        )
                                        if not (ain ^ binv):
                                            return True
                            ...
                            for i in range(1, len(newp)):
                                ...
                                if _pass_through(a, bpt):
                                    seg2 = self._visibility_route(
                                        a,
                                        bpt,
                                        label_rects_all + cut_obs_v + area_walls_v,
                                    )
                                    if seg2:
                                        if repaired[-1] == seg2[0]:
                                            repaired.extend(seg2[1:])
                            ...
                            nb = self._ligature_bounds_excluding_children(
                                v_area, repaired, layout
                            )
                            layout[nid].spatial_bounds = nb
                            try:
                                bp = BranchingPoint(
                                    ligature_id=nid,
                                    position_on_ligature=0.05,
                                    spatial_position=hub,
                                    constant_label=self._get_vertex_constant_label(
                                        v_id
                                    ),
                                )
                                ...
                                pass
                            new_bounds_i = self._ligature_bounds_excluding_children(
                                v_area, repaired, layout
                            )
                            se = layout.get(nid)
                            if se:
            ...
            for oid, oelem in layout.items():
                ...
                label_rects.append(
                    (ob.x - pad, ob.y - pad, ob.width + 2 * pad, ob.height + 2 * pad)
                )
            ...
            def _pt_on_rect_border(
                ...
                on_right = abs(p[0] - (rx + rw)) < 1e-6 and (ry - 1e-6) <= p[1] <= (
                    ry + rh + 1e-6
                )
                on_top = abs(p[1] - ry) < 1e-6 and (rx - 1e-6) <= p[0] <= (
                    rx + rw + 1e-6
                )
                on_bottom = abs(p[1] - (ry + rh)) < 1e-6 and (rx - 1e-6) <= p[0] <= (
                    rx + rw + 1e-6
                )
                return on_left or on_right or on_top or on_bottom
            ...
            while passes < 6:
                ...
                for i, pt in enumerate(path_in):
                    ...
                    if crosses:
                        ...
                        layout_tokens = self.style.resolve(type="layout")
                        base_pad = float(
                            layout_tokens.get("label_obstacle_padding", 6.0)
                        )
                        padded_cuts = [
                            (
                                x - base_pad,
                        ...
                        if offending_rect is not None:
                            ...
                            if not success:
                                ...
                                for c in candidates:
                                    det1 = self._visibility_route(
                                        a, c, label_rects_heavy
                                    )
                                    det2 = self._visibility_route(
                                        c, b, label_rects_heavy
                                    )
                                    if (
                                        det1
                                        and det2
    ...
    def _hook_point_for_edge_argument(
        self, bounds: SpatialBounds, index: int, arity: int
    ) -> Tuple[float, float]:
    ...
    def _position_superscripts(self, layout: Dict[str, SpatialElement]) -> None:
        ...
        for vid, v_elem in list(layout.items()):
            ...
            for oid, oelem in layout.items():
                if oid == vid:
                    continue
                if (
                    getattr(oelem, "element_type", "") == "edge"
                    and self._determine_element_area(oid) == area_id
                ):
                    obstacles.append(oelem.spatial_bounds)
                if (
                    getattr(oelem, "element_type", "") == "vertex"
                    and self._determine_element_area(oid) == area_id
                    and getattr(oelem, "vertex_sup_bounds", None)
                ):
            ...
            while any(rects_overlap(bounds, ob, sep_margin) for ob in obstacles):
                ...
                bounds = SpatialBounds(
                    nbx - est_w / 2.0, nby - est_h / 2.0, est_w, est_h
                )
        ...
        for eid, e_elem in list(layout.items()):
            ...
            ex, ey, ew, eh = (
                e_elem.spatial_bounds.x,
                e_elem.spatial_bounds.y,
                e_elem.spatial_bounds.width,
                e_elem.spatial_bounds.height,
            )
    ...
    def _ligature_bounds_excluding_children(
        self,
        lig_area: str,
        path_points: List[Tuple[float, float]],
        layout: Dict[str, SpatialElement],
    ) -> SpatialBounds:
        ...
        def rects_overlap(
            ax: float, ay: float, aw: float, ah: float, b: SpatialBounds
        ) -> bool:
            return not (
                ax + aw <= b.x
                or b.x + b.width <= ax
        ...
        while (
            any(rects_overlap(nx, ny, w, h, cb) for cb in child_bounds) and guard < 12
        ):
            for cb in child_bounds:
                ...
                right_overlap = (
                    nx + w
                ) - cb.x  # >0 if our right intrudes into child's left
                left_overlap = (
                    cb.x + cb.width
                ) - nx  # >0 if our left intrudes into child's right
                top_overlap = (
                    cb.y + cb.height
                ) - ny  # >0 if our top intrudes into child's bottom
                bottom_overlap = (
                    ny + h
                ) - cb.y  # >0 if our bottom intrudes into child's top
    ...
    def _compute_border_hook(
        self, rect: "SpatialBounds", from_pt: Tuple[float, float]
    ) -> Tuple[float, float]:
        ...
        def add_if_valid(t: float, x: float, y: float):
            if (
                t >= 0
                and rx1 - 1e-6 <= x <= rx2 + 1e-6
                and ry1 - 1e-6 <= y <= ry2 + 1e-6
            ):
    ...
    def validate_mapping_consistency(
        self, layout: Optional[Dict[str, SpatialElement]] = None
    ) -> bool:
        ...
        for vid in egi_vertex_ids:
            ...
            if se.element_type != "vertex":
                raise ValueError(
                    f"Spatial element for vertex {vid} has wrong type {se.element_type}"
                )
            sid = self.correspondence.egi_to_spatial.get(vid)
            if not sid or self.correspondence.spatial_to_egi.get(sid) != vid:
                raise ValueError(
                    f"Correspondence mapping missing/incorrect for vertex {vid}"
                )
        return True

    def validate_spatial_exclusion(
        self, layout: Optional[Dict[str, SpatialElement]] = None
    ) -> bool:
        ...
        def path_intersects_rect(
            path: List[Tuple[float, float]], rect: SpatialBounds
        ) -> bool:
            rx1, ry1 = rect.x, rect.y
            rx2, ry2 = rect.x + rect.width, rect.y + rect.height
            ...
            def seg_intersects(
                ...
                x2, y2 = p2
                # Quick inside
                if (rx1 <= x1 <= rx2 and ry1 <= y1 <= ry2) or (
                ...
                def on_seg(a, b, c):
                    return min(a[0], b[0]) <= c[0] <= max(a[0], b[0]) and min(
                        a[1], b[1]
                    ) <= c[1] <= max(a[1], b[1])
        ...
        for sid, se in layout.items():
            ...
            if area in area_bounds:
                ...
                if se.element_type == "ligature":
                    ...
                    if (
                        getattr(se, "ligature_geometry", None)
                        and se.ligature_geometry.spatial_path
                    ):
                    ...
                    if not cut_b.contains_point(base_pt[0], base_pt[1]):
                        raise ValueError(
                            f"Ligature {sid} base point not inside assigned cut '{area}'"
                        )
                else:
                    if not contains(se_bounds, cut_b):
                        raise ValueError(
                            f"Element {sid} assigned to cut '{area}' not fully contained within its bounds"
                        )
            ...
            for child in children:
                ...
                if overlaps(se_bounds, child_b):
                    if se.element_type == "ligature":
                        ...
                        if (
                            getattr(se, "ligature_geometry", None)
                            and se.ligature_geometry.vertices
                        ):
                        ...
                        if getattr(
                            se, "ligature_geometry", None
                        ) and path_intersects_rect(
                            se.ligature_geometry.spatial_path, child_b
                        ):
                            if child in target_areas:
                                continue
                            raise ValueError(
                                f"Ligature {sid} path intersects child cut '{child}' hole"
                            )
                        # Otherwise bounding box overlap without path crossing is acceptable
                        continue
                    raise ValueError(
                        f"Element {sid} in area '{area}' overlaps child cut '{child}' hole"
                    )
        return True

    def _apply_styling_system(
        self, layout: Dict[str, SpatialElement]
    ) -> Dict[str, SpatialElement]:
        ...
        for element in layout.values():
            if element.element_type == "ligature" and element.ligature_geometry:
                if not element.ligature_geometry.validate_chapter16_constraints():
                    raise ValueError(
                        f"Ligature {element.element_id} violates Chapter 16 constraints"
                    )

    def handle_branching_point_drag(
        self, ligature_id: str, branch_point_index: int, new_position: float
    ) -> bool:
        ...
        if not ligature_geometry or branch_point_index >= len(
            ligature_geometry.branching_points
        ):
            return False
        ...
        if not self.validator.validate_branch_movement(
            ligature_id, branch_point, new_position
        ):
            return False
    ...
    def apply_ligature_transformation(
        self,
        transformation_type: LigatureTransformationType,
        ligature_id: str,
        **kwargs,
    ) -> bool:
    ...
    def _identify_ligatures(self) -> Dict[str, List[str]]:
        ...
        for vertex_id, connected_edges in vertex_to_edges.items():
            if (
                len(connected_edges) >= 1
            ):  # Vertex participates in at least one relation
    ...
    def _generate_ligature_path(
        self, vertex_ids: List[str], layout: Dict[str, SpatialElement]
        ...
        for c in self.egi.Cut:
            if (
                is_descendant_cut(c.id, lig_area)
                or self._determine_cut_parent_area(c.id) == lig_area
            ):
                b = self.correspondence.area_mappings.get(c.id)
                if b:
                    obstacles.append((b.x, b.y, b.width, b.height))
        ...
        for i, edge_id in enumerate(connected_edges):
            ...
            if edge_area != vertex_area:
                ...
                segment_path = self._visibility_route(
                    (base_x, base_y), pred, eff_obstacles
                )
                ...
                for j in range(i + 1, n):
                    ...
                    for p1, p2 in segs(a.spatial_path):
                        for q1, q2 in segs(b.spatial_path):
                            ...
                            if not any(
                                math.hypot(ipt[0] - bx, ipt[1] - by) < 1.0
                                for (bx, by) in under.bridges
                            ):
    ...
    def _segment_intersects_rect(
        ...
        if (rx1 <= p1[0] <= rx2 and ry1 <= p1[1] <= ry2) or (
            rx1 <= p2[0] <= rx2 and ry1 <= p2[1] <= ry2
        ):
            return True
    ...
    def _segment_rect_intersection_point(
        self,
        p1: Tuple[float, float],
        p2: Tuple[float, float],
        rect: Tuple[float, float, float, float],
    ...
    def _interpolate_path(
        self, path: List[Tuple[float, float]], position: float
    ) -> Tuple[float, float]:
    ...
    def _calculate_ligature_bounds(
    ...
    def _calculate_predicate_position(
        self, connected_vertices: Tuple[str, ...], layout: Dict[str, SpatialElement]
    ) -> Tuple[float, float]:
        ...
        pred_x, pred_y = self._pick_safe_point_in_area(
            vertex_area, layout, (pred_x, pred_y)
        )
    ...
    def _clamp_bounds_into_area(
        self, bounds: SpatialBounds, area_id: str
    ) -> SpatialBounds:
        ...
        new_x = max(
            area_bounds.x + pad,
            min(bounds.x, area_bounds.x + area_bounds.width - pad - bounds.width),
        )
        new_y = max(
            area_bounds.y + pad,
            min(bounds.y, area_bounds.y + area_bounds.height - pad - bounds.height),
        )
        return SpatialBounds(new_x, new_y, bounds.width, bounds.height)

    def _pick_safe_point_in_area(
        self,
        area_id: str,
        layout: Dict[str, SpatialElement],
        ...
        for c in self.egi.Cut:
            if (
                is_descendant_cut(c.id, area_id)
                or self._determine_cut_parent_area(c.id) == area_id
            ):
                b = self.correspondence.area_mappings.get(c.id)
                if b:
                    holes.append(b)
        ...
        if holes:
            ...
            if in_any:
                ...
                if area_bounds:
                    cand = (
                        max(
                            area_bounds.x,
                            min(cand[0], area_bounds.x + area_bounds.width),
                        ),
                        max(
                            area_bounds.y,
                            min(cand[1], area_bounds.y + area_bounds.height),
                        ),
                    )
                x, y = cand
            ...
            child_ids = [
                c.id
                for c in self.egi.Cut
                if c.id in self.egi.area.get(area_id, frozenset())
            ]
    ...
    def _avoid_child_cut_holes(
        self, bounds: SpatialBounds, area_id: str, layout: Dict[str, SpatialElement]
    ) -> SpatialBounds:
        ...
        for hole in holes:
            ...
            while overlaps(adjusted, hole) and guard < 50:
                ...
                dx_left = (
                    (hole.x - (adjusted.x + adjusted.width))
                    if adjusted.x + adjusted.width > hole.x
                    else 0
                )
                dx_right = (
                    ((hole.x + hole.width) - adjusted.x)
                    if adjusted.x < hole.x + hole.width
                    else 0
                )
                dy_up = (
                    (hole.y - (adjusted.y + adjusted.height))
                    if adjusted.y + adjusted.height > hole.y
                    else 0
                )
                dy_down = (
                    ((hole.y + hole.height) - adjusted.y)
                    if adjusted.y < hole.y + hole.height
                    else 0
                )
                ...
                candidates = []
                if dx_left < 0:
                    candidates.append(("left", abs(dx_left)))
                if dx_right > 0:
                    candidates.append(("right", abs(dx_right)))
                if dy_up < 0:
                    candidates.append(("up", abs(dy_up)))
                if dy_down > 0:
                    candidates.append(("down", abs(dy_down)))
                ...
                step = 8.0
                if direction == "left":
                    adjusted = SpatialBounds(
                        adjusted.x - step, adjusted.y, adjusted.width, adjusted.height
                    )
                    ...
                    adjusted = SpatialBounds(
                        adjusted.x, adjusted.y - step, adjusted.width, adjusted.height
                    )
                else:
                    adjusted = SpatialBounds(
                        adjusted.x, adjusted.y + step, adjusted.width, adjusted.height
    ...
    def _assert_path_within_area(
        self,
        path: List[Tuple[float, float]],
        area_id: str,
        *,
        allowed_child: Optional[str] = None,
        allow_last_inside_child: bool = False,
    ) -> None:
        ...
        def point_in_rect_strict(px: float, py: float, r: SpatialBounds) -> bool:
            return (r.x + eps < px < r.x + r.width - eps) and (
                r.y + eps < py < r.y + r.height - eps
            )

        def point_in_rect_with_border(px: float, py: float, r: SpatialBounds) -> bool:
            return (r.x - eps_area <= px <= r.x + r.width + eps_area) and (
                r.y - eps_area <= py <= r.y + r.height + eps_area
            )

        # Check each waypoint
        for idx, (px, py) in enumerate(path):
            # Must be inside area bounds if area is bounded
            if area_bounds is not None and not point_in_rect_with_border(
                px, py, area_bounds
            ):
                ...
                if allow_last_inside_child and allowed_child is not None:
                    for cid, r in child_rects:
                        if cid == allowed_child and point_in_rect_with_border(
                            px, py, r
                        ):
                            break
                    else:
            ...
            for cid, r in child_rects:
                ...
                if (
                    allow_last_inside_child
                    and allowed_child is not None
                    and cid == allowed_child
                ):
                    continue
    ...
    def _rearrange_ligature(
        self, ligature_id: str, new_geometry: LigatureGeometry
    ) -> bool:
...
def create_spatial_correspondence_engine(
