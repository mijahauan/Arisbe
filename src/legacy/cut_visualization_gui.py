...
from egi_core_dau import Cut, ElementID, RelationalGraphWithCuts
...
class CutVisualizationCanvas(tk.Canvas):
    ...
    def __init__(self, parent, width=800, height=600, **kwargs):
        super().__init__(
            parent, width=width, height=height, bg=VisualStyle.sheet_color, **kwargs
        )
    ...
    def setup_test_cuts(self):
        ...
        for cut_id, bounds in cut_bounds.items():
            success = self.cut_tracker.add_cut(
                egi, cut_id, bounds, CutPlacementType.BESIDE
            )
            if not success:
...
Click cuts to select and drag to move (with constraint validation)
        ...
        h_scrollbar = ttk.Scrollbar(
            main_frame, orient=tk.HORIZONTAL, command=self.canvas.xview
        )
        ...
        ttk.Button(
            button_frame, text="Load Test Cuts", command=self.load_test_cuts
        ).pack(side=tk.LEFT, padx=(0, 10))

        ttk.Button(
            button_frame, text="Validate Constraints", command=self.validate_constraints
        ).pack(side=tk.LEFT, padx=(0, 10))

        ttk.Button(
            button_frame, text="Show R-tree Info", command=self.show_rtree_info
        ).pack(side=tk.LEFT, padx=(0, 10))

        ttk.Button(button_frame, text="Reset View", command=self.reset_view).pack(
            side=tk.LEFT
        )
    ...
    def validate_constraints(self):
        ...
        overlapping_pairs = (
            self.canvas.cut_tracker.validate_non_overlapping_constraint()
        )

        if overlapping_pairs:
            overlap_text = "\n".join(
                [f"{c1} overlaps {c2}" for c1, c2 in overlapping_pairs]
            )
            messagebox.showwarning(
                "Constraint Violations", f"Overlapping cuts found:\n{overlap_text}"
            )
