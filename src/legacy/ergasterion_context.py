...
from immutable_transformation_architecture import (
    ImmutableEGIRepository, TransformationSequence, ContextType, TransformationRuleType
)
...
class ErgasterionWorkspace:
    ...
    def create_experiment(self, title: str, description: str, 
                         hypothesis_type: HypothesisType,
                         initial_egi: Optional[RelationalGraphWithCuts] = None) -> str:
    ...
    def apply_transformation_to_experiment(self, experiment_id: str,
        ...
        # Update sequence
        sequence = self.repository.get_transformation_sequence(experiment.transformation_sequence_id)
        if sequence:
    ...
    def create_workspace_snapshot(self, description: str = "") -> str:
        ...
        snapshot = WorkspaceSnapshot(
            ...
            timestamp=datetime.now(),
            active_experiments=[exp_id for exp_id, exp in self.experiments.items() 
    ...
    def analyze_experiment_patterns(self) -> Dict[str, Any]:
        ...
        for sequence in self.repository.transformation_sequences.values():
            for step in sequence.steps:
                ...
                transformation_counts[rule_type] = transformation_counts.get(rule_type, 0) + 1
        ...
        return {
            ...
            "average_observations_per_experiment": sum(len(exp.observations) for exp in self.experiments.values()) / len(self.experiments),
            "total_egis_created": self.total_egis_created,
            "total_transformations": self.total_transformations
        }
...
class ErgasterionManager:
    ...
    def analyze_cross_workspace_patterns(self) -> Dict[str, Any]:
        ...
        total_transformations = sum(ws.total_transformations for ws in self.workspaces.values())
        ...
        return {
            ...
            "average_experiments_per_workspace": total_experiments / len(self.workspaces),
            "most_active_workspace": max(self.workspaces.values(), 
                                       key=lambda ws: ws.total_transformations).workspace_id
        }
...
def demonstrate_ergasterion():
    ...
    workspace.apply_transformation_to_experiment(
        ...
        transformation_data={"element_type": "vertex", "element_id": "v1", "target_area": "sheet"},
    ...
    workspace.apply_transformation_to_experiment(
        ...
        rule_type=TransformationRuleType.INSERTION,
        transformation_data={"element_type": "vertex", "element_id": "v3", "target_area": "sheet"},
    ...
    workspace.apply_transformation_to_experiment(
        ...
        rule_type=TransformationRuleType.INSERTION,
        transformation_data={
            "element_type": "cut", "element_id": "c1", "target_area": "sheet",
