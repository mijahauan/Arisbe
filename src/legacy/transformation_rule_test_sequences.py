...
from formal_transformation_rules import (
    FormalTransformationEngine, TransformationContext, TransformationResult,
    AreaPolarity
)
...
class TransformationRuleTestSuite:
    ...
    def _create_simple_egi(self, vertices: List[str], edges: List[Tuple[str, List[str], str]] = None) -> RelationalGraphWithCuts:
        ...
        if edges:
            for edge_id, vertex_sequence, relation_name in edges:
                ...
                nu_mapping[ElementID(edge_id)] = tuple(ElementID(v) for v in vertex_sequence)
                rel_mapping[ElementID(edge_id)] = relation_name
        
        all_elements = [ElementID(v) for v in vertices] + [ElementID(e[0]) for e in (edges or [])]
    ...
    def _create_dc_plus_tests(self):
        ...
        self.test_sequences.append(TestSequence(
        ...
        self.test_sequences.append(TestSequence(
            ...
            test_steps=[
                {
                    "target_area": ElementID("sheet"),
                    "selected_subgraph": frozenset([ElementID("A"), ElementID("R")]),
                    "description": "Enclose vertex A and edge R in double cut"
                }
            ],
            expected_outcomes=[
                {"vertices": 2, "edges": 1, "cuts": 2}
            ]
        ))
    ...
    def _create_dc_minus_tests(self):
        ...
        double_cut_egi = RelationalGraphWithCuts(
            ...
            area=frozendict({
                ElementID("sheet"): frozenset([ElementID("P"), ElementID("outer_cut")]),
    ...
    def _create_insertion_tests(self):
        ...
        self.test_sequences.append(TestSequence(
            ...
            test_steps=[
                {
                    "target_area": ElementID("sheet"),
                    "selected_subgraph": frozenset([ElementID("Z")]),  # Simple vertex ID
    ...
    def _create_iteration_tests(self):
        ...
        self.test_sequences.append(TestSequence(
            ...
            test_steps=[
                {
                    "target_area": ElementID("sheet"),
                    "selected_subgraph": frozenset([ElementID("A"), ElementID("loves")]),
                    "description": "Iterate vertex A and edge loves"
                }
            ],
            expected_outcomes=[
                {"vertices": 3, "edges": 2, "cuts": 0}
            ]
        ))
    ...
    def execute_test_sequence(self, sequence: TestSequence) -> TestResult:
        ...
        for i, step in enumerate(sequence.test_steps):
            try:
                ...
                step_results.append(TransformationResult(
                    success=False,
                    result_egi=None,
                    error_message=str(e),
                    changes_made={}
        ...
        if sequence.expected_outcomes:
            ...
            if "success" in expected and not expected["success"]:
                ...
                if "error_contains" in expected:
                    ...
                    overall_success = overall_success and expected["error_contains"] in error_text
            else:
                # Check EGI structure
                if current_egi:
                    if "vertices" in expected:
                        overall_success = overall_success and len(current_egi.V) == expected["vertices"]
                    if "edges" in expected:
                        overall_success = overall_success and len(current_egi.E) == expected["edges"]
                    if "cuts" in expected:
                        overall_success = overall_success and len(current_egi.Cut) == expected["cuts"]
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        for sequence in self.test_sequences:
            ...
            if sequence.rule_name not in rule_results:
                rule_results[sequence.rule_name] = {"passed": 0, "total": 0, "tests": []}
            ...
            if result.overall_success:
                ...
                total_passed += 1
                print(f"   ✅ PASS - {result.steps_successful}/{result.steps_executed} steps successful")
                
                if result.final_egi:
                    print(f"   Result: {len(result.final_egi.V)}V, {len(result.final_egi.E)}E, {len(result.final_egi.Cut)}C")
            else:
                print(f"   ❌ FAIL - {result.steps_successful}/{result.steps_executed} steps successful")
        ...
        for rule_name, results in rule_results.items():
            ...
            print(f"   {rule_name}: {results['passed']}/{results['total']} ({success_rate:.0f}%)")
