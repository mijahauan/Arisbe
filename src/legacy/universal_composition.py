{{ ... }}
from historical_graph_model import HistoricalGraph, HistoryEventType, HistoryEvent
{{ ... }}
...
class UniversalComposer:
    ...
    def create_historical_sheet(self, metadata: SheetMetadata, 
                              source: CompositionSource = CompositionSource.DE_NOVO) -> HistoricalGraph:
    ...
    def compose_from_source(self, sheet_id: str, source: CompositionSource, 
                          input_data: Any, author: str = "") -> CompositionResult:
        """
        ...
        try:
            ...
            import_event = HistoryEvent(
                ...
                author=author,
                metadata={"source": source.value, "input_type": type(input_data).__name__}
            )
            
            historical_graph.history.add_event(import_event)
            
            # Execute composition sequence with history tracking
            result = self._execute_sequence_with_history(historical_graph, sequence, author)
    ...
    def _execute_sequence_with_history(self, historical_graph: HistoricalGraph, 
                                     sequence: CompositionSequence, 
                                     author: str) -> CompositionResult:
        ...
        if successful_steps > 0:
            historical_graph.create_checkpoint(
                f"Completed composition: {successful_steps} steps successful",
                author
            )
    ...
    def import_from_egif(self, sheet_id: str, egif_text: str, author: str = "") -> CompositionResult:
        """Import EGIF with full history tracking."""
        return self.compose_from_source(sheet_id, CompositionSource.EGIF_IMPORT, egif_text, author)
    
    def import_from_cgif(self, sheet_id: str, cgif_text: str, author: str = "") -> CompositionResult:
        """Import CGIF with full history tracking."""
        return self.compose_from_source(sheet_id, CompositionSource.CGIF_IMPORT, cgif_text, author)
    
    def import_from_clif(self, sheet_id: str, clif_text: str, author: str = "") -> CompositionResult:
        """Import CLIF with full history tracking."""
        return self.compose_from_source(sheet_id, CompositionSource.CLIF_IMPORT, clif_text, author)
    
    def compose_from_text(self, sheet_id: str, natural_language: str, author: str = "") -> CompositionResult:
        """Compose from natural language with full history tracking."""
        return self.compose_from_source(sheet_id, CompositionSource.NATURAL_LANGUAGE, natural_language, author)
    
    def add_subgraph(self, target_sheet_id: str, source_graph: RelationalGraphWithCuts, 
                    author: str = "") -> CompositionResult:
        """Add subgraph with full history tracking."""
        return self.compose_from_source(target_sheet_id, CompositionSource.SUBGRAPH_ADDITION, source_graph, author)
    
    def compose_interactively(self, sheet_id: str, sequence: CompositionSequence, 
                            author: str = "") -> CompositionResult:
        """Interactive composition with full history tracking."""
        return self.compose_from_source(sheet_id, CompositionSource.DE_NOVO, sequence, author)
    ...
    def create_branch(self, sheet_id: str, branch_name: str, from_checkpoint: str = None) -> bool:
    ...
    def merge_graphs(self, target_sheet_id: str, source_sheet_id: str, author: str = "") -> CompositionResult:
        ...
        result = self._execute_sequence_with_history(target_graph, source_sequence, author)
