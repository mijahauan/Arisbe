...
from egi_validity_analyzer import EGIValidityAnalyzer, ValidationReport, ValidationSeverity
from egif_transformation_interface import EGIFTransformationInterface, TransformationRequest, TransformationResponse
...
class TransformationValidationPipeline:
    ...
    def validate_transformation(self, request: TransformationRequest, 
                              existing_egi: Optional[RelationalGraphWithCuts] = None,
                              transformation_id: Optional[str] = None) -> TransformationValidationResult:
        """
        ...
        if not self._should_proceed_with_transformation(pre_validation):
            return self._create_blocked_result(
                transformation_id, request, pre_validation, 
                "Pre-transformation validation failed"
            )
        
        # Step 2: Apply transformation
        try:
            transformation_response = self.transformation_interface.apply_transformation(
                request, existing_egi
            )
        except Exception as e:
            return self._create_error_result(
                transformation_id, request, f"Transformation failed: {e}",
                pre_validation
            )
        ...
        if transformation_response.success and transformation_response.result_egi:
            try:
                ...
                return self._create_error_result(
                    transformation_id, request, f"Post-transformation validation failed: {e}",
                    pre_validation, transformation_response
                )
        else:
    ...
    def _should_proceed_with_transformation(self, pre_validation: ValidationReport) -> bool:
    ...
    def _apply_validation_policy(self, pre_validation: ValidationReport,
                               post_validation: ValidationReport,
                               transformation_response: TransformationResponse) -> bool:
        ...
        if self.policy == ValidationPolicy.DISABLED:
            ...
            return (transformation_response.success and 
                   not post_validation.has_critical_issues())
        elif self.policy == ValidationPolicy.STRICT:
            return (transformation_response.success and 
                   post_validation.is_valid and
                   not self._has_new_errors(pre_validation, post_validation))
        
        return True
    
    def _has_new_errors(self, pre_validation: ValidationReport, 
                       post_validation: ValidationReport) -> bool:
        """Check if new errors were introduced."""
        
        pre_errors = len(pre_validation.get_issues_by_severity(ValidationSeverity.ERROR))
        pre_critical = len(pre_validation.get_issues_by_severity(ValidationSeverity.CRITICAL))
        
        post_errors = len(post_validation.get_issues_by_severity(ValidationSeverity.ERROR))
        post_critical = len(post_validation.get_issues_by_severity(ValidationSeverity.CRITICAL))
        
        return (post_errors > pre_errors) or (post_critical > pre_critical)
    
    def _analyze_validation_changes(self, pre_validation: ValidationReport,
                                  post_validation: ValidationReport) -> Tuple[List[str], List[str]]:
        ...
        post_issue_descriptions = {issue.description for issue in post_validation.issues}
    ...
    def _create_error_result(self, transformation_id: str, request: TransformationRequest,
                           error_message: str, pre_validation: Optional[ValidationReport] = None,
                           transformation_response: Optional[TransformationResponse] = None) -> TransformationValidationResult:
    ...
    def _create_blocked_result(self, transformation_id: str, request: TransformationRequest,
                             pre_validation: ValidationReport, reason: str) -> TransformationValidationResult:
    ...
    def get_validation_summary(self) -> Dict[str, Any]:
        ...
        validation_passed = sum(1 for r in self.validation_history if r.validation_passed)
        
        # Count issues by type
        total_issues_introduced = sum(len(r.issues_introduced) for r in self.validation_history)
        total_issues_resolved = sum(len(r.issues_resolved) for r in self.validation_history)
