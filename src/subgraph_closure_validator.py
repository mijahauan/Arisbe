...
class SubgraphClosureValidator:
    ...
    2. All vertices in subgraph that have edges must have those edges in subgraph
    ...
    def analyze_closure(self, selection: FrozenSet[ElementID],
                       allow_expansion: bool = True) -> ClosureAnalysis:
        """
        ...
        for iteration in range(max_iterations):
            ...
            for elem_id in list(current_subgraph):
                if elem_id in self._edge_ids:
                    ...
                    edge_violations = self._check_edge_closure(elem_id, current_subgraph)
                    if edge_violations:
                    ...
                    vertex_violations = self._check_vertex_closure(elem_id, current_subgraph)
        ...
        # Final validation
        is_closed = self._is_truly_closed(frozenset(current_subgraph))
    ...
    def _is_truly_closed(self, subgraph: FrozenSet[ElementID]) -> bool:
        ...
        for vertex_id in subgraph_vertices:
            for edge in self.egi.E:
                ...
                if vertex_id in vertex_sequence:
                    ...
                    all_vertices_in = all(v in subgraph for v in vertex_sequence)
    ...
    def validate_for_transformation(self, selection: FrozenSet[ElementID],
                                   rule_name: str) -> Tuple[bool, str, FrozenSet[ElementID]]:
        """
