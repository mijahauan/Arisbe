...
class ResizeHandle(QGraphicsRectItem):
    """Resize handle for cuts that actually resizes the parent."""
    
    def __init__(self, x, y, w, h, parent_cut):
    ...
    def itemChange(self, change, value):
        """Handle resize handle movement to resize parent cut."""
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionHasChanged and self.parent_cut:
            # Get handle position in parent's coordinate system
            handle_pos = self.pos()
            parent_rect = self.parent_cut.rect()
            
            # Calculate new cut size based on handle position
            # Handle is positioned at bottom-right corner
            new_width = max(50, handle_pos.x() + self.rect().width()/2)
            new_height = max(50, handle_pos.y() + self.rect().height()/2)
            
            # Update parent cut rectangle, keeping top-left corner fixed
            new_rect = QRectF(parent_rect.x(), parent_rect.y(), new_width, new_height)
...
class StyledCutItem(QGraphicsRectItem):
    ...
    def __init__(self, rect: QRectF, cut_id: str, style_manager: StyleManager, interactive: bool = True, validation_mode: str = "composition", nesting_depth: int = 0):
        ...
        # Apply basic styling (no apply_cut_style method exists)
        ...
        print(f"DEBUG: Created StyledCutItem {cut_id} with interactive={interactive}, validation_mode={validation_mode}, depth={nesting_depth}")
        
        # No permanent resize handles - use hover cursor instead
        self.resize_handles = []
    
    def hoverEnterEvent(self, event):
        """Show resize cursor when hovering over bottom-right corner."""
        print(f"DEBUG: Cut {self.cut_id} hoverEnterEvent - interactive={self.interactive}")
        if not self.interactive:
            return super().hoverEnterEvent(event)
            
        # Check if hovering near bottom-right corner for resize
        pos = event.pos()
        rect = self.rect()
        corner_size = 20  # Resize area size
        
        if (pos.x() > rect.right() - corner_size and 
            pos.y() > rect.bottom() - corner_size):
    ...
    def itemChange(self, change, value):
        """Validate cut position and handle position changes."""
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionChange:
            # Validate position before allowing move
            new_pos = value
            if not self._validate_position(new_pos):
            ...
            # Position changed successfully - move contained elements with the cut
