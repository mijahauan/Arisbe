...
class CutAreaCorrespondence:
    ...
    def _layout_cut_in_canvas(self, cut_id: str, egi: RelationalGraphWithCuts) -> SpatialBounds:
        ...
        available_width = self.canvas.bounds.width - (2 * margin) - (padding * (cols - 1))
        available_height = self.canvas.bounds.height - (2 * margin) - (padding * (rows - 1))
    ...
    def _layout_cut_in_parent(self, cut_id: str, parent_id: str, egi: RelationalGraphWithCuts) -> SpatialBounds:
    ...
    def get_correspondence_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "root_cuts": len([c for c in self.cut_areas.values() if c.logical_parent == "sheet"]),
            "max_nesting_depth": max([c.nesting_depth for c in self.cut_areas.values()], default=0),
                ...
                for child2_id in children[i+1:]:
                    if child1_id in self.cut_areas and child2_id in self.cut_areas:
                        ...
                        if bounds1.overlaps(bounds2):
                            raise ValueError(f"Sibling cuts {child1_id} and {child2_id} overlap")
    ...
    def _validate_spatial_containment(self) -> None:
        ...
        for cut_id, cut_area in self.cut_areas.items():
            if cut_area.logical_parent == "sheet":
                ...
                if not parent_cut.spatial_bounds.contains_bounds(cut_area.spatial_bounds):
                    raise ValueError(f"Cut {cut_id} not contained in parent {cut_area.logical_parent}")
