...
class Valuation:
    ...
    def is_partial_for_context(
        ...
        vertices_at_or_below_context = self._get_vertices_at_or_below_context(
            egi, context
        )

        return vertices_above_context.issubset(self.domain) and self.domain.isdisjoint(
            vertices_at_or_below_context
        )
    ...
    def extend(self, extensions: Dict[ElementID, Any]) -> "Valuation":
        ...
        return Valuation(
            mapping=frozendict(new_mapping), domain=frozenset(new_mapping.keys())
        )

    def restrict_to(self, vertex_set: Set[ElementID]) -> "Valuation":
        """Restrict valuation to given vertex set."""
        restricted_mapping = {
            v: obj for v, obj in self.mapping.items() if v in vertex_set
        }
        return Valuation(
    ...
    def _get_vertices_above_context(
        self, egi: RelationalGraphWithCuts, context: ElementID
    ) -> Set[ElementID]:
    ...
    def _get_vertices_at_or_below_context(
        self, egi: RelationalGraphWithCuts, context: ElementID
    ) -> Set[ElementID]:
...
class SemanticEvaluationEngine:
    ...
    def evaluate_endoporeutic(
        self, egi: RelationalGraphWithCuts, model: RelationalStructure
    ) -> EvaluationResult:
        """
        ...
        try:
            ...
            empty_valuation = Valuation(mapping=frozendict(), domain=frozenset())
            sheet_context = ElementID("sheet")
    ...
    def _evaluate_classical_context(
        self,
        egi: RelationalGraphWithCuts,
        model: RelationalStructure,
        context: ElementID,
    ...
    def _evaluate_endoporeutic_context(
        self,
        egi: RelationalGraphWithCuts,
        model: RelationalStructure,
        context: ElementID,
    ...
    def _get_context_vertices(
        self, egi: RelationalGraphWithCuts, context: ElementID
    ) -> Set[ElementID]:
        """Get vertices directly contained in the given context."""
        area_contents = egi.area.get(context, frozenset())
        return {
            element_id for element_id in area_contents if element_id in egi._vertex_map
        }

    def _get_context_edges(
        self, egi: RelationalGraphWithCuts, context: ElementID
    ) -> Set[ElementID]:
        """Get edges directly contained in the given context."""
        area_contents = egi.area.get(context, frozenset())
        return {
            element_id for element_id in area_contents if element_id in egi._edge_map
        }

    def _get_context_cuts(
        self, egi: RelationalGraphWithCuts, context: ElementID
    ) -> Set[ElementID]:
        """Get cuts directly contained in the given context."""
        area_contents = egi.area.get(context, frozenset())
        return {
            element_id for element_id in area_contents if element_id in egi._cut_map
        }
    ...
    def _get_incident_vertices(
        self, egi: RelationalGraphWithCuts, edge: Edge
    ) -> List[ElementID]:
        """Get vertices incident to the given edge using ν mapping."""
        return list(egi.nu.get(edge.id, tuple()))

    def verify_evaluation_equivalence(
        self,
        egi: RelationalGraphWithCuts,
        model: RelationalStructure,
        valuation: Valuation,
...
def create_simple_relational_structure(universe_size: int = 3) -> RelationalStructure:
    ...
    return RelationalStructure(
        universe=universe, interpretation=frozendict(interpretation)
    )


def create_total_valuation(
    egi: RelationalGraphWithCuts, model: RelationalStructure
) -> Valuation:
    ...
    return Valuation(mapping=frozendict(mapping), domain=frozenset(vertex_ids))
