...
class EGIFTransformationInterface:
    ...
    def load_corpus_egif(self, corpus_path: str) -> Tuple[str, Dict[str, Any]]:
        ...
        egif_content = (
            corpus_data.get("linear_forms", {}).get("egif", {}).get("content")
        )
        if not egif_content:
    ...
    def identify_target_area(
        self, egi: RelationalGraphWithCuts, area_description: str
    ) -> ElementID:
        """
        ...
        if area_description == "second_negation" or area_description == "inner_cut":
            ...
            for cut in egi.Cut:
                ...
                has_nested_cuts = any(
                    other_cut.id in cut_contents
                    for other_cut in egi.Cut
                    if other_cut.id != cut.id
                )
    ...
    def _calculate_area_polarity(
        self, egi: RelationalGraphWithCuts, area_id: str
    ) -> Tuple[str, int]:
    ...
    def create_insertion_subgraph(self, relation_spec: str) -> FrozenSet[ElementID]:
        ...
        try:
            ...
            raise ValueError(
                f"Could not parse insertion specification '{relation_spec}': {e}"
            )

    def apply_transformation(
        self,
        request: TransformationRequest,
        existing_egi: RelationalGraphWithCuts = None,
        ...
        try:
            ...
            target_area = self.identify_target_area(
                original_egi, request.target_area_description
            )
            ...
            if request.rule_name == "INS":
                ...
                if not insertion_spec:
                    raise ValueError(
                        "INS operation requires 'insert_content' specification"
                    )
                ...
                relation_match = re.match(
                    r"\(\s*(\w+)\s+(.*?)\s*\)", insertion_spec.strip()
                )
                            ...
                            for i, arg in enumerate(orig_arg_tokens):
                                if not (
                                    ...
                                    if i < len(vertex_sequence):
                                        original_var_to_vertex[var_name] = (
                                            vertex_sequence[i]
                                        )
                            break
                ...
                for arg in arg_tokens:
                    if arg.startswith('"') and arg.endswith('"'):
                        ...
                        for vertex in original_egi.V:
                            ...
                            if (
                                not vertex_obj.is_generic
                                and vertex_obj.label == constant_name
                            ):
                        ...
                        if var_name in original_var_to_vertex:
                            mapped_vertex_sequence.append(
                                original_var_to_vertex[var_name]
                            )
                        else:
                            raise ValueError(
                                f"Variable {var_name} not found in original graph"
                            )
                ...
                polarity, depth = self._calculate_area_polarity(
                    original_egi, target_area
                )
                area_polarity = (
                    AreaPolarity.POSITIVE if depth % 2 == 0 else AreaPolarity.NEGATIVE
                )
                ...
                selected_elements = request.operation_details.get(
                    "selected_elements", []
                )
                selected_subgraph = frozenset(
                    ElementID(elem) for elem in selected_elements
                )

                result = self.engine.apply_rule(
                    request.rule_name, original_egi, target_area, selected_subgraph
                )
                ...
                if destination_area:
                    ...
                    destination_area_id = (
                        ElementID(destination_area)
                        if isinstance(destination_area, str)
                        else destination_area
                    )
                    result = self.engine.apply_rule(
                        request.rule_name,
                        original_egi,
            ...
            if request.rule_name == "IT-":
                ...
                print(
                    f"DEBUG: IT- transformation result.result_egi = {result.result_egi}"
                )
                if result.result_egi is None:
                    print(
                        f"DEBUG: IT- transformation error_message = {result.error_message}"
                    )

            # Check if transformation succeeded and has result EGI
            if not result.success or result.result_egi is None:
                raise ValueError(
                    f"Transformation {request.rule_name} failed: {result.error_message if result else 'Unknown error'}"
                )
    ...
    def demonstrate_transformation(
        ...
        if response.success:
            ...
            if response.original_egi and response.result_egi:
                print(f"\n📊 EGI Changes:")
                print(
                    f"   Vertices: {len(response.original_egi.V)} → {len(response.result_egi.V)}"
                )
                print(
                    f"   Edges: {len(response.original_egi.E)} → {len(response.result_egi.E)}"
                )
                print(
                    f"   Cuts: {len(response.original_egi.Cut)} → {len(response.result_egi.Cut)}"
                )

                if (
                    response.transformation_details
                    and response.transformation_details.changes_made
                ):
