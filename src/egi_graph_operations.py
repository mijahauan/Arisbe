...
from egi_logical_areas import EGIGraph, EGILogicalSystem, LogicalArea
from egi_spatial_correspondence import (
    SpatialBounds,
)
from egi_spatial_correspondence import (
    SpatialCorrespondenceEngine as EGISpatialCorrespondence,
)
...
class EGIGraphOperations:
    ...
    def add_predicate_graph_to_area(
        self, edge_id: str, relation_name: str, target_area_id: str
    ) -> bool:
        """
    ...
    def bind_vertex_to_predicate(
        self, vertex_id: str, edge_id: str, target_area_id: str
    ) -> bool:
        """
        ...
        if not predicate_graph or not vertex_graph:
            if os.environ.get("ARISBE_DEBUG_EGI") == "1":
                print(
                    f"DEBUG: Binding failed - predicate_graph: {predicate_graph is not None}, vertex_graph: {vertex_graph is not None}"
                )
            return False

        # Update predicate graph's nu mapping to include the vertex
        predicate_graph.nu_mapping[edge_id] = predicate_graph.nu_mapping.get(
            edge_id, set()
        )
        ...
        if os.environ.get("ARISBE_DEBUG_EGI") == "1":
            print(
                f"DEBUG: Bound {vertex_id} to {edge_id}, nu_mapping: {predicate_graph.nu_mapping}"
            )
        return True

    def conjoin_graphs_in_area(
        self, graph1_id: str, graph2_id: str, target_area_id: str
    ) -> Optional[str]:
        """
        ...
        target_area.contained_graphs = [
            g
            for g in target_area.contained_graphs
    ...
    def move_graph_between_areas(
        self, graph_id: str, source_area_id: str, target_area_id: str
    ) -> bool:
        """
        ...
        """
        if (
            source_area_id not in self.logical_system.logical_areas
            or target_area_id not in self.logical_system.logical_areas
        ):
            return False
...
class EGISystemController:
    ...
    def add_predicate_to_area(
        self, edge_id: str, relation_name: str, area_id: str
    ) -> bool:
        """Step 1: Add a predicate graph to a logical area (without vertices)."""
        success = self.graph_operations.add_predicate_graph_to_area(
            edge_id, relation_name, area_id
        )
        if success:
            self._update_presentation()
        return success
    ...
    def bind_vertex_to_predicate(
        self, vertex_id: str, edge_id: str, area_id: str
    ) -> bool:
        """Step 3: Bind vertex to predicate with ligature and update nu mapping."""
        success = self.graph_operations.bind_vertex_to_predicate(
            vertex_id, edge_id, area_id
        )
        if success:
            self._update_presentation()
        return success
    ...
    def conjoin_graphs(
        self, graph1_id: str, graph2_id: str, area_id: str
    ) -> Optional[str]:
        """Conjoin two graphs in the same area."""
        result = self.graph_operations.conjoin_graphs_in_area(
            graph1_id, graph2_id, area_id
        )
        if result:
            self._update_presentation()
        return result
