...
class NaryIdentityRelation:
    ...
    def __init__(self, edge_id: ElementID, vertices: List[ElementID], arity: int):
        ...
        if len(vertices) != arity:
            raise ValueError(
                f"Vertex count {len(vertices)} does not match arity {arity}"
            )
    ...
    def apply_transformation(
        ...
        try:
            ...
            identity_edge_id = ElementID(
                f"nary_id_{arity}_{'_'.join(str(v) for v in vertices)}"
            )
            ...
            if vertex_context in new_area_mapping:
                ...
                new_area_mapping[vertex_context] = current_contents | frozenset(
                    [identity_edge_id]
                )
            else:
                # Context is sheet
...
class SeparateNaryIdentityRule(FormalTransformationRule):
    ...
    def check_preconditions(
        ...
        if len(context.selected_subgraph) != 1:
            return (
                False,
                "Must select exactly one vertex to separate from n-ary identity",
            )
        ...
        separable_relations = [
            rel for rel in nary_relations if rel.can_separate_vertex(vertex_id)
        ]
        if not separable_relations:
            return (
                False,
                "Vertex cannot be separated from any n-ary identity (all are binary)",
            )
    ...
    def apply_transformation(
        ...
        try:
            ...
            nary_relations = self._find_nary_identities_containing_vertex(
                egi, vertex_id
            )
            ...
            if not target_relation:
                return TransformationResult(
                    False, None, "No separable n-ary identity found", {}
                )
    ...
    def _apply_nary_separation(
        self,
        egi: RelationalGraphWithCuts,
        relation: NaryIdentityRelation,
        vertex_to_separate: ElementID,
    ...
    def _find_nary_identities_containing_vertex(
        self, egi: RelationalGraphWithCuts, vertex_id: ElementID
    ) -> List[NaryIdentityRelation]:
                ...
                if vertex_id in vertex_sequence:
                    ...
                    relations.append(
                        NaryIdentityRelation(
                            edge_id=edge_id, vertices=list(vertex_sequence), arity=arity
                        )
                    )
...
class NaryIdentityAnalyzer:
    ...
    def get_identities_containing_vertex(
        self, vertex_id: ElementID
    ) -> List[NaryIdentityRelation]:
        """Get all identity relations containing the given vertex."""
        return [
            rel
            for rel in self.get_all_nary_identities()
            if rel.contains_vertex(vertex_id)
        ]
    ...
    def get_identity_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "nary_count": sum(
                count for arity, count in arity_counts.items() if arity > 2
            ),
        }

    def can_create_nary_identity(
        self, vertices: List[ElementID]
    ) -> Tuple[bool, Optional[str]]:
...
def demonstrate_nary_identity_relations():
    ...
    test_egi = RelationalGraphWithCuts(
        ...
        E=frozenset([edge_r]),
        nu=frozendict({ElementID("R"): (ElementID("A"), ElementID("B"))}),
        ...
        rel=frozendict({ElementID("R"): "Relation1"}),
    )
