...
class TestCase:
    ...
    def validate_result(self, final_egi: RelationalGraphWithCuts) -> Dict[str, bool]:
        ...
        results["vertex_count"] = len(final_egi.V) == self.expected_outcome.get("vertices", 0)
        results["edge_count"] = len(final_egi.E) == self.expected_outcome.get("edges", 0)
        results["cut_count"] = len(final_egi.Cut) == self.expected_outcome.get("cuts", 0)
...
class GraphUtteranceTestSuite:
    ...
    def _create_relational_test_cases(self):
        ...
        self.test_cases["negated_relation"] = TestCase(
            ...
            validation_checks=["vertex_count", "edge_count", "cut_count", "total_elements"]
        )
        ...
        for test_id, result in summary["results"].items():
            if result["status"] != "PASSED":
                ...
                if "validation_results" in result:
                    failed_checks = [k for k, v in result["validation_results"].items() if not v]
