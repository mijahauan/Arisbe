...
class DauDiagramCorrespondence:
    ...
    def _validate_nary_relation_constraint(self, diagram: DiagramRepresentation) -> None:
        """
        ...
        for relation_id, relation_sign in diagram.relation_signs.items():
            ...
            attached_lines = [
                line for line in diagram.edge_lines.values()
    ...
    def _validate_dominating_nodes_constraint(self, diagram: DiagramRepresentation) -> None:
        """
        ...
        for edge_line in diagram.edge_lines.values():
            ...
            if vertex_spot.containing_cut is not None:
                if not self._is_dominated_by(
                    relation_sign.containing_cut, 
                    vertex_spot.containing_cut,
                    diagram
                ):
                    raise ConstraintViolation(
    ...
    def _is_dominated_by(self, 
                        relation_cut: Optional[str], 
                        vertex_cut: Optional[str], 
                        diagram: DiagramRepresentation) -> bool:
        """
    ...
    def _cut_contains_cut(self, 
        ...
        while current_cut is not None and current_cut not in visited:
            ...
            current_cut = containing_area if containing_area in diagram.cut_lines else None
    ...
    def diagram_to_egi(self, diagram: DiagramRepresentation) -> RelationalGraphWithCuts:
        ...
        edges = frozenset(
            Edge(id=sign.element_id)
            for sign in diagram.relation_signs.values()
        )
        
        # Build cuts from cut-lines
        cuts = frozenset(
            Cut(id=cut.element_id)
            for cut in diagram.cut_lines.values()
        )
        ...
        sheet_contents = set()
        for element_type_dict in [diagram.vertex_spots, diagram.relation_signs, diagram.cut_lines]:
    ...
    def _find_containing_cut(self, element_id: str, egi: RelationalGraphWithCuts) -> Optional[str]:
    ...
    def calculate_graph_aware_layout(self, diagram: DiagramRepresentation) -> Dict[str, Tuple[float, float]]:
        """
    ...
    def _build_connection_graph(self, diagram: DiagramRepresentation) -> Dict[str, List[str]]:
    ...
    def _build_element_chain(self, diagram: DiagramRepresentation, 
                           connections: Dict[str, List[str]]) -> List[str]:
