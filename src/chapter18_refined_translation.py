...
class RefinedChapter18Translator(EnhancedChapter18Translator):
    ...
    def _selective_quantifier_reconstruction(self, base_formula: str, egi: RelationalGraphWithCuts) -> str:
        ...
        if self.original_formula_context:
            has_existential = self._formula_has_existential(self.original_formula_context)
            if has_existential:
        ...
        if len(egi.Cut) == 0 and len(egi.V) == 1:
            ...
            edge_count = sum(1 for vertex_seq in egi.nu.values() if vertex_id in vertex_seq)
            if edge_count >= 1:
                should_quantify = True
        
        if should_quantify:
            # Find variables to quantify
            variables_in_formula = self._extract_variables_from_formula_string(base_formula)
    ...
    def _formula_has_existential(self, formula: FOPLFormula) -> bool:
        ...
        if isinstance(formula, ExistentialFormula):
            ...
            return (self._formula_has_existential(formula.left) or 
                   self._formula_has_existential(formula.right))
        elif isinstance(formula, NegationFormula):
            return self._formula_has_existential(formula.formula)
        elif isinstance(formula, ImplicationFormula):
            return (self._formula_has_existential(formula.antecedent) or 
                   self._formula_has_existential(formula.consequent))
        elif isinstance(formula, UniversalFormula):
            return self._formula_has_existential(formula.formula)
    ...
    def _translate_area_to_fopl_improved(self, egi: RelationalGraphWithCuts, 
...
class EnhancedLogicalEquivalenceChecker:
    ...
    def formulas_logically_equivalent(f1_str: str, f2_str: str) -> bool:
        ...
        try:
            ...
            return EnhancedLogicalEquivalenceChecker._check_structural_equivalence(f1, f2)
        except:
            # Fallback to string-based comparison
            return EnhancedLogicalEquivalenceChecker._string_based_equivalence(f1_str, f2_str)
    ...
    def _check_structural_equivalence(f1: FOPLFormula, f2: FOPLFormula) -> bool:
        ...
        return EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(norm_f1, norm_f2)
    ...
    def _apply_var_mapping(formula: FOPLFormula, var_mapper) -> FOPLFormula:
        ...
        if isinstance(formula, AtomicFormula):
            ...
            new_left = EnhancedLogicalEquivalenceChecker._apply_var_mapping(formula.left, var_mapper)
            new_right = EnhancedLogicalEquivalenceChecker._apply_var_mapping(formula.right, var_mapper)
            return ConjunctionFormula(new_left, new_right)
        elif isinstance(formula, NegationFormula):
            new_inner = EnhancedLogicalEquivalenceChecker._apply_var_mapping(formula.formula, var_mapper)
            ...
            new_ant = EnhancedLogicalEquivalenceChecker._apply_var_mapping(formula.antecedent, var_mapper)
            new_cons = EnhancedLogicalEquivalenceChecker._apply_var_mapping(formula.consequent, var_mapper)
    ...
    def _formulas_structurally_equal(f1: FOPLFormula, f2: FOPLFormula) -> bool:
        ...
        if isinstance(f1, AtomicFormula):
            ...
            return ((EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.left, f2.left) and
                    EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.right, f2.right)) or
                   (EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.left, f2.right) and
                    EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.right, f2.left)))
        elif isinstance(f1, NegationFormula):
            return EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.formula, f2.formula)
        elif isinstance(f1, ExistentialFormula):
            return (f1.variable == f2.variable and
                   EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.formula, f2.formula))
        elif isinstance(f1, UniversalFormula):
            return (f1.variable == f2.variable and
                   EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.formula, f2.formula))
        elif isinstance(f1, ImplicationFormula):
            return (EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.antecedent, f2.antecedent) and
                   EnhancedLogicalEquivalenceChecker._formulas_structurally_equal(f1.consequent, f2.consequent))
...
def test_refined_translation():
    ...
    for i, formula_str in enumerate(test_cases, 1):
        ...
        try:
            ...
            equivalent = checker.formulas_logically_equivalent(formula_str, roundtrip_formula)
