...
class Chapter20SyntacticTranslator(EnhancedChapter18Translator):
    ...
    def psi_translate_with_universal_closure(
        self, formula: FOPLFormula
    ...
    def _translate_implication_fixed(
        self, formula: ImplicationFormula
    ) -> RelationalGraphWithCuts:
        """
    ...
    def _translate_with_proper_cut_management(
        self, formula: FOPLFormula
    ) -> RelationalGraphWithCuts:
    ...
    def _add_cut_with_conflict_resolution(
        ...
        if conflicting_cuts:
            ...
            for item in sheet_contents:
                if not any(
                    item in egi.area.get(cut_id, frozenset())
                    for cut_id in conflicting_cuts
                ):
    ...
    def _get_all_variables(self, formula: FOPLFormula) -> Set[str]:
        ...
        if isinstance(formula, AtomicFormula):
            ...
            return self._get_all_variables(formula.left) | self._get_all_variables(
                formula.right
            )
            ...
            return self._get_all_variables(
                formula.antecedent
            ) | self._get_all_variables(formula.consequent)
        return set()
    ...
    def _get_bound_variables(self, formula: FOPLFormula) -> Set[str]:
        ...
        if isinstance(formula, ExistentialFormula):
            ...
            return self._get_bound_variables(formula.left) | self._get_bound_variables(
                formula.right
            )
        elif isinstance(formula, NegationFormula):
            return self._get_bound_variables(formula.formula)
        elif isinstance(formula, ImplicationFormula):
            return self._get_bound_variables(
                formula.antecedent
            ) | self._get_bound_variables(formula.consequent)
        return set()
    ...
    def _check_exact_syntactic_identity(
        ...
        return (
            ...
            and len(egi1.nu) == len(egi2.nu)
            and len(egi1.area) == len(egi2.area)
            and len(egi1.rel) == len(egi2.rel)
        )

    def verify_syntactic_entailment_preservation(
        self, premises: List[FOPLFormula], conclusion: FOPLFormula
    ) -> bool:
        """
        ...
        try:
            ...
            return self._check_entailment_structure_preservation(
                premise_egis, conclusion_egi
            )

        except Exception:
            return False

    def _check_entailment_structure_preservation(
        self,
        premise_egis: List[RelationalGraphWithCuts],
        conclusion_egi: RelationalGraphWithCuts,
    ) -> bool:
    ...
    def verify_beta_calculus_completeness(
        ...
        for test_name, premise_str, conclusion_str in test_cases:
            try:
                ...
                completeness_preserved = self._verify_completeness_preservation(
                    premise_egi, conclusion_egi
                )
    ...
    def _verify_completeness_preservation(
        self,
        premise_egi: RelationalGraphWithCuts,
        conclusion_egi: RelationalGraphWithCuts,
    ) -> bool:
        ...
        conclusion_has_structure = (
            len(conclusion_egi.V) > 0 or len(conclusion_egi.E) > 0
        )
...
def test_chapter20_fixes():
    ...
    for formula_str in test_formulas:
        ...
        print(
            f"   {formula_str}: Free vars {free_vars} → EGI({len(egi.V)}v,{len(egi.E)}e,{len(egi.Cut)}c)"
        )

    # Test 2: Fixed implication translation
    print("\n2️⃣ Testing Fixed Implication Translation")
    implication_tests = ["Man(x) → Mortal(x)", "∃x.Man(x) → ∃y.Mortal(y)"]
    ...
    for formula_str in implication_tests:
        try:
            ...
            print(
                f"   ✅ {formula_str} → EGI({len(egi.V)}v,{len(egi.E)}e,{len(egi.Cut)}c)"
            )
    ...
    entailment_tests = [
        ...
        (
            "Conjunction",
            [parse_fopl_formula("Man(x)"), parse_fopl_formula("Mortal(x)")],
            parse_fopl_formula("Man(x) ∧ Mortal(x)"),
        ),
    ]

    for test_name, premises, conclusion in entailment_tests:
        preserved = translator.verify_syntactic_entailment_preservation(
            premises, conclusion
        )
