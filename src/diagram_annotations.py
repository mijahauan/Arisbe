...
class AnnotationGenerator:
    ...
    def generate_hook_annotations(self) -> FrozenSet[HookAnnotation]:
        ...
        for edge_id in {e.id for e in self.egi.E}:
            ...
            for edge_id_hook, position in hooks:
                ...
                annotations.add(HookAnnotation(
                    edge_id=edge_id,
                    position=position,
                    vertex_id=vertex_id,
    ...
    def generate_identity_annotations(self) -> FrozenSet[IdentityEdgeAnnotation]:
        ...
        for edge_id in self.egi.get_identity_edges():
            ...
            if len(vertex_seq) == 2:
                annotations.add(IdentityEdgeAnnotation(
                    edge_id=edge_id,
    ...
    def generate_ligature_annotations(self) -> FrozenSet[LigatureAnnotation]:
        ...
        for i, ligature in enumerate(ligatures):
            if len(ligature) > 1:  # Only annotate non-trivial ligatures
                annotations.add(LigatureAnnotation(
...
class AnnotationRenderer:
    ...
    @staticmethod
    def get_identity_display_elements(annotation: IdentityEdgeAnnotation) -> Dict[str, any]:
    ...
    def should_use_simplified_identity_display(annotations: DiagramAnnotations) -> bool:
        ...
        return annotations.mode not in [AnnotationMode.EXPLICIT_IDENTITY, AnnotationMode.BOTH]


# Utility functions for integration with existing rendering system

def create_annotations_for_egi(egi: RelationalGraphWithCuts, 
                              mode: AnnotationMode = AnnotationMode.NONE) -> DiagramAnnotations:
...
def get_branching_point_annotations(egi: RelationalGraphWithCuts) -> Dict[ElementID, int]:
...
def get_identity_edge_display_mode(annotations: DiagramAnnotations, edge_id: ElementID) -> str:
...
def get_ligature_membership(egi: RelationalGraphWithCuts, vertex_id: ElementID) -> Optional[FrozenSet[ElementID]]:
