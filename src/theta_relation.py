...
class ThetaRelationEngine:
    ...
    def compute_theta_relation(
        self, egi: RelationalGraphWithCuts, v1: ElementID, v2: ElementID
    ) -> ThetaRelationResult:
        """
        ...
        if v1 == v2:
            return ThetaRelationResult(
                ...
                paths=[
                    ThetaPath((v1,), (), (self._get_vertex_context(egi, v1),), True)
                ],
        ...
        return ThetaRelationResult(
            ...
            paths=paths,
            reason=(
                None if paths else "No valid Θ path found respecting context nesting"
            ),
        )

    def _find_theta_paths(
        self, egi: RelationalGraphWithCuts, start: ElementID, end: ElementID
    ) -> List[ThetaPath]:
        """
    ...
    ) -> Dict[ElementID, List[Tuple[ElementID, ElementID]]]:
    ...
    def _satisfies_context_nesting(
        self, egi: RelationalGraphWithCuts, path: List[ElementID]
    ) -> bool:
    ...
    def demonstrate_non_transitivity(
        self, egi: RelationalGraphWithCuts
    ) -> Dict[str, any]:
        """
            ...
            for v2 in vertices:
                for v3 in vertices:
                    if v1 != v2 != v3 != v1:
                        ...
                        if (
                            rel12.are_theta_related
                            and rel23.are_theta_related
                            and not rel13.are_theta_related
                        ):
    ...
    def get_theta_equivalence_classes(
        self, egi: RelationalGraphWithCuts
    ) -> List[Set[ElementID]]:
        """
...
def demonstrate_theta_relation():
    ...
    print(
        f"   A Θ A: {'✅' if result_aa.are_theta_related else '❌'} ({result_aa.reason})"
    )
    ...
    print(
        f"   Symmetric: {'✅' if result_ab.are_theta_related == result_ba.are_theta_related else '❌'}"
    )
    ...
    print(
        f"   A Θ C: {'✅' if result_ac.are_theta_related else '❌'} ({result_ac.reason})"
    )
    ...
    if non_trans["counter_example"]:
        print(
            f"   Counter-example found: {non_trans['v1']} Θ {non_trans['v2']} and {non_trans['v2']} Θ {non_trans['v3']} but NOT {non_trans['v1']} Θ {non_trans['v3']}"
        )
    else:
