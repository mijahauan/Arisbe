...
from formal_transformation_rules import (
    FormalTransformationRule, TransformationResult, TransformationContext
)
...
class SubgraphValidator:
    ...
    def validate_subgraph(self, egi: RelationalGraphWithCuts, 
                         selection: SubgraphSelection) -> SubgraphSelection:
        """
        ...
        for edge_id in selection.edges:
            if edge_id in egi.nu:
                ...
                if not incident_vertices.issubset(selection.vertices):
                    ...
                    selection.validation_message = f"Edge {edge_id} requires all incident vertices"
                    return selection
        ...
        for cut_id in selection.cuts:
            if cut_id in egi.area:
                ...
                if not (required_vertices.issubset(selection.vertices) and
                        required_edges.issubset(selection.edges) and
                        required_cuts.issubset(selection.cuts)):
                    selection.is_valid = False
                    selection.validation_message = f"Cut {cut_id} requires all enclosed elements"
                    return selection
    ...
    def _determine_context(self, egi: RelationalGraphWithCuts, 
                          selection: SubgraphSelection) -> Optional[ElementID]:
...
class ViewManager:
    ...
    def _expand_from_focus(self, egi: RelationalGraphWithCuts, 
                          view_spec: ViewSpecification) -> ViewResult:
    ...
    def _find_element_context(self, egi: RelationalGraphWithCuts, 
                             element_id: ElementID) -> Optional[ElementID]:
    ...
    def _generate_layout(self, egi: RelationalGraphWithCuts, 
                        view: ViewResult) -> Dict[ElementID, Tuple[float, float]]:
        ...
        for cut_id in view.visible_cuts:
            if cut_id in egi.area:
                ...
                content_positions = [positions.get(c, (0, 0)) for c in contents if c in positions]
                if content_positions:
                    # Position cut at centroid of contents
                    avg_x = sum(pos[0] for pos in content_positions) / len(content_positions)
                    avg_y = sum(pos[1] for pos in content_positions) / len(content_positions)
                    positions[cut_id] = (avg_x, avg_y)
        
        return positions
    
    def _generate_subgraph_hints(self, egi: RelationalGraphWithCuts, 
                               view: ViewResult) -> List[SubgraphSelection]:
        ...
        components = self._find_connected_components(egi, view.visible_vertices, view.visible_edges)
        for component in components:
            if len(component) > 1:  # Only suggest multi-element components
                selection = SubgraphSelection(
                    vertices=component,
                    edges={e for e in view.visible_edges if 
                          e in egi.nu and set(egi.nu[e]).issubset(component)},
    ...
    def _find_connected_components(self, egi: RelationalGraphWithCuts, 
                                 vertices: Set[ElementID], 
                                 edges: Set[ElementID]) -> List[Set[ElementID]]:
...
class UniversalEGIEngine:
    ...
    Implements EGI-first approach: all transformations operate on EGI structure,
    with display formats as synchronized views.
    """
    ...
    def apply_transformation(self, context: Chapter21TransformationContext) -> TransformationResult:
        """
        ...
        if not self.validate_subgraph_selection(context.source_egi, context.subgraph_elements):
        ...
        # Apply transformation using existing engine
        # Create a basic transformation context for now
        # This will need enhancement when integrating with full transformation system
        try:
            # For now, return a success result to test the framework
            result = TransformationResult(
                success=True,
                result_egi=context.source_egi,  # Placeholder
                error_message=None,
                changes_made={"rule": context.transformation_rule.get_rule_name() if hasattr(context.transformation_rule, 'get_rule_name') else "unknown"}
            )
    ...
    def synchronize_formats(self, egi: RelationalGraphWithCuts) -> Dict[DisplayFormat, str]:
        """
        ...
        synchronized_formats[DisplayFormat.CGIF] = "# CGIF representation (to be implemented)"
        synchronized_formats[DisplayFormat.CLIF] = "# CLIF representation (to be implemented)"
        synchronized_formats[DisplayFormat.DIAGRAM] = "# Diagram representation (handled by renderer)"
    ...
    def validate_subgraph_selection(self, egi: RelationalGraphWithCuts, 
    ...
    def _egi_to_egif(self, egi: RelationalGraphWithCuts) -> str:
        ...
        egif_parts = []
        egif_parts.append(f"// EGI with {len(egi.V)} vertices, {len(egi.E)} edges, {len(egi.Cut)} cuts")
...
def test_chapter21_engine():
    ...
    print(f"✅ View created: {len(view.visible_vertices)} vertices, {len(view.visible_edges)} edges")
    ...
    print(f"✅ Subgraph validation: {validated.is_valid} - {validated.validation_message}")
