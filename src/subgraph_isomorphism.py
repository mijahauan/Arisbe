...
class DauSubgraphExtractor:
    ...
    def extract_subgraph(self, selected_elements: FrozenSet[ElementID], context: ElementID) -> SubgraphCandidate:
        """
        ...
        # Separate by type
        vertices = frozenset(elem for elem in subgraph_elements 
                           if any(v.id == elem for v in self.egi.V))
        edges = frozenset(elem for elem in subgraph_elements 
                         if any(e.id == elem for e in self.egi.E))
        cuts = frozenset(elem for elem in subgraph_elements 
...
class EGIsomorphismChecker:
    ...
    def are_isomorphic(self, subgraph1: SubgraphCandidate, subgraph2: SubgraphCandidate) -> bool:
        """
        ...
        if (len(subgraph1.vertices) != len(subgraph2.vertices) or
            len(subgraph1.edges) != len(subgraph2.edges) or
            len(subgraph1.cuts) != len(subgraph2.cuts)):
            return False
    ...
    def _simple_structural_match(self, subgraph1: SubgraphCandidate, subgraph2: SubgraphCandidate) -> bool:
        """
...
class ITMinusValidator:
    ...
    def can_deiterate(self, selected_elements: FrozenSet[ElementID], target_area: ElementID) -> Tuple[bool, Optional[str]]:
        """
        ...
        target_subgraph = self.extractor.extract_subgraph(selected_elements, target_area)
        ...
        return False, "No isomorphic subgraph found that could have been the source of iteration"
    
    def _find_subgraph_candidates(self, search_area: ElementID, target_subgraph: SubgraphCandidate) -> List[SubgraphCandidate]:
        """
        ...
        if len(target_subgraph.edges) == 1 and len(target_subgraph.vertices) > 0:
            ...
            for element_id in area_contents:
                if element_id in self.egi.rel:
                    if (self.egi.rel[element_id] == target_relation and
                        len(self.egi.nu.get(element_id, ())) == target_arity):
