...
class VertexSplittingRule(FormalTransformationRule):
    ...
    def check_preconditions(self, context: TransformationContext) -> Tuple[bool, Optional[str]]:
        ...
        if not self._is_context_accessible_for_splitting(egi, vertex_context, target_context, hooks):
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            hooks_to_move = all_hooks[:len(all_hooks)//2] if len(all_hooks) > 1 else []
    ...
    def _apply_vertex_split(self, egi: RelationalGraphWithCuts, split_spec: VertexSplitSpec) -> RelationalGraphWithCuts:
        ...
        identity_edge_id = ElementID(f"id_{split_spec.source_vertex}_{split_spec.new_vertex_id}")
        ...
        for edge_id, position in split_spec.hooks_to_move:
            if edge_id in new_nu:
                ...
                if position < len(old_sequence) and old_sequence[position] == split_spec.source_vertex:
    ...
    def _get_vertex_hooks(self, egi: RelationalGraphWithCuts, vertex_id: ElementID) -> List[Tuple[ElementID, int]]:
    ...
    def _is_context_accessible_for_splitting(self, 
                                           egi: RelationalGraphWithCuts,
                                           vertex_context: ElementID,
                                           target_context: ElementID,
                                           hooks: List[Tuple[ElementID, int]]) -> bool:
...
class VertexMergingRule(FormalTransformationRule):
    ...
    def check_preconditions(self, context: TransformationContext) -> Tuple[bool, Optional[str]]:
        ...
        if not (any(v.id == v1_id for v in egi.V) and any(v.id == v2_id for v in egi.V)):
        ...
        if not (self._context_contains_or_equals(egi, v1_context, edge_context) and
                edge_context == v2_context):
            return False, "Context constraints not satisfied: ctx(v1) ≥ ctx(e) = ctx(v2)"
    ...
    def apply_transformation(self, context: TransformationContext) -> TransformationResult:
        ...
        try:
            ...
            # Find identity edge
            identity_edge_id = self._find_identity_edge_between_vertices(egi, v1_id, v2_id)
    ...
    def _apply_vertex_merge(self, 
                           egi: RelationalGraphWithCuts,
                           v1_id: ElementID,
                           v2_id: ElementID,
                           identity_edge_id: ElementID) -> RelationalGraphWithCuts:
    ...
    def _find_identity_edge_between_vertices(self, 
                                           egi: RelationalGraphWithCuts,
                                           v1_id: ElementID,
                                           v2_id: ElementID) -> Optional[ElementID]:
        """Find identity edge connecting two vertices."""
        for edge_id, vertex_sequence in egi.nu.items():
            if (egi.rel.get(edge_id) == "=" and
                len(vertex_sequence) == 2 and
                ((vertex_sequence[0] == v1_id and vertex_sequence[1] == v2_id) or
                 (vertex_sequence[0] == v2_id and vertex_sequence[1] == v1_id))):
                return edge_id
        return None
    ...
    def _context_contains_or_equals(self, egi: RelationalGraphWithCuts, context1: ElementID, context2: ElementID) -> bool:
...
def demonstrate_vertex_splitting_merging():
    ...
    test_egi = RelationalGraphWithCuts(
        ...
        Cut=frozenset([cut1]),
        area=frozendict({
            ElementID("sheet"): frozenset([ElementID("A"), ElementID("B"), ElementID("R"), ElementID("S"), ElementID("T"), ElementID("cut1")]),
