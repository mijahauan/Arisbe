...
class InteractiveTransformerWithHistory:
    ...
    def create_new_session(self, 
                          name: str,
                          description: str = "",
                          initial_egif_path: Optional[str] = None) -> str:
        ...
        if initial_egif_path:
            ...
            self.base_transformer.current_egi = self.base_transformer.interface.parse_egif_to_egi(egif_string)
        ...
        initial_egi = self.current_egi if initial_egif_path else self.base_transformer.current_egi
        if initial_egi is None:
            raise ValueError("No initial EGI available. Load an EGIF file first.")
    ...
    def load_session(self, session_path: str) -> str:
        ...
        self.base_transformer.analysis = self._create_analysis_from_egi(current_state.egi)
    ...
    def apply_transformation_with_history(self,
                                        rule_name: str,
                                        target_area: str,
                                        user_annotation: str = "",
                                        domain_contexts: Set[str] = None) -> Dict[str, Any]:
        ...
        result = self.base_transformer.interface.apply_transformation(
            request,
            existing_egi=self.base_transformer.current_egi
        )
        ...
        context = TransformationContext(
            ...
            selected_subgraph=frozenset(ElementID(e) for e in (selected_elements or [])),
        ...
        self.base_transformer.analysis = self._create_analysis_from_egi(result.result_egi)
        ...
        if (self.current_session.auto_save_enabled and 
            self.steps_since_save >= self.current_session.save_interval_steps):
            self._auto_save_session()
    ...
    def undo_transformation(self) -> Dict[str, Any]:
        ...
        # Get previous state
        current_step = self.current_session.history.states[self.current_session.history.current_state_id].step_number
        ...
        success = self.current_session.history.rollback_to_state(previous_state_id, create_branch=False)
        
        if success:
            # Update current EGI
            previous_state = self.current_session.history.states[previous_state_id]
            self.base_transformer.current_egi = previous_state.egi
            self.base_transformer.analysis = self._create_analysis_from_egi(previous_state.egi)
    ...
    def get_transformation_narrative(self, 
                                   from_step: Optional[int] = None,
                                   to_step: Optional[int] = None) -> str:
        ...
        return self.current_session.history.get_natural_language_narrative(from_state_id, to_state_id)
    ...
    def save_session(self, format_type: str = "json") -> str:
        ...
        if format_type == "json":
            filepath = self.persistence_manager.save_history_json(self.current_session.history)
        elif format_type == "yaml":
            filepath = self.persistence_manager.save_history_yaml(self.current_session.history)
        elif format_type == "compressed":
            filepath = self.persistence_manager.save_history_compressed(self.current_session.history)
        else:
    ...
    def get_session_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "history_state": self.history_state,
            "domain_contexts": len(self.current_session.history.domain_model_manager.domain_contexts),
            "semantic_annotations": len(self.current_session.history.domain_model_manager.semantic_annotations)
        }
    ...
    def _auto_save_session(self):
        ...
        try:
            # Save as checkpoint
            self.persistence_manager.save_incremental_checkpoint(self.current_session.history)
            self.steps_since_save = 0
            self.history_state.unsaved_changes = False
        ...
        for json_file in self.persistence_manager.json_path.glob("history_*.json"):
            try:
                ...
                sessions.append({
                    ...
                    "format": "json",
                    "history_id": data.get("history_metadata", {}).get("history_id", "unknown"),
                    "created": data.get("history_metadata", {}).get("created_timestamp", "unknown"),
