...
class IterationContext:
    ...
    ligature_connections: Dict[ElementID, Set[ElementID]]  # v -> {w: wΘv and w in target}
...
class FormalIterationEngine:
    ...
    def apply_formal_iteration(self, 
                             source_egi: RelationalGraphWithCuts,
                             subgraph_elements: FrozenSet[ElementID],
                             target_context: ElementID) -> FormalIterationResult:
        """
    ...
    def _validate_iteration_preconditions(self, 
                                        source_egi: RelationalGraphWithCuts,
                                        subgraph_elements: FrozenSet[ElementID],
                                        target_context: ElementID) -> Tuple[bool, Optional[str]]:
        ...
        subgraph_context = self._determine_subgraph_context(source_egi, subgraph_elements)
        ...
        if not self._satisfies_context_nesting_constraint(source_egi, target_context, subgraph_context):
            return False, f"Context nesting violation: {target_context} not ≤ {subgraph_context}"
        
        # Check that target context is not in subgraph cuts
        subgraph_cuts = {elem for elem in subgraph_elements 
    ...
    def _build_iteration_context(self, 
                               source_egi: RelationalGraphWithCuts,
                               subgraph_elements: FrozenSet[ElementID],
                               target_context: ElementID) -> IterationContext:
        ...
        context.W0_vertices = {
            elem for elem in subgraph_elements 
        ...
        for v in context.W0_vertices:
            ...
            for w in target_vertices:
                theta_result = self.theta_engine.compute_theta_relation(source_egi, w, v)
    ...
    def _apply_formal_transformation(self, context: IterationContext) -> Tuple[RelationalGraphWithCuts, Dict[ElementID, IndexedElement]]:
        ...
        subgraph_vertices = {elem for elem in context.subgraph_elements 
                           if any(v.id == elem for v in source_egi.V)}
        ...
        subgraph_edges = {elem for elem in context.subgraph_elements 
        ...
        # Fresh edge nu mappings: ev,w -> ((w,1), (v,2))
        # Only create mappings for fresh edges that actually exist
        for fresh_edge_id in context.fresh_edges:
        ...
        subgraph_cuts = {elem for elem in context.subgraph_elements 
                        if any(c.id == elem for c in source_egi.Cut)}
        ...
        if source_egi.sheet == context.target_context:
            ...
            subgraph_context = self._determine_subgraph_context(source_egi, context.subgraph_elements)
            if subgraph_context:
        ...
        # Fresh edge relations (all identity)
        for fresh_edge_id in context.fresh_edges:
        ...
        result_egi = RelationalGraphWithCuts(
            ...
            sheet=ElementID(f"{source_egi.sheet}×1") if source_egi.sheet in element_mapping else source_egi.sheet,
    ...
    def _determine_subgraph_context(self, 
                                  egi: RelationalGraphWithCuts, 
                                  subgraph_elements: FrozenSet[ElementID]) -> Optional[ElementID]:
    ...
    def _satisfies_context_nesting_constraint(self, 
                                            egi: RelationalGraphWithCuts,
                                            target_context: ElementID,
                                            subgraph_context: ElementID) -> bool:
...
def demonstrate_formal_iteration():
    ...
    if result.success:
        ...
        if result.iteration_context:
            ...
            print(f"   Ligature connections: {result.iteration_context.ligature_connections}")
