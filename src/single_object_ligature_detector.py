...
class SingleObjectLigatureDetector:
    ...
    def is_single_object_ligature(self, ligature_vertices: List[ElementID]) -> Tuple[bool, List[str]]:
    ...
    def separate_into_single_object_components(self, ligature_vertices: List[ElementID]) -> List[List[ElementID]]:
    ...
    def _analyze_ligature_impl(self, 
                        egi: RelationalGraphWithCuts,
                        ligature_vertices: Set[ElementID]) -> LigatureAnalysis:
        """
        ...
        context_mapping = self._build_context_mapping(egi, ligature_vertices, identity_edges)
        ...
        condition1_violations = self._check_condition_1(egi, ligature_vertices, identity_edges, context_mapping)
        violation_reasons.extend(condition1_violations)
        
        # Condition 2: No path w1 f1 w f2 w2 where w < w1 and w < w2
        condition2_violations = self._check_condition_2(egi, ligature_vertices, identity_edges, context_mapping)
    ...
    def _find_ligature_identity_edges(self, egi: RelationalGraphWithCuts, ligature_vertices: Set[ElementID]) -> Set[ElementID]:
    ...
    def _build_context_mapping(self, egi: RelationalGraphWithCuts, 
                              ligature_vertices: Set[ElementID], 
                              identity_edges: Set[ElementID]) -> Dict[ElementID, ElementID]:
    ...
    def _get_element_context(self, egi: RelationalGraphWithCuts, element_id: ElementID) -> ElementID:
    ...
    def _find_ligature_cycles(self, egi: RelationalGraphWithCuts, 
                             ligature_vertices: Set[ElementID], 
                             identity_edges: Set[ElementID]) -> List[List[ElementID]]:
    ...
    def _check_condition_1(self, egi: RelationalGraphWithCuts, 
                          ligature_vertices: Set[ElementID], 
                          identity_edges: Set[ElementID], 
                          context_mapping: Dict[ElementID, ElementID]) -> List[str]:
        ...
        for edge_id in identity_edges:
            ...
            if len(vertex_sequence) == 2:
                ...
                if (self._is_context_deeper(egi, edge_context, w1_context) and
                    self._is_context_deeper(egi, edge_context, w2_context)):
                    violations.append(f"Identity edge {edge_id} violates condition 1: edge context deeper than both vertices")
        
        return violations
    
    def _check_condition_2(self, egi: RelationalGraphWithCuts, 
                          ligature_vertices: Set[ElementID], 
                          identity_edges: Set[ElementID], 
                          context_mapping: Dict[ElementID, ElementID]) -> List[str]:
                ...
                if len(vertex_seq1) == 2 and w in vertex_seq1:
                    ...
                    for edge2_id in identity_edges:
                        ...
                        if len(vertex_seq2) == 2 and w in vertex_seq2:
                            w2 = vertex_seq2[0] if vertex_seq2[1] == w else vertex_seq2[1]
                            ...
                            if w1 != w2:  # Different endpoints
                                ...
                                if (self._is_context_deeper(egi, w_context, w1_context) and
                                    self._is_context_deeper(egi, w_context, w2_context)):
                                    violations.append(f"Path {w1}-{w}-{w2} violates condition 2: intermediate vertex context deeper")
        
        return violations
    
    def _check_condition_3(self, egi: RelationalGraphWithCuts, 
            ...
            for i in range(len(cycle)):
                for j in range(i + 1, len(cycle)):
                    ...
                    if self._is_context_deeper(egi, w2_context, w1_context):
                        violations.append(f"Cycle vertices {w1}, {w2} violate condition 3: context ordering")
        
        return violations
    
    def _is_context_deeper(self, egi: RelationalGraphWithCuts, context1: ElementID, context2: ElementID) -> bool:
    ...
    def _get_parent_context(self, egi: RelationalGraphWithCuts, context: ElementID) -> Optional[ElementID]:
