...
def _move_elements_away(
    elements: List[Dict[str, Any]], obstacle: Rect, min_distance: float = 15.0
) -> Dict[str, Any]:
...
def validate_syntactic_constraints(dto: Dict[str, Any]) -> ValidationResult:
    ...
    for i in range(len(cut_ids)):
        for j in range(i + 1, len(cut_ids)):
            ...
            if not (
                _rect_contains(b, a)
                or _rect_contains(a, b)
                or not _rect_intersects(a, b)
            ):
    ...
    return ValidationResult(
        True, "Syntactic constraints satisfied", {"bridges_needed": bridge_info}
    )
...
def validate_semantic_constraints(dto: Dict[str, Any]) -> ValidationResult:
    ...
    if violations:
        return ValidationResult(
            False, f"SEMANTIC VIOLATION: {violations[0]}", {"violations": violations}
        )

    return ValidationResult(True, "Semantic constraints satisfied")


def validate_movement_permissive(
    dto: Dict[str, Any], element_id: str, element_type: str, new_position: Any
) -> ValidationResult:
    """
...
def validate_movement_strict(
    ...
    if moved_element_bounds:
        ...
        for vid, v in trial_dto["vertices"].items():
            if vid != element_id:
                ...
                if _point_too_close_to_rect(pos, moved_element_bounds, 15.0):
                    elements_to_adjust.append({"id": vid, "type": "vertex", "pos": pos})
        ...
        for cid, c in trial_dto["cuts"].items():
            if cid != element_id:
                ...
                if _rect_too_close_to_rect(rect, moved_element_bounds, 10.0):
                    elements_to_adjust.append({"id": cid, "type": "cut", "rect": rect})

        # Calculate adjustments to move elements away
        element_adjustments = _move_elements_away(
            elements_to_adjust, moved_element_bounds
        )
        ...
        if element_type in ["vertex", "predicate"]:
            ...
            if area_id and area_id in trial_dto["cuts"]:
                parent_rect = trial_dto["cuts"][area_id].get(
                    "rect", (0.0, 0.0, 0.0, 0.0)
                )
                if not _rect_contains_point(
                    parent_rect,
                    (
                        new_position
                        if element_type == "vertex"
                        else (
                            new_position[0] + new_position[2] / 2,
                            new_position[1] + new_position[3] / 2,
                        )
                    ),
                ):
                    # Expand parent cut
                    expanded_rect = _expand_rect_to_contain(
                        parent_rect, moved_element_bounds
                    )
    ...
    if not syntactic_result.valid:
        return ValidationResult(
            False, f"Automatic adjustment failed: {syntactic_result.message}"
        )

    semantic_result = validate_semantic_constraints(trial_dto)
    if not semantic_result.valid:
        return ValidationResult(
            False, f"Automatic adjustment failed: {semantic_result.message}"
        )

    return ValidationResult(
        True, "Movement allowed with automatic adjustments", adjustments
    )


def validate_naming_change(
    mode: ConstraintMode, element_type: str, element_id: str, new_name: str
) -> ValidationResult:
    """
...
def validate_arity_change(
    mode: ConstraintMode, predicate_id: str, new_arity: int
) -> ValidationResult:
    """
...
def _line_segment_intersection(
    seg_a: Tuple[Point, Point], seg_b: Tuple[Point, Point]
) -> Optional[Point]:
