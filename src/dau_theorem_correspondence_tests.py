...
class DauTheoremCorrespondenceTests:
    ...
    def _test_theta_non_transitivity(self) -> TheoremTestResult:
        ...
        test_egis = [
            self._create_non_transitivity_test_egi()
        ]
    ...
    def _test_ligature_consistency(self) -> TheoremTestResult:
        ...
        if hasattr(consistency_result, 'is_consistent'):
            ...
            is_consistent = consistency_result[0] if isinstance(consistency_result, tuple) else True
    ...
    def _create_reflexivity_test_egi(self) -> RelationalGraphWithCuts:
        ...
        cuts = frozenset()
        area = frozendict({ElementID("sheet"): frozenset([ElementID("A"), ElementID("B"), ElementID("e1")])})
        rel = frozendict({
        ...
        return RelationalGraphWithCuts(
            V=vertices, E=edges, nu=nu, sheet=ElementID("sheet"),
            Cut=cuts, area=area, rel=rel
        )
    ...
    def _create_symmetry_test_egi(self) -> RelationalGraphWithCuts:
        ...
        cuts = frozenset()
        area = frozendict({ElementID("sheet"): frozenset([ElementID("A"), ElementID("B"), ElementID("id_AB")])})
        rel = frozendict({
    ...
    def _create_non_transitivity_test_egi(self) -> RelationalGraphWithCuts:
        ...
        edges = frozenset([
            Edge(ElementID("id_AB")), Edge(ElementID("id_BC"))
        ])
        nu = frozendict({
        ...
        area = frozendict({
            ElementID("sheet"): frozenset([ElementID("A"), ElementID("B"), ElementID("C"), 
                                         ElementID("id_AB"), ElementID("id_BC")])
        })
        rel = frozendict({
            ElementID("A"): RelationName("="),
            ElementID("B"): RelationName("="), 
    ...
    def _create_iteration_soundness_test_cases(self) -> List[Tuple[RelationalGraphWithCuts, FrozenSet[ElementID], ElementID]]:
    ...
    def _create_ligature_test_egi(self) -> RelationalGraphWithCuts:
        ...
        vertices = frozenset([
            Vertex(ElementID("A")), Vertex(ElementID("B"))
        ])
        edges = frozenset([Edge(ElementID("id_AB"))])
        nu = frozendict({ElementID("id_AB"): (ElementID("A"), ElementID("B"))})
        cuts = frozenset()
    ...
    def _compute_nesting_levels(self, egi: RelationalGraphWithCuts) -> Dict[ElementID, int]:
    ...
    def _validate_nesting_preservation(self, original: Dict[ElementID, int], result: Dict[ElementID, int]) -> bool:
