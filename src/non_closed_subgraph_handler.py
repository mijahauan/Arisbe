...
class NonClosedSubgraphHandler:
    ...
    def is_closed_subgraph(self, 
                          egi: RelationalGraphWithCuts, 
                          subgraph_elements: FrozenSet[ElementID]) -> bool:
        """
        ...
        subgraph_vertices = {elem for elem in subgraph_elements 
                           if any(v.id == elem for v in egi.V)}
        subgraph_edges = {elem for elem in subgraph_elements 
                         if any(e.id == elem for e in egi.E)}
        ...
        for edge in egi.E:
            ...
            internal_vertices = sum(1 for v_id in vertex_sequence if v_id in subgraph_vertices)
            external_vertices = sum(1 for v_id in vertex_sequence if v_id not in subgraph_vertices)
    ...
    def create_decomposition_plan(self, 
                                ...
                                target_area: ElementID,
                                strategy: DecompositionStrategy = DecompositionStrategy.EDGE_FIRST) -> DecompositionPlan:
        """
    ...
    def _create_edge_first_plan(self, 
                               egi: RelationalGraphWithCuts,
                               subgraph_elements: FrozenSet[ElementID],
                               target_area: ElementID) -> DecompositionPlan:
    ...
    def _create_vertex_first_plan(self, 
                                 egi: RelationalGraphWithCuts,
                                 subgraph_elements: FrozenSet[ElementID],
                                 target_area: ElementID) -> DecompositionPlan:
        ...
        if remaining_elements:
            edge_plan = self._create_edge_first_plan(egi, frozenset(remaining_elements), target_area)
    ...
    def _create_minimal_cuts_plan(self, 
                                 egi: RelationalGraphWithCuts,
                                 subgraph_elements: FrozenSet[ElementID],
                                 target_area: ElementID) -> DecompositionPlan:
    ...
    def _find_boundary_edges(self, 
                           egi: RelationalGraphWithCuts,
                           subgraph_elements: FrozenSet[ElementID]) -> Set[ElementID]:
        ...
        for edge in egi.E:
            ...
            # Check if edge connects subgraph vertex to external vertex
            internal_count = sum(1 for v_id in vertex_sequence if v_id in subgraph_vertices)
            external_count = sum(1 for v_id in vertex_sequence if v_id not in subgraph_vertices)
    ...
    def _find_isolated_vertices(self, 
                              egi: RelationalGraphWithCuts,
                              subgraph_elements: FrozenSet[ElementID]) -> Set[ElementID]:
    ...
    def execute_decomposition_plan(self, 
                                 egi: RelationalGraphWithCuts,
                                 plan: DecompositionPlan) -> DecompositionResult:
        """
    ...
    def erase_non_closed_subgraph(self, 
                                egi: RelationalGraphWithCuts,
                                subgraph_elements: FrozenSet[ElementID],
                                target_area: ElementID,
                                strategy: DecompositionStrategy = DecompositionStrategy.EDGE_FIRST) -> DecompositionResult:
        """
        ...
        plan = self.create_decomposition_plan(egi, subgraph_elements, target_area, strategy)
...
def demonstrate_non_closed_subgraph_handling():
    ...
    print(f"   Subgraph {[str(e) for e in non_closed_subgraph]} is closed: {'✅' if is_closed else '❌'}")
    
    if not is_closed:
        print(f"\n📋 Creating Decomposition Plan:")
        plan = handler.create_decomposition_plan(
            test_egi, non_closed_subgraph, ElementID("sheet"), 
