...
class SimpleDiagramEditor:
    ...
    def setup_ui(self):
        ...
        ttk.Button(toolbar, text="Sync EGI", command=self.sync_to_egi).pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Label(
            status_frame, textvariable=self.context_var, background="lightblue"
        ...
        self.validation_label = ttk.Label(
            status_frame, textvariable=self.validation_var, background="lightgreen"
        )
    ...
    def create_vertex(self, x: float, y: float) -> str:
        ...
        circle = self.canvas.create_oval(
            x - 12, y - 12, x + 12, y + 12, fill="lightblue", outline="blue", width=2
        )

        # Draw label
        label = self.canvas.create_text(
            x, y + 25, text=element_id, font=("Arial", 10), fill="black"
        )
    ...
    def find_containing_cut(self, element: Element) -> Optional[str]:
        ...
        for cut_element in self.elements.values():
            if cut_element.element_type == "cut" and cut_element != element:
    ...
    def update_transformation_buttons(self):
        ...
        for rule_name, btn in self.transform_buttons.items():
            if rule_name == "IT+":
                ...
                btn.configure(
                    state="normal" if len(self.current_egi.V) > 0 else "disabled"
                )
            elif rule_name == "DC+":
                ...
                btn.configure(
                    state="normal" if len(self.current_egi.Cut) >= 2 else "disabled"
                )
            elif rule_name == "INS":
                # Insertion only in negative areas
                btn.configure(
                    state="normal" if polarity == AreaPolarity.NEGATIVE else "disabled"
                )
            elif rule_name == "ERA":
                ...
                btn.configure(
                    state=(
                        "normal"
                        if polarity == AreaPolarity.POSITIVE and has_content
                        else "disabled"
                    )
                )
    ...
    def reconstruct_diagram_from_egi(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Reconstruction Error", f"Failed to reconstruct diagram: {e}"
            )
    ...
    def create_vertex_at_position(self, vertex_id: str, x: float, y: float):
        ...
        # Draw label
        label = self.canvas.create_text(
            x, y + 25, text=vertex_id, font=("Arial", 10), fill="black"
        )
    ...
    def create_cut_at_position(
        self, cut_id: str, center_x: float, center_y: float, radius: float
    ):
    ...
    def handle_edge_click(self, clicked_element: Element):
        ...
        if not self.edge_start_element:
            ...
            self.select_element(clicked_element.element_id)
            self.status_var.set(
                f"Edge started from {clicked_element.element_id}. Click target vertex."
            )
        else:
    ...
    def create_edge(self, start_vertex_id: str, end_vertex_id: str) -> str:
        ...
        self.status_var.set(
            f"Created edge: {edge_id} from {start_vertex_id} to {end_vertex_id}"
        )
    ...
    def highlight_nary_relation_violations(self):
        ...
        for element in self.elements.values():
            if element.element_type == "edge":
                relation_id = f"rel_{element.element_id}"
                if (
                    relation_edge_counts.get(relation_id, 0) != 2
                ):  # Binary relations should have exactly 2 connections
    ...
    def show_validation_tooltip(self, event):
        ...
        if self.last_validation_result:
            ...
            label = tk.Label(
                tooltip,
                text=self.last_validation_result,
