...
class ErgasterionEditor(QMainWindow):
    ...
    def _create_control_panel(self) -> QWidget:
        ...
        for rule_name in ["Insert Vertex", "Insert Double Cut", "Erase Vertex", "Erase Double Cut", "Iterate", "Deiterate"]:
            btn = QPushButton(rule_name)
            btn.setEnabled(False)  # Will be enabled based on context
            btn.clicked.connect(lambda checked, name=rule_name: self._apply_transformation(name))
    ...
    def _initialize_empty_sheet(self):
        ...
        self.current_egi = RelationalGraphWithCuts(
            ...
            area=frozendict({"sheet_of_assertion": frozenset()}),  # Sheet contains nothing initially
    ...
    def _update_display(self):
        ...
        # Fit view
        self.diagram_view.fitInView(self.diagram_scene.itemsBoundingRect(), Qt.KeepAspectRatio)
    ...
    def _update_available_transformations(self):
        ...
        if is_empty:
            ...
            self.transform_buttons["Insert Vertex"].setEnabled(True)
            self.transform_buttons["Insert Double Cut"].setEnabled(True)
            self.transform_buttons["Erase Vertex"].setEnabled(len(self.current_egi.V) > 0)
            self.transform_buttons["Erase Double Cut"].setEnabled(len(self.current_egi.Cut) > 0)
    ...
    def _execute_transformation(self, rule_name: str) -> Optional[RelationalGraphWithCuts]:
        ...
        if rule_name == "Insert Vertex":
            ...
            outer_cut = Cut(id=outer_cut_id, vertices=set(), edges=set(), cuts={inner_cut_id})
    ...
    def _on_graph_changed(self, egi: RelationalGraphWithCuts):
        ...
        self.status_bar.showMessage(f"Graph: {vertex_count} vertices, {edge_count} edges, {cut_count} cuts")
