...
class DiagramViewer(QWidget):
    ...
    def load_egi_dto_readonly(self, egi_dto: EGIStateDTO) -> None:
        ...
        print(f"[DiagramViewer] Loading EGI DTO with clean renderer: {len(egi_dto.vertices)} vertices, {len(egi_dto.edges)} edges, {len(egi_dto.cuts)} cuts")
    ...
    def _convert_dto_to_egi(self, egi_dto: EGIStateDTO):
        ...
        edges = frozenset(
            Edge(id=edge_id)
            for edge_id in egi_dto.edges.keys()
        )
        ...
        sheet_id = "sheet"
        all_element_ids = set(egi_dto.vertices.keys()) | set(egi_dto.edges.keys()) | set(egi_dto.cuts.keys())
