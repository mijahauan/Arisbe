...
class CorpusNavigator:
    ...
    def setup_ui(self):
        ...
        # Actions
        ttk.Button(toolbar, text="Refresh", command=self.refresh_corpus).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar, text="New Universe", command=self.create_universe).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar, text="Import EGI", command=self.import_egi).pack(side=tk.LEFT, padx=5)
        ...
        self.tree = ttk.Treeview(tree_container, columns=("type", "modified"), show="tree headings")
        ...
        # Scrollbars for tree
        tree_v_scroll = ttk.Scrollbar(tree_container, orient=tk.VERTICAL, command=self.tree.yview)
        tree_h_scroll = ttk.Scrollbar(tree_container, orient=tk.HORIZONTAL, command=self.tree.xview)
    ...
    def setup_details_panel(self):
        ...
        ttk.Button(action_frame, text="Open", command=self.open_selected).pack(side=tk.LEFT, padx=2)
        ttk.Button(action_frame, text="Export", command=self.export_selected).pack(side=tk.LEFT, padx=2)
        ttk.Button(action_frame, text="Delete", command=self.delete_selected).pack(side=tk.LEFT, padx=2)
    ...
    def populate_tree(self):
        ...
        for universe in self.universes.values():
            universe_item = self.tree.insert("", "end", 
                                           text=universe.name,
                                           values=("Universe", ""),
                                           tags=("universe",))
            ...
            for entry_id in universe.entries:
                if entry_id in self.entries:
                    ...
                    self.tree.insert(universe_item, "end",
                                   text=entry.title,
        ...
        orphaned_entries = []
        for entry in self.entries.values():
            if not any(entry.entry_id in universe.entries for universe in self.universes.values()):
                orphaned_entries.append(entry)
        
        if orphaned_entries:
            orphaned_item = self.tree.insert("", "end",
                                           text="Uncategorized",
                                           values=("Folder", ""),
                                           tags=("folder",))
            ...
            for entry in orphaned_entries:
                modified_str = entry.last_modified.strftime("%Y-%m-%d")
                self.tree.insert(orphaned_item, "end",
                               text=entry.title,
                               values=("EGI", modified_str),
    ...
    def on_tree_selection(self, event):
        ...
        if "entry" in tags and len(tags) > 1:
            ...
            universe = next((u for u in self.universes.values() if u.name == universe_name), None)
            self.current_selection = None
    ...
    def import_egi(self):
        ...
        if file_path:
            ...
            messagebox.showinfo("Import", f"Import functionality not yet implemented for {file_path}")
    ...
    def open_selected(self):
        ...
        if self.current_selection:
            ...
            messagebox.showinfo("Open", f"Opening {entry.title} - functionality not yet implemented")
    ...
    def export_selected(self):
        ...
        if self.current_selection:
            ...
            messagebox.showinfo("Export", f"Export functionality not yet implemented for {entry.title}")
...
class UniverseDialog:
    ...
    def setup_ui(self):
        ...
        # Universe ID
        ttk.Label(main_frame, text="Universe ID:").grid(row=0, column=0, sticky="w", pady=2)
        self.id_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=self.id_var, width=40).grid(row=0, column=1, sticky="ew", pady=2)
        
        # Name
        ttk.Label(main_frame, text="Name:").grid(row=1, column=0, sticky="w", pady=2)
        self.name_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=self.name_var, width=40).grid(row=1, column=1, sticky="ew", pady=2)
        
        # Description
        ttk.Label(main_frame, text="Description:").grid(row=2, column=0, sticky="nw", pady=2)
        self.description_text = tk.Text(main_frame, width=40, height=8)
        self.description_text.grid(row=2, column=1, sticky="ew", pady=2)
        
        # Domain Model
        ttk.Label(main_frame, text="Domain Model:").grid(row=3, column=0, sticky="w", pady=2)
        self.domain_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=self.domain_var, width=40).grid(row=3, column=1, sticky="ew", pady=2)
