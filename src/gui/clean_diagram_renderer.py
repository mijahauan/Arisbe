...
from PySide6.QtWidgets import QGraphicsScene, QGraphicsEllipseItem, QGraphicsTextItem, QGraphicsLineItem, QGraphicsRectItem
...
from dau_diagram_correspondence import (DauDiagramCorrespondence, 
                                      DiagramRepresentation, VertexSpot, 
                                      RelationSign, EdgeLine, CutLine)
...
class CleanDiagramRenderer:
    ...
    def __init__(self, style_config=None):
        ...
        self.vertex_radius = self.style.get('vertex', {}).get('dot', {}).get('radius', 3)
        self.cut_padding = self.style.get('layout', {}).get('cut_padding', 20.0)
        self.element_spacing = self.style.get('layout', {}).get('sibling_shift', [40.0, 30.0])[0]
        self.ligature_approach_margin = self.style.get('layout', {}).get('ligature_approach_margin', 4.0)
    ...
    def _load_style_config(self, style_config):
        ...
        if style_config is None:
            ...
            import json
            import os
            style_path = os.path.join(os.path.dirname(__file__), '..', 'styles', 'default.json')
            try:
                ...
                return {
                    'vertex': {'dot': {'radius': 3, 'fill_color': '#000000', 'border_color': '#000000'}},
                    'ligature': {'arm': {'line_color': '#000000', 'line_width': 3}},
                    'layout': {'cut_padding': 20.0, 'sibling_shift': [40.0, 30.0], 'ligature_approach_margin': 4.0}
                }
        return style_config
        
    def render_egi_to_scene(self, egi: RelationalGraphWithCuts, scene: QGraphicsScene) -> None:
        """
        ...
        layout_positions = self.correspondence.calculate_graph_aware_layout(self.current_diagram)
        layout = {elem_id: QPointF(x, y) for elem_id, (x, y) in layout_positions.items()}
        ...
        print(f"[CleanDiagramRenderer] Rendered EGI with {len(egi.V)} vertices, "
    ...
    def _build_element_chain(self) -> List[str]:
        ...
        while len(used_relations) < len(relations) or len(used_vertices) < len(vertices):
            ...
            for edge_id, edge_line in self.current_diagram.edge_lines.items():
                if (edge_line.vertex_spot_id == current_vertex and 
                    edge_line.relation_sign_id not in used_relations):
            ...
            if connected_relation:
                ...
                next_vertex = None
                for edge_id, edge_line in self.current_diagram.edge_lines.items():
                    if (edge_line.relation_sign_id == connected_relation and 
                        edge_line.vertex_spot_id != current_vertex and
                        edge_line.vertex_spot_id not in used_vertices):
    ...
    def _render_vertices(self, layout: Dict[str, QPointF]) -> None:
        ...
        for vertex_id, vertex_spot in self.current_diagram.vertex_spots.items():
            ...
            vertex_item = QGraphicsEllipseItem(
                ...
                2 * self.vertex_radius, 2 * self.vertex_radius
            )
    ...
    def _render_relations(self, layout: Dict[str, QPointF]) -> None:
        ...
        for relation_id, relation_sign in self.current_diagram.relation_signs.items():
            ...
            relation_item.setFlags(relation_item.GraphicsItemFlag.ItemIsMovable | 
    ...
    def _render_edge_lines(self, layout: Dict[str, QPointF]) -> None:
        ...
        for line_id, edge_line in self.current_diagram.edge_lines.items():
            ...
            for i in range(len(path_points) - 1):
                ...
                line_item.setPen(QPen(QColor(line_color), line_width, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            ...
            mid_point = path_points[len(path_points) // 2] if path_points else relation_pos
    ...
    def _generate_ligature_path(self, start: QPointF, end: QPointF, layout: Dict[str, QPointF]) -> List[QPointF]:
        ...
        if (not self._path_intersects_relations(start, curve_point, layout) and
            not self._path_intersects_relations(curve_point, end, layout)):
    ...
    def _path_intersects_relations(self, start: QPointF, end: QPointF, layout: Dict[str, QPointF]) -> bool:
        ...
        for relation_id, relation_sign in self.current_diagram.relation_signs.items():
            ...
            if self._line_intersects_rect(start, end, relation_pos, text_width + padding, text_height + padding):
                return True
        
        return False
    
    def _line_intersects_rect(self, line_start: QPointF, line_end: QPointF, 
                             rect_center: QPointF, rect_width: float, rect_height: float) -> bool:
        ...
        return not (line_max_x < rect_left or line_min_x > rect_right or
                   line_max_y < rect_top or line_min_y > rect_bottom)
    ...
    def _render_cuts(self, layout: Dict[str, QPointF]) -> None:
        ...
        for cut_id, cut_line in self.current_diagram.cut_lines.items():
            ...
            if contents:
                ...
                content_positions = [layout.get(elem_id) for elem_id in contents 
                                   if elem_id in layout]
...
if __name__ == "__main__":
    ...
    print(f"✓ Created test EGI: {len(graph.V)} vertices, {len(graph.E)} edges, {len(graph.Cut)} cuts")
    ...
    print(f"✓ Converted to diagram: {len(diagram.vertex_spots)} vertex-spots, "
