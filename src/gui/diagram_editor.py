...
from dau_diagram_correspondence import (
    ...
    RelationSign, EdgeLine, CutLine, ConstraintViolation
)
...
class DiagramEditor:
    ...
    def setup_ui(self):
        ...
        context_label = ttk.Label(status_frame, textvariable=self.context_var, relief=tk.SUNKEN, background="lightblue")
    ...
    def create_toolbar(self, parent):
        ...
        ttk.Button(toolbar, text="Export", command=self.export_diagram).pack(side=tk.LEFT, padx=5)
        ...
        self.transform_buttons["IT+"] = ttk.Button(toolbar, text="IT+", command=lambda: self.apply_transformation("IT+"))
        self.transform_buttons["IT+"].pack(side=tk.LEFT, padx=2)
        
        self.transform_buttons["IT-"] = ttk.Button(toolbar, text="IT-", command=lambda: self.apply_transformation("IT-"))
        self.transform_buttons["IT-"].pack(side=tk.LEFT, padx=2)
        
        self.transform_buttons["DC+"] = ttk.Button(toolbar, text="DC+", command=lambda: self.apply_transformation("DC+"))
        self.transform_buttons["DC+"].pack(side=tk.LEFT, padx=2)
        
        self.transform_buttons["DC-"] = ttk.Button(toolbar, text="DC-", command=lambda: self.apply_transformation("DC-"))
        self.transform_buttons["DC-"].pack(side=tk.LEFT, padx=2)
        
        self.transform_buttons["INS"] = ttk.Button(toolbar, text="INS", command=lambda: self.apply_transformation("INS"))
        self.transform_buttons["INS"].pack(side=tk.LEFT, padx=2)
        
        self.transform_buttons["ERA"] = ttk.Button(toolbar, text="ERA", command=lambda: self.apply_transformation("ERA"))
    ...
    def on_tool_changed(self):
        ...
        self.status_var.set(f"Tool: {self.current_tool.value.title()} - {tool_messages[self.current_tool]}")
    ...
    def create_vertex(self, x: float, y: float) -> str:
        ...
        label_id = self.canvas.create_text(
            x, y + 25, text=vertex.label, font=("Arial", 10),
            tags=f"element_{element_id}"
        )
    ...
    def apply_transformation(self, transformation_type: str):
        ...
        if not self.selected_elements:
            messagebox.showwarning("No Selection", "Please select elements to transform")
            return
    ...
    def draw_context_background(self):
        ...
        if self.current_context == "sheet":
            ...
            self.canvas.create_rectangle(
                10, 10, 1990, 1990,
                outline="darkblue", width=2, fill="", dash=(10, 5),
                tags="context_bg"
            )
            # Add corner label
            self.canvas.create_text(
                30, 30, text="Sheet of Assertion (Positive)", 
                font=("Arial", 12, "bold"), fill="darkblue",
                anchor="nw", tags="context_bg"
            )
        else:
            ...
            self.canvas.create_text(
                ...
                font=("Arial", 10, "bold"), fill=border_color,
                anchor="nw", tags="context_bg"
            )
