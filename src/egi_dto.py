        ...
        for edge_id, vertex_ids in self.nu_mapping.items():
            ...
            for vertex_id in vertex_ids:
                if vertex_id not in all_vertex_ids:
                    raise ValueError(f"nu_mapping references unknown vertex: {vertex_id}")
...
def egi_to_dto(egi: RelationalGraphWithCuts, spatial_data: Dict[str, SpatialInfo] = None) -> EGIStateDTO:
...
def dto_to_egi(dto: EGIStateDTO) -> RelationalGraphWithCuts:
    ...
    edges = frozenset(
        Edge(id=e.id)
        for e in dto.edges.values()
    )
    
    # Convert cuts
    cuts = frozenset(
        Cut(id=c.id)
        for c in dto.cuts.values()
    )
    
    # Build relation mapping
    rel_mapping = frozendict({
        e.id: e.relation_name
        for e in dto.edges.values()
    })
