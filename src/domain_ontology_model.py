...
class DomainContext:
    ...
    natural_language_forms: Dict[str, str] = field(
...
class DomainModelManager:
    ...
    def register_ontology(
        ...
        connector: Optional[OntologyConnector] = None,
    ):
    ...
    def create_domain_context(
        ...
        description: str,
        primary_ontology_id: str,
        secondary_ontologies: List[str] = None,
    ) -> str:
    ...
    def create_semantic_annotation(
        self,
        target_elements: Set[ElementID],
        domain_context_id: str,
        natural_language: str,
        ...
        annotation = SemanticAnnotation(
            ...
            logical_form_description_logic=(
                logical_forms.get("dl") if logical_forms else None
            ),
        )
    ...
    def get_element_contexts(self, element_id: ElementID) -> List[DomainContext]:
        ...
        return [
            self.domain_contexts[cid]
            for cid in context_ids
            if cid in self.domain_contexts
        ]

    def get_element_concept_mappings(
    ...
    def export_domain_model_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "element_to_contexts": {
                elem_id: list(contexts)
                for elem_id, contexts in self.element_to_contexts.items()
            },
        }
        ...
        for ctx_id, ctx_data in data.get("domain_contexts", {}).items():
            ...
            for ann_id, ann_data in ctx_data.get("semantic_annotations", {}).items():
                semantic_annotations[ann_id] = SemanticAnnotation(
                    ...
                    logical_form_description_logic=ann_data[
                        "logical_form_description_logic"
                    ],
                )
    ...
    def lookup_concept_in_ontology(
        self, ontology_id: str, concept_uri: str
    ...
    def generate_natural_language_for_subgraph(
        self, elements: Set[ElementID], context_id: Optional[str] = None
    ) -> str:
    ...
    def validate_domain_consistency(self, context_id: str) -> List[str]:
        ...
        if context.primary_ontology_id not in self.ontology_references:
            issues.append(
                f"Primary ontology {context.primary_ontology_id} not registered"
            )
