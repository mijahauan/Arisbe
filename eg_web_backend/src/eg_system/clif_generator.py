...
class CLIFGenerator:
    ...
    def _generate_context_content(self, graph: EGGraph, context_id: ContextId) -> str:
        ...
        # Process ligatures (equalities) first
        for ligature in graph.ligatures.values():
            if ligature.properties.get('type') == 'equality':
                # Check if ligature nodes are in this context
                ligature_nodes = [graph.nodes.get(node_id) for node_id in ligature.nodes 
                                if node_id in graph.nodes]
                
                if len(ligature_nodes) == 2:
                    # Check if both nodes are in this context
                    nodes_in_context = [node for node in ligature_nodes 
                                      if node.id in direct_items]
                    
                    if len(nodes_in_context) == 2:
                        term1 = self._get_term_value(nodes_in_context[0])
                        term2 = self._get_term_value(nodes_in_context[1])
                        if term1 and term2:
                            content_parts.append(f"(= {term1} {term2})")
