...
- **The Agon interpretation register** (`docs/GENERATION_AND_TESTING.md`, `docs/DOMAIN_ORACLE_AND_M.md`) — the inning *given M, then G*: choose M → peel → decide. The conceptual cut is **eliminative (the game = Agon/testing) vs additive (making = Ergasterion)**; deduction earns the corpus through Agon (self-certifies validity, not warrant).
  ...
  - `model_revision.py` — the minimal step by which a model **M transforms through ongoing dialog** (docs/EXEMPLARS.md §6): `assert_fact` (enlargement — admit a `new_fact` disposition as a low-warrant posit, juxtaposed onto M's sheet) + `retract_relation` (relinquishment — *free to demote*, the ERA dual) + `revise_with_disposition`. A revised M is a diachronic UoD carrying its own history. Geometry-free; the `dialogue_model_revision` exemplar flips an audited verdict FALSE→TRUE→FALSE→TRUE as M grows.
- `modal_query.py` — read **◇ / □ off the diachronic branching DAG** with no modal mark (the *trajectory reading* of `docs/MODALITY_WITHOUT_GAMMA.md` §1): `possibly`/`necessarily` over a chain's reachability (`reachable_states`/`leaf_states`) — ◇φ = "some legal trajectory scribes φ", □φ = "every legal trajectory scribes φ" (*possibility is branching, necessity is convergence*). Predicate helpers `scribes_relation`/`equals_graph`/`is_blank`; `over="states"|"leaves"`. Geometry-free (DAG + EGI content only), adds no §3.3 obligation. The `possible_and_necessary` exemplar demonstrates it.
