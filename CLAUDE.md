...
- **The Agon interpretation register** (`docs/GENERATION_AND_TESTING.md`, `docs/DOMAIN_ORACLE_AND_M.md`) — the episode *given M, then G*: choose M → peel → decide. The conceptual cut is **eliminative (the game = Agon/testing) vs additive (making = Ergasterion)**; deduction earns the corpus through Agon (self-certifies validity, not warrant).
  ...
  - `agon_llm.py` — **the LLM Graphist, Stage 1 of the *automated Endoporeutic Game*** (three LLM roles — Graphist·doubt / Grapheus·defense / Agonothetes·judge — playing the game autonomously; docs/AUTOMATED_ENDOPOREUTIC_GAME.md). This increment builds the **Graphist** as an `agon_evolution.Proposer`; the Grapheus/Agonothetes stay the **mechanical** `Agonothetes()` panel. Governing principle *the LLM argues, the calculus decides*: each round the Graphist reads M's **thin spots** (`attention_brief` — relations with ≤1 instance, ungrounded laws, lonely individuals; the `m_render` neighbourhood idea) and voices **one doubt** via forced tool-use (`propose_graph`, emitting FOL + `doubt_type`); Arisbe **reduces it to a calculus artifact** (`nl_to_logic.build_proposal` → EGIF, `_normalize_fol` maps Capitalized FOL predicates onto M's lowercase vocabulary) and re-checks it — an unparseable doubt never reaches the loop; the mechanical peel decides truth-in-M. Mirrors `nl_to_logic` exactly: optional `nl` extra (`ANTHROPIC_AVAILABLE`), **injectable client**, **never raises** (retries with the parse error fed back, then returns `None` to end the run cleanly). Reproduces the swan trajectory *even when the LLM emits Capitalized predicates*; bootstraps a model **from the blank sheet**. Demo `tools/build_llm_graphist_demo.py` (key-gated). Stages 2–3 (LLM Grapheus defense; LLM Agonothetes + branch-the-DAG) are future.
...
- `test_agon_llm.py` — the LLM Graphist (Stage 1 automated EPG), CI-safe with a **scripted fake client** (no SDK/key): `_normalize_fol` maps Capitalized FOL predicates onto M's vocabulary; `attention_brief` invites bootstrapping on the blank sheet and names thin spots otherwise; `propose` reduces FOL→EGIF and **retries on unparseable then recovers**, returns `None` (never raises) when all attempts fail or the client is unreachable, and avoids repeats; the headline — fed the swan doubts (emitted as *Capitalized* FOL) the Graphist drives the mechanical panel to **reproduce the swan trajectory**; bootstraps a model from the blank sheet; the trajectory §3.3-persists. A live test is skipped unless the `nl` extra + key are present
