...
def cmd_new(args: argparse.Namespace) -> None:
    ...
    if isinstance(metadata.get("source"), dict):
        metadata["source"] = {
            k: v for k, v in metadata["source"].items() if v is not None
        }
    ...
    if args.enrich:
        code = run_script(
            "enrich_corpus_entries.py", "--root", str(target_dir), "--write"
        )
        if code != 0:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    pn.add_argument(
        "--from-egi", help="Path to an existing .egi.json to copy adjacent to metadata"
    )
    pn.add_argument("--overwrite", action="store_true")
    pn.add_argument(
        "--enrich", action="store_true", help="Run enrichment after creating"
    )
    ...
    pe.add_argument(
        ...
        action="append",
        default=[],
        help="Glob of entries to include (relative to root)",
    )
    pe.add_argument(
        "--exclude",
        action="append",
        default=[],
        help="Glob of entries to exclude (relative to root)",
    )
    ...
    pie = sub.add_parser(
        "ingest-multi-egif",
        help="Split a multi-example .egif file into per-example EGI + metadata",
    )
    pie.add_argument(
        "--file", required=True, help="Path to .egif containing multiple examples"
    )
