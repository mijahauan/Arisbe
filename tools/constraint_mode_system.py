...
class ConstraintMode(Enum):
    ...
    SYNTACTIC_ONLY = (
        "syntactic_only"  # Free creation/editing - only syntactic constraints
    )
    SEMANTIC_LOCKED = (
        "semantic_locked"  # Locked to reference EGI - both syntactic and semantic
    )
...
class ConstraintManager(QObject):
    ...
    def validate_element_position(
        self, element_type: str, element_id: str, new_position, size=None
    ) -> tuple[bool, str]:
        """
        ...
        overlap_result = self._check_element_overlaps(
            element_type, element_id, new_position, size
        )
    ...
    def _check_element_overlaps(
        self, element_type: str, element_id: str, position, size=None
    ) -> tuple[bool, str]:
        ...
        if element_type == "cut":
            for other_cut_id, other_cut in diagram_state.cuts.items():
                ...
                if self._rectangles_overlap(
                    position, size, other_cut.position, other_cut.size
                ):
    ...
    def validate_element_area_change(
        self, element_type: str, element_id: str, new_area_id: str
    ) -> tuple[bool, str]:
        """
        ...
        if current_area and current_area != new_area_id:
            return (
                False,
                f"Semantic constraints: {element_type} {element_id} cannot move from area {current_area} to {new_area_id}",
            )

        return True, ""

    def validate_name_change(
        self, element_type: str, element_id: str, new_name: str
    ) -> tuple[bool, str]:
        """
        ...
        return (
            False,
            f"Semantic constraints: Cannot change {element_type} name while locked to reference EGI",
        )

    def validate_arity_change(
        self, predicate_id: str, new_arity: int
    ) -> tuple[bool, str]:
        """
        ...
        return (
            False,
            f"Semantic constraints: Cannot change predicate arity while locked to reference EGI",
        )
    ...
    def get_constraint_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "can_enable_semantic": self.reference_egi is not None
            and self.has_confirmed_match,
        }
...
class ConstraintModeWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.free_edit_btn.setToolTip(
            "Enable free editing with only syntactic constraints"
        )
        button_layout.addWidget(self.free_edit_btn)

        self.lock_to_egi_btn = QPushButton("Lock to EGI")
        self.lock_to_egi_btn.setToolTip(
            "Lock diagram to reference EGI (semantic constraints)"
        )
    ...
    def _update_display(self):
        ...
        # Update mode label
        mode_text = (
            "Free Edit" if status["mode"] == "syntactic_only" else "Locked to EGI"
        )
        ...
        self.lock_to_egi_btn.setEnabled(
            status["can_enable_semantic"] and status["mode"] != "semantic_locked"
        )
        self.confirm_match_btn.setEnabled(
            status["has_reference_egi"] and not status["match_confirmed"]
        )
