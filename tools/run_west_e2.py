...
def main():
    ...
    print("notes: A3 PAID DOWN — the coordinator tax is now a true per-round "
          ...
          "carries no Horn laws — true, not a bug). A weak fit (n<6 or "
          "r2<0.90) makes its prior 'undetermined', never held or refuted. "
          "The canary runs ONE config (F=6), not the whole sweep — a "
          "narrowing relative to E1, disclosed. Synthetic corpus: this is an "
          "exponent of the generator's topology, not of real reasoning "
          "corpora.", flush=True)
