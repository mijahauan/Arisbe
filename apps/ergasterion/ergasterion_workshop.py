...
class ErgasterionWorkshop(QMainWindow):
    ...
    def _add_vertex(self):
        ...
        # Implementation would add vertex to current EGI
        self.status_bar.showMessage("Add vertex - click on canvas")
    ...
    def _add_predicate(self):
        ...
        self.status_bar.showMessage("Add predicate - click on canvas")
    ...
    def _add_cut(self):
        ...
        self.status_bar.showMessage("Add cut - drag to define area")
    ...
    def _update_displays(self):
        ...
        try:
            # Create spatial primitives for canvas display
            spatial_primitives = self._create_spatial_primitives(self.current_egi)
            
            # Update canvas
            self.canvas_widget.set_graph(self.current_egi, spatial_primitives)
            
            # Update EGIF display
            from canonical import CanonicalPipeline
            pipeline = CanonicalPipeline()
            egif_output = pipeline.egi_to_egif(self.current_egi)
            self.egif_display.setPlainText(egif_output)
            
        except Exception as e:
            self.status_bar.showMessage(f"Error updating displays: {e}", 5000)
    
    def _create_spatial_primitives(self, egi):
        """Create spatial primitives for EGI elements."""
        primitives = []
